...
declare
	...
	num_nodes	int;
begin
	nodenames_query := 'SELECT srvname FROM pg_foreign_server WHERE srvtype = ''D''';
...
BEGIN
	...
	IF (distribution IS NOT NULL) then
		cr_command := cr_command  || ', with(' || distribution ||')';
	END IF;
...
DECLARE
	r		pg_foreign_server%rowtype;
	node		int;
	nodenames_query	varchar;
BEGIN
	nodenames_query := 'SELECT * FROM pg_foreign_server WHERE srvtype = ''D'' ORDER BY srvname';
