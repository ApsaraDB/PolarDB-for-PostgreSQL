...
static bool pgfdw_get_cleanup_result(PGconn *conn, TimestampTz endtime,
						 PGresult **result);
static void polarx_fdw_xact_callback(XactEvent event, void *arg);


void
RegisterFdwTxnCallback(void)
{
	RegisterXactCallback(polarx_fdw_xact_callback, NULL);
}
/*
...
/*
 * Get all PGconn which can be used to execute queries on the remote polarx-DB DNs
 * server with the user's authorization.  A new connection is established
 * if we don't already have a suitable one, and a transaction is opened at
 * the right subtransaction nesting depth if we didn't do that already.
 *
 ...
 */
PGconn **
GetConnections(UserMapping *user, bool will_prep_stmt, List *node_list, int *conn_num)
{
	PGconn		**connRes;
	ListCell	*lc;
	ForeignServer	*server = GetForeignServer(user->serverid);
	int             connNum = (node_list == NIL)? 0 : list_length(node_list);
	int		currentInx = 0;
	...
	connRes = (PGconn **) palloc0(connNum * sizeof(PGconn *));
	...
	foreach(lc, node_list)
	{
		...
		if (entry->conn == NULL)
		{
			server->options = GetSingleNodeLibpgOption(server->options, key.index);
 ...
 */
void
ReleaseConnectionCluster(PGconn **conns)
{
	/*
	 * Currently, we don't actually track connection references because all
	 * cleanup is managed on a transaction or subtransaction basis instead. So
...
/*
 * Submit a query and no wait for the result.
 *
 * This function is interruptible by signals.
 *
 * Caller is responsible for the error handling on the result.
 */
void
pgfdw_async_exec_query_single(PGconn *conn, const char *query)
{
	/*
	 * Submit a query.  Since we don't use non-blocking mode, this also can
	 * block.  But its risk is relatively small, so we ignore that for now.
...
void
pgfdw_exec_query_cluster(PGconn **conns, const char *query, int conn_num)
{
	int i = 0;

	for(i = 0; i < conn_num; i++)
	{
		pgfdw_async_exec_query_single(conns[i], query);
	}
}

void
pgfdw_exec_query_cluster_custom(PGconn **conns, const char *query, int conn_num, bool *conn_finish)
{
	int i = 0;
...
pgfdw_reject_incomplete_xact_state_change(ConnCacheEntry *entry)
{
	...
	ereport(ERROR,
			(errcode(ERRCODE_CONNECTION_EXCEPTION),
			 errmsg("connection to server \"%s\" : index %d was lost",
					server->servername, entry->key.index)));
}
