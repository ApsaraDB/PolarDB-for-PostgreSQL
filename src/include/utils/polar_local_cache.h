...
typedef void (*polar_segment_file_path) (char *full_path, uint64 segno, bool long_segment_names);
...
typedef struct polar_local_cache_data
{
	...
	bool		long_segment_names; /* If true, use long segment file names.
									 * Otherwise, use short file names. For
									 * details about the file name format, see
									 * SlruFileName(). */
...
#define POLAR_LOCAL_FILE_PATH(path, cache, segno) \
	do { \
		...
		(cache)->seg_path((path)+strlen(path), (segno), (cache)->long_segment_names); \
	} while (0)
...
polar_local_cache polar_create_local_cache(const char *cache_name, const char *dir_name,
										   ...
										   int tranche_id, uint32 io_permission, bool long_segment_names,
...
void		polar_validate_data_dir(const char *dir_name);
