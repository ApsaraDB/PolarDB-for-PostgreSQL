...
extern int	polar_wait_xlog_meta_timeout;
extern double polar_xlog_queue_data_keep_ratio;

/* Define the maximum ratio for keep data in xlog queue */
#define POLAR_XLOG_QUEUE_MAX_DATA_KEEP_RATIO (0.6)
...
extern DecodedXLogRecord *polar_xlog_send_queue_pop(polar_ringbuf_t queue, polar_ringbuf_ref_t *ref, XLogReaderState *state);
extern bool polar_xlog_send_queue_keep_data(polar_ringbuf_t queue, uint64_t new_pread);
...
extern DecodedXLogRecord *polar_xlog_queue_decode_record(polar_ringbuf_ref_t *ref, ssize_t offset, uint32 pktlen,
														 XLogRecPtr read_rec_ptr, XLogRecPtr end_rec_ptr, XLogReaderState *state, bool decode_payload);
