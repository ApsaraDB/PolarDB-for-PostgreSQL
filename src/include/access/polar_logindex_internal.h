...
#define LOG_INDEX_TABLE_CACHE_SIZE    (LOG_INDEX_TABLE_NUM_PER_FILE * sizeof(log_index_table_t))
#define LOG_INDEX_GET_CACHE_TABLE(cache, tid) \
	(((tid) < (cache)->min_idx_table_id || (tid) > (cache)->max_idx_table_id) ? NULL : \
	 (log_index_table_t *)((cache)->data + sizeof(log_index_table_t) * ((tid) - (cache)->min_idx_table_id)))
...
#define LOG_INDEX_FILE_TABLE_SEGMENT_OFFSET(tid) \
	((((tid)-1) % LOG_INDEX_TABLE_NUM_PER_FILE) * sizeof(log_index_table_t))
 ...
 * 2. The hash lock size is LOG_INDEX_MEM_TBL_HASH_LOCK_NUM
 * 3. One lwlock for logindex meta io
 * 4. lwlock for flush active tbl, only used by fullpage snapshot now
 */
...
#define LOG_INDEX_HASH_LOCK_OFFSET                  (LOG_INDEX_MEMTBL_LOCK_OFFSET + logindex_snapshot->mem_tbl_size)
...
typedef struct log_index_lsn_info_t
{
	BufferTag  *tag;
	XLogRecPtr	lsn;
	XLogRecPtr	prev_lsn;
} log_index_lsn_info_t;

typedef log_index_lsn_info_t *log_index_lsn_info;
typedef bool (*log_index_lsn_info_cmp) (log_index_lsn_info info_a, log_index_lsn_info info_b);
...
typedef struct log_index_table_t
{
...
typedef struct log_index_mem_table_t
{
	log_seg_id_t free_head;
	pg_atomic_uint32 state;
	/* The following data will be saved to file */
	log_index_table_t data;
} log_index_mem_table_t;

typedef log_index_mem_table_t *log_index_mem_table;

typedef struct log_index_table_bloom_t
{
	log_idx_table_id_t idx_table_id;
	XLogRecPtr	min_lsn;
	XLogRecPtr	max_lsn;
...
typedef struct log_index_promoted_info_t
{
	...
	XLogRecPtr	old_primary_max_inserted_lsn;	/* The max lsn generated by
												 * primary node */
	log_idx_table_id_t old_primary_max_tid; /* The max logindex table id
...
typedef struct log_index_snapshot_t
{
	...
	pg_atomic_uint64 flush_active_table_lsn;
	log_idx_table_id_t max_idx_table_id;
	...
	log_index_mem_table_t mem_table[FLEXIBLE_ARRAY_MEMBER];
} log_index_snapshot_t;

typedef struct log_index_snapshot_t *log_index_snapshot;
...
typedef struct log_index_page_iter_t
{
	BufferTag	tag;
	XLogRecPtr	min_lsn;
	XLogRecPtr	max_lsn;
...
typedef log_index_page_iter_t *log_index_page_iter;
...
typedef struct log_index_lsn_iter_t
{
	XLogRecPtr	start_lsn;
	log_index_iter_state_t state;
	log_idx_table_id_t idx_table_id;
...
typedef struct log_index_lsn_iter_t *log_index_lsn_iter;
...
typedef bool (*log_index_table_flushable_hook_type) (log_index_snapshot logindex_snapshot, log_index_mem_table table);
extern PGDLLIMPORT log_index_table_flushable_hook_type polar_logindex_table_flushable_hook;

extern log_item_head_t *log_index_tbl_find(BufferTag *tag, log_index_table_t *table, uint32 key);
extern void log_index_insert_lsn(log_index_snapshot logindex_snapshot, log_index_lsn_info lsn_info, uint32 key);
extern XLogRecPtr log_index_item_max_lsn(log_index_table_t *table, log_item_head_t *item);
extern pg_crc32 log_index_calc_crc(unsigned char *data, size_t size);

extern bool log_index_write_table(log_index_snapshot logindex_snapshot, log_index_mem_table table);
extern log_index_table_t *log_index_read_table(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid, log_index_mem_table *mem_table);
extern log_index_table_bloom_t *log_index_get_tbl_bloom(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid);
extern bool log_index_get_meta(log_index_snapshot logindex_snapshot, log_index_meta_t *meta);
extern void log_index_force_save_table(log_index_snapshot logindex_snapshot, log_index_mem_table table);
extern bool log_index_read_table_data(log_index_snapshot logindex_snapshot, log_index_table_t *table, log_idx_table_id_t tid, int elevel);

extern XLogRecPtr log_index_get_order_lsn(log_index_table_t *table, uint32 order, log_index_lsn_info lsn_info);

extern bool polar_logindex_table_flushable(log_index_snapshot logindex_snapshot, log_index_mem_table table);
extern void polar_logindex_init_lwlock_array(log_index_snapshot logindex_snapshot, int offset, int size, int tranche_id);

/* Log log_index_meta_t's detail */
...
/* Log log_index_table_t's detail */
...
/* Log log_index_mem_table_t's detail */
...
log_index_item_head(log_index_table_t *table, log_seg_id_t head)
{
...
log_index_item_seg(log_index_table_t *table, log_seg_id_t seg)
{
