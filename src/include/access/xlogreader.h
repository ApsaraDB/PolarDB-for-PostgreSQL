...
#define POLAR_XLOG_BULK_READ_BUFFER_END_LSN(state) \
	(state->bulk_read_buffer_start + XLOG_BLCKSZ * state->bulk_read_buffer_len)

#define POLAR_XLOG_BULK_READ_BUFFER_AVAILABLE(state, target_page_ptr) \
	(state->bulk_read_buffer && \
	 target_page_ptr >= state->bulk_read_buffer_start && \
	 target_page_ptr < POLAR_XLOG_BULK_READ_BUFFER_END_LSN(state))

#define POLAR_XLOG_BULK_READ_BUFFER_INVALIDATE(state)		\
	do														\
	{														\
