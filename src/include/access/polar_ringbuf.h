...
typedef struct polar_ringbuf_stat
{
	...
	/* The Physical IO counter of recv queue ref. */
	pg_atomic_uint64 recv_phys_io_cnt;
...
#define POLAR_RINGBUF_INVALID_PREAD				(UINT64_MAX)
#define POLAR_RINGBUF_MIN_PREAD   				(0)
...
extern bool polar_ringbuf_set_ref_strong_property(polar_ringbuf_ref_t *ref, bool strong);
...
static inline ssize_t
polar_ringbuf_free_size_at_pwrite(polar_ringbuf_t rbuf, uint64 pwrite)
{
	ssize_t		free_bytes;

	free_bytes = pwrite - pg_atomic_read_u64(&rbuf->pread);
	Assert(free_bytes >= 0);
 ...
 * And set packet length when data is ready for read
 */
static inline uint8
 ...
 */
static inline void
polar_ringbuf_set_pkt_flag(polar_ringbuf_t rbuf, uint64 pwrite, uint8 flag)
{
	/* ensure all previous writes are visible before follower continues. */
	pg_write_barrier();
	rbuf->data[pwrite % rbuf->size] = flag;
}
