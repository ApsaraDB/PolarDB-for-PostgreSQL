...
#define POLAR_BUFFER_EXTEND_SIZE(LEN)	(LEN + PG_IO_ALIGN_SIZE)
#define POLAR_BUFFER_ALIGN(LEN)			TYPEALIGN(PG_IO_ALIGN_SIZE, LEN)

/*
 * polar VFS interface kind
 */
...
static inline int
polar_fdatasync(int fd)
{
	int			rc = 0;

	if (polar_vfs[polar_vfs_switch].vfs_fdatasync)
		rc = polar_vfs[polar_vfs_switch].vfs_fdatasync(fd);
	return rc;
}
