 ...
 *
 * IDENTIFICATION
 *	  src/include/storage/polar_procpool.h
 *
 *-------------------------------------------------------------------------
 */
...
#include "storage/polar_shmqueue.h"
...
extern int	polar_replay_worker_smgrdestroyall_multiplier;
extern int	polar_subproc_delay;
...
#define POLAR_TASK_NODE_INIT_STATUS(t)      (pg_atomic_init_u32(&(t)->task_status, POLAR_TASK_NODE_IDLE))
...
typedef struct polar_task_node_t
{
	...
	uint64		add_seq;
	uint64		finished_seq;
	pg_atomic_uint32 task_status;	/* The first 16bit indicates which sub
									 * process own this task; and the last
									 * 16bit indicates the status of this task */
} polar_task_node_t;
...
typedef struct polar_task_sched_t
{
	...
	slock_t		lock;
	SHM_QUEUE  *running_queue_head; /* The address of the head in the running
									 * task queue */
	uint64		added_seq;
...
typedef struct polar_sub_proc_t
{
	...
	bool		ring_full;
	polar_task_node_t *ring_task_nodes_begin;	/* The start address of the
												 * ring */
	polar_task_node_t *ring_task_nodes_end; /* The end address of the ring */
	uint64		adding_tasks_num;	/* The number of tasks dispatched to this
									 * process after the last wakeup signal */
} polar_sub_proc_t;
...
typedef struct polar_task_sched_ctl_t
{
	...
	uint32		dst_proc;		/* Try to dispatch to this process for the
								 * next task */
	...
	uint64		fail_add_task_num;	/* The fail times when try to add task but
									 * the running queue is full */
	uint64		finished_task_num;	/* The number of finished tasks */
	uint64		hold_task_num;	/* Total number of tasks which status is hold
								 * when dispatch */
	uint64		reset_task_num; /* Total number reset task's status to be idle
								 * when it's finished */
...
extern bool polar_sched_broadcast_task(polar_task_sched_ctl_t *ctl, polar_task_node_t *task);
