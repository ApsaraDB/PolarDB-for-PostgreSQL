 ...
 * (1) In replica node.
 * The startup process, the logindex worker and the backends
 * could read the same xlog block more than one time. But we
 * should use it more carefully with xlog queue together in
 * startup process.
...
typedef struct polar_xlog_buffer_ctl_t
{
	...
	char	   *buffers;
	LWLockPadded *buffer_lock_array;
...
extern void polar_xlog_buffer_dccheck(void);
