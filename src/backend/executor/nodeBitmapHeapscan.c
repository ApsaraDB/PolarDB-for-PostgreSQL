...
ExecInitBitmapHeapScan(BitmapHeapScan *node, EState *estate, int eflags)
{
	...
	/*
	 * Unfortunately it turns out that the below optimization does not take
	 * the removal of TIDs by a concurrent vacuum into account. The concurrent
	 * vacuum can remove dead TIDs and make pages ALL_VISIBLE while those dead
	 * TIDs are referenced in the bitmap. This would lead to a !need_tuples
	 * scan returning too many tuples.
	 *
	 * In the back-branches, we therefore simply disable the optimization.
	 * Removing all the relevant code would be too invasive (and a major
	 * backpatching pain).
	 */
