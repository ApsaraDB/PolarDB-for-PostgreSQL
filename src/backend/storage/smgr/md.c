	   ...
	   void *buffer)
{
	...
	if (nbytes != BLCKSZ)
	{
		 ...
		 *
		 * POLAR: If it's in logindex background process it may replay xlog
		 * which block doesn't exist in the storage, like FPI or init page.So
		 * we should to init this buffer to be zero if it doesn't exist.
		 */
