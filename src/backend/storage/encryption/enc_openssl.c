...
{
	ossl_EVP_cipher_func cipherfunc;
	...
	cipherfunc = cipher_func_table[t_data_encryption_cipher - 1];
...
setup_encryption_ossl(void)
{
	 ...
	 * since AES key wrap algorithms have introduced in openssl 1.1.0 we
	 * require 1.1.0 or higher version for cluster encryption.
	 */
	ereport(ERROR,
			(errcode(ERRCODE_CONFIG_FILE_ERROR),
			 (errmsg("cluster encryption is not supported because TDE is not supported by this build"),
			  errhint("Compile with --with-tde to use cluster encryption."))));
#endif
