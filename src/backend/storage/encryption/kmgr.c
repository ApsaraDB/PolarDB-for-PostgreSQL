 ...
 */
keydata_t	keyEncKey[TDE_KEK_SIZE];
...
const char *tde_key_types[TDE_MAX_DEK] = {
	"relation",
	"wal"
};
...
void		generate_key_and_hmac(keydata_t * key, keydata_t * hmackey,
...
BootStrapKmgr(int bootstrap_data_encryption_cipher)
{
	...
	/*
	 * Set data encryption cipher so that subsequent bootstrapping process can
	 * proceed.
	 */
	...
	/*
	 * Generate relation encryption key and WAL encryption key. The generated
	 * two keys must be stored in relEncKey and walEncKey that can be used by
	 * other modules since even during bootstrapping we need to encrypt both
	 * systemcatalogs and WAL.
	 */
									...
									keydata_t hmackey_out[TDE_HMAC_KEY_SIZE])
{
	...
	pg_sha512_update((pg_sha512_ctx *) &ctx, (const uint8 *) passphrase, passlen);
	pg_sha512_final((pg_sha512_ctx *) &ctx, enckey_and_hmackey);

	/*
	 * SHA-512 results 64 bytes. We extract it into two keys for each 32
	 * bytes: one for key encryption and another one for HMAC.
	 */
...
polar_read_kmgr_file(void)
{
	...
	/*
	 * When POLARDB read it in the postmaster, shared storage has not been
	 * mounted in the plugin, so please assert it.
	 */
	...
	fd = BasicOpenFile(polar_kmgr_file, O_RDONLY | PG_BINARY);
...
write_kmgr_file(KmgrFileData * filedata)
{
	...
	fd = BasicOpenFile(polar_kmgr_file, PG_BINARY | O_CREAT | O_RDWR);
