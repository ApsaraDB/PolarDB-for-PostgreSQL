...
polar_init_xlog_buffer(char *prefix)
{
	...
	polar_xlog_buffer_ins->buffer_lock_array = (LWLockPadded *)
		ShmemInitStruct(name,
						block_count * sizeof(LWLockPadded),
						&foundLock);
...
polar_xlog_buffer_shmem_size(void)
{
	...
	/* size of xlog page buffer descriptor */
	...
	size = add_size(size, mul_size(block_count, sizeof(LWLockPadded)));
	/* to allow aligning the above */
	size = add_size(size, PG_CACHE_LINE_SIZE);
...
static void
polar_xlog_buffer_precheck(XLogRecPtr cur_page_lsn, int len)
{
...
polar_xlog_buffer_lookup(XLogRecPtr cur_page_lsn, int len, char *cur_page)
{
	...
	polar_xlog_buffer_precheck(cur_page_lsn, len);
...
void
polar_xlog_buffer_dccheck(void)
{
	...
	if (!AmStartupProcess() || !polar_xlog_buffer_ins)
		return;

	max_valid_lsn = GetCurrentReplayRecPtr(NULL);
	block_count = (Size) (polar_xlog_page_buffers * 1024 / (XLOG_BLCKSZ / 1024));

	for (buf_id = 0; buf_id < block_count; ++buf_id)
	{
		buf = polar_get_xlog_buffer_desc(buf_id);
		LWLockAcquire(polar_xlog_buffer_desc_get_lock(buf), LW_SHARED);
		POLAR_ASSERT_PANIC(buf->start_lsn <= buf->end_lsn);
		POLAR_ASSERT_PANIC(buf->end_lsn <= max_valid_lsn);
		LWLockRelease(polar_xlog_buffer_desc_get_lock(buf));
	}
#endif
}
