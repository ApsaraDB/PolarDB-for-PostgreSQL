...
#define start_copy_buffer_io(buf, for_input)	\
	(polar_start_buffer_io_extend(buf, for_input, false, true))
	...
	polar_copy_buffer_blocks = (char *)
		TYPEALIGN(PG_IO_ALIGN_SIZE,
				  ShmemInitStruct("Copy Buffer Blocks",
								  ((polar_copy_buffers * (Size) BLCKSZ) + PG_IO_ALIGN_SIZE),
								  &found_copy_bufs));
							   ...
							   bool check_io)
{
	...
	if (XLogRecPtrIsInvalid(oldest_lsn))
	{
		if (unlikely(polar_enable_debug))
		{
			elog(WARNING,
				 ...
				 buf->tag.blockNum,
				 buf->tag.forkNum,
				 is_dirty, ((PageHeader) BufferGetPage(BufferDescriptorGetBuffer(buf)))->pd_flags);
		}
	...
	if ((buf->recently_modified_count > polar_buffer_copy_min_modified_count &&
		 ...
		 polar_buffer_copy_threshold_lag * 1024 * 1024) ||
		lsn_lag_with_cons_lsn <
		polar_buffer_copy_threshold_lag * 1024 * 1024 / 2)
		return true;
...
copy_buffer_alloc(BufferDesc *buf, XLogRecPtr oldest_apply_lsn)
{
	...
	if (unlikely(polar_enable_debug))
	{
		elog(DEBUG1,
			 ...
			 buf->tag.relNumber,
			 buf->tag.blockNum,
			 LSN_FORMAT_ARGS(BufferGetLSN(buf)),
			 LSN_FORMAT_ARGS(polar_buffer_get_oldest_lsn(buf)),
...
polar_free_copy_buffer(BufferDesc *buf)
{
	...
	if (unlikely(polar_enable_debug))
	{
		elog(DEBUG1,
			 ...
			 buf->tag.relNumber,
			 buf->tag.blockNum,
			 LSN_FORMAT_ARGS(cbuf->oldest_lsn),
			 LSN_FORMAT_ARGS(polar_copy_buffer_get_lsn(cbuf)),
			 cbuf->is_flushed);
	}
...
polar_flush_copy_buffer(BufferDesc *buf, SMgrRelation reln)
{
	 ...
	 * copy buffer.
	 */
	PageSetChecksumInplace((Page) buf_block, buf->tag.forkNum, buf->tag.blockNum);
