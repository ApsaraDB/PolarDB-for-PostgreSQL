...
/**
 * Extend the size of the given file with file locking. This prevents
 * multiple processes trying to extend the same file concurrently.
 * Mainly used for parallel recovery.
 */
void FileExtendLocked(File file, int newSize, uint32 wait_event_info, bool logLocks)
{
	int			returnCode;
	Vfd		   *vfdP;
	int			saveErrno = 0;
	...
	if (newSize <= 0){
		elog(ERROR, "Can not expand file to size to %d", newSize);
	}
...
#define EXPANDED  4
	...
	DO_DB(elog(LOG, "FileWrite: %d (%s) " INT64_FORMAT " %d",
			   file, VfdCache[file].fileName,
			   (int64) VfdCache[file].seekPos, newSize));

	returnCode = FileAccess(file);
	if (returnCode < 0){
		elog(ERROR, "Failed to open file for extension: %s, error: %s", VfdCache[file].fileName, strerror(errno));
	}
	vfdP = &VfdCache[file];

	// TODO!! report error if temp file?
	...
	if (logLocks)
		elog(LOG, "Lock %s", vfdP->fileName);
	returnCode = flock(vfdP->fd, LOCK_EX);
	if (returnCode < 0){
		elog(ERROR, "Failed to lock file %s, error: %s", vfdP->fileName, strerror(errno));
	}
	status = LOCKED;

	vfdP->seekPos = lseek(vfdP->fd, 0L, SEEK_END);
	if (vfdP->seekPos < 0){
		saveErrno = errno;
		goto unlock;
	}
	...
	if (vfdP->seekPos < newSize){
		status = EXPANDING;
		returnCode = ftruncate(vfdP->fd, newSize);
		if (returnCode < 0){
			saveErrno = errno;
			goto unlock;
		}
...
unlock:
	returnCode = flock(vfdP->fd, LOCK_UN);
	if (returnCode < 0){
		elog(ERROR, "Failed to unlock file %s, error: %s", vfdP->fileName, strerror(errno));
	}
	if (logLocks)
		elog(LOG, "UnLock %s", vfdP->fileName);
	pgstat_report_wait_end();
	...
	if (saveErrno != 0){
		...
		{
		case LOCKED:
			elog(ERROR, "Failed to seek file %s, error %s", vfdP->fileName, strerror(saveErrno));
			break;
		
		case EXPANDING:
			elog(ERROR, "Failed to expand file %s, error %s", vfdP->fileName, strerror(saveErrno));
			break;
		}
	}
