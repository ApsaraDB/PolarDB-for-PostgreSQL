...
CXformFactory::Instantiate()
{
	...
	Add(GPOS_NEW(m_mp) CXformGet2TableShareScan(m_mp));
	...
	/* POLAR px */
	Add(GPOS_NEW(m_mp) CXformIndexGet2ShareIndexScan(m_mp));
	Add(GPOS_NEW(m_mp) CXformDynamicIndexGet2DynamicShareIndexScan(m_mp));
