...
CDrvdPropScalar::CDrvdPropScalar(CMemoryPool *mp)
	: m_mp(mp),
	  ...
	  m_fHasScalarRowNum(false),/* POLAR px */
	  m_ulDistinctAggs(0),
...
// scalar expression contains rownum?
BOOL
CDrvdPropScalar::HasScalarRowNum() const
{
	GPOS_RTL_ASSERT(IsComplete());
	return m_fHasScalarRowNum;
}
...
/* POLAR px */
BOOL
CDrvdPropScalar::DeriveHasScalarRowNum(CExpressionHandle &exprhdl)
{
	if (!m_is_prop_derived->ExchangeSet(EdptFHasScalarRowNum))
	{
		CScalar *popScalar = CScalar::PopConvert(exprhdl.Pop());
		m_fHasScalarRowNum = popScalar->FHasScalarRowNum(exprhdl);
	}
	return m_fHasScalarRowNum;
}
...
CDrvdPropScalar::OsPrint(IOstream &os) const
{
	...
	os << "Defined Columns: [" << *GetDefinedColumns() << "], "
	   ...
	   << "Has ScalarRowNum: [" << HasScalarRowNum() << "], ";/* POLAR px */
