											   ...
											   BOOL infer_nulls_as)
{
	...
	switch (pexpr->Pop()->Eopid())
	{
		...
		case COperator::EopScalarFunc:
			pci = PciIntervalFromScalarFunc(mp, pexpr, colref);
			break;
		default:
			pci = nullptr;
	}
										  ...
										  CColRef *colref) const
{
	...
	{
		m_pexprHash->AddRef();
		return GPOS_NEW(mp) CConstraintInterval(mp, colref, m_pexprHash);
	}
...
CConstraintInterval::PciIntersect(CMemoryPool *mp, CConstraintInterval *pci)
{
	...
	/* POLAR px: used for hash partition func */
	if (FUsedForExprHash() != pci->FUsedForExprHash())
		return PciIntersectWithHash(mp, pci);
...
/* POLAR px: used for hash partition func */
CConstraintInterval::CConstraintInterval(CMemoryPool *mp,
										 const CColRef *colref,
										 CExpression *pexprHash)
	: CConstraint(mp, GPOS_NEW(mp) CColRefSet(mp)),
	  m_pcr(colref),
	  m_pdrgprng(PciUnbounded(mp, colref)),//default include all
	  m_fIncludesNull(false),
	  m_pexprHash(pexprHash)
{
	GPOS_ASSERT(nullptr != colref);
	GPOS_ASSERT(nullptr != pexprHash);
	m_pcrsUsed->Include(colref);
...
/* POLAR px: used for hash partition func */
bool
CConstraintInterval::FContainPointsOnly() const
{
	if (1 > m_pdrgprng->Size())
	{
		return false;
	...
	const ULONG length = m_pdrgprng->Size();
	for (ULONG ul = 0; ul < length && isAllPoint; ul++)
	{
		isAllPoint &= (*m_pdrgprng)[ul]->FPoint();
	}
	return isAllPoint;
}
...
/* POLAR px: used for hash partition func */
CConstraintInterval *
CConstraintInterval::PciIntervalFromScalarFunc(CMemoryPool *mp,
											  CExpression *pexpr,
											  CColRef *colref)
{
	GPOS_ASSERT(nullptr != pexpr);
	...
	if (CUtils::FScalarFuncForHashPartition(pexpr))
	{
#ifdef GPOS_DEBUG
		CScalarIdent *popScId = CScalarIdent::PopConvert((*pexpr)[3]->Pop());
		GPOS_ASSERT(colref == (CColRef *) popScId->Pcr());
#endif	// GPOS_DEBUG
		return GPOS_NEW(mp) CConstraintInterval(mp, colref, pexpr);
	}
	return nullptr;
}
...
/* POLAR px: used for hash partition func */
CConstraintInterval *
CConstraintInterval::PciIntersectWithHash(CMemoryPool *mp, CConstraintInterval *pci)
{
	GPOS_ASSERT(nullptr != pci);
	GPOS_ASSERT(m_pcr == pci->Pcr());
	GPOS_ASSERT(FUsedForExprHash() != pci->FUsedForExprHash());

	CConstraintInterval *pcstr_hash = FUsedForExprHash() ? this : pci;
	CConstraintInterval *pcstr_range = FUsedForExprHash() ? pci : this;

	if (pcstr_range->FContainPointsOnly())
	{
		CExpression *pexprHash = pcstr_hash->PExprHash();;
		CRangeArray *prangeArray = pcstr_range->Pdrgprng();
		CRangeArray *pdrgprngNew = GPOS_NEW(mp) CRangeArray(mp);

		const IComparator *pcomp = COptCtxt::PoctxtFromTLS()->Pcomp();
		for (ULONG ul = 0; ul < prangeArray->Size(); ul++)
		{
			CRange *prange = (*prangeArray)[ul];
			IDatum *datum = (*prangeArray)[ul]->PdatumRight();

			if (!datum->IsNull()
				&& pcomp->FEvalHashExprResult(pexprHash, datum))
			{
				prange->AddRef();
				pdrgprngNew->Append(prange);
			}
		}

		return GPOS_NEW(mp) CConstraintInterval(
			mp, m_pcr, pdrgprngNew, pcstr_range->FIncludesNull());
	}

	if (pcstr_range->Pdrgprng()->Size() == 0 && pcstr_range->FIncludesNull())
	{
		return GPOS_NEW(mp) CConstraintInterval(
			mp, m_pcr, GPOS_NEW(mp) CRangeArray(mp), false);
	}

	return GPOS_NEW(mp) CConstraintInterval(
		mp, m_pcr, PciUnbounded(mp, m_pcr), false);
}
