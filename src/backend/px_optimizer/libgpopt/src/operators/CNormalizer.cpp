...
CNormalizer::FPushable(CExpression *pexprLogical, CExpression *pexprPred)
{
	...
	// POLAR px: project with rownum cannot push
	if ((COperator::EopLogicalSelect == pexprLogical->Pop()->Eopid() ||
		 COperator::EopLogicalProject == pexprLogical->Pop()->Eopid()) &&
		pexprLogical->Arity() > 0)
	{
		CExpression *pexprProjList = (*pexprLogical)[pexprLogical->Arity() - 1];
		if (pexprProjList->DeriveHasScalarRowNum())
			return false;
	}

	// POLAR px: only limit op we can push rownum
	if (pexprPred->DeriveHasScalarRowNum() &&
		pexprLogical->Pop()->Eopid() != COperator::EopLogicalLimit)
		return false;
...
//	PRINT_EXPR("before PexprNormalize", pexpr);
...
//	PRINT_EXPR("after PexprNormalize", pexprResult);
...
)
{
	...
	for (ULONG ul = 0; ul < ulProjElements; ul++)
	{
		...
		// NB we don't pull up projections that call set-returning functions and
		// has rownum
		pexprPrEl->AddRef();
		...
		if (!pcrsUsed->FMember(pcrDefined) &&
			...
			!pexprPrEl->DeriveHasScalarRowNum())
		{
