	...
	BOOL fUseConstraints, const IMDRelation *pmdrel)
{
	...
	for (ULONG ul = 0; ul < ulLevels; ul++)
	{
		...
		BOOL isKnownToBeNotRangePartitioned = false;
		if (pmdrel != nullptr)
		{
			...
			isKnownToBeNotRangePartitioned =
				(IMDRelation::ErelpartitionRange != szPartType);
		}
		...
		CExpression *pexprCmp = PexprPartPruningPredicate(
			...
			isKnownToBeNotRangePartitioned /* allowNotEqualPreds */);
