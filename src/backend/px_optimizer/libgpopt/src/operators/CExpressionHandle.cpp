...
/* POLAR px */
BOOL
CExpressionHandle::FChildrenHaveScalarRowNum() const
{
	const ULONG arity = Arity();
	for (ULONG ul = 0; ul < arity; ul++)
	{
		CExpression *pexpr = PexprScalarExactChild(ul);
		if (pexpr != NULL && pexpr->DeriveHasScalarRowNum())
		{
			return true;
		}
	}
...
/* POLAR px */
BOOL
CExpressionHandle::DeriveHasScalarRowNum(ULONG child_index) const
{
	if (nullptr != Pexpr())
	{
		return (*Pexpr())[child_index]->DeriveHasScalarRowNum();
	}

	return GetDrvdScalarProps(child_index)->HasScalarRowNum();
}
