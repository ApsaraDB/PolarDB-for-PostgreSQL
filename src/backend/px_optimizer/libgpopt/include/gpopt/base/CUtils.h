...
public:
	...
	/* POLAR px */
	static BOOL FPartitionWiseJoinAble(CMemoryPool *mp,
									   const CExpression *pexpr);

	static void ExtractEqualJoinColRefs(CExpression *pexprCmp,
	...
	// POLAR px: check for existence of rownum
	static BOOL FHasScalarRowNum(CExpression *pexpr);
	...
	static BOOL FRowNum(COperator *pop);
	...
	// check if expression is scalar func for partition
	static BOOL FScalarFuncForHashPartition(CExpression *pexpr);

	// check if expression is scalar func for partition calc
	static BOOL FScalarFuncForHashPartitionCalc(CExpression *pexpr);
