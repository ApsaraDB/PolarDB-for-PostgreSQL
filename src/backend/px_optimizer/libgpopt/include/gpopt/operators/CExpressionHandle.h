...
public:
	...
	NeedsSingletonExecution() const
	{
		return DeriveFunctionProperties()->NeedsSingletonExecution() ||
				FChildrenHaveScalarRowNum()/* POLAR px */;
	}
	...
	BOOL IsGlobalFunc()
	{
		return DeriveFunctionProperties()->IsGlobalFunc();
	}
	...
	// POLAR px: check whether an expression's children have rownum
	BOOL FChildrenHaveScalarRowNum() const;
	...
	/* POLAR px */
	BOOL DeriveHasScalarRowNum(ULONG child_index) const;
