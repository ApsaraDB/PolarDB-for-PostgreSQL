...
public:
	...
	enum EXformId
	{
		...
		/* POLAR px */
		ExfIndexGet2ShareIndexScan,
		ExfDynamicIndexGet2DynamicShareIndexScan,
	...
	// returuns a set containing all xforms that generate a plan with a nestloop join
	static
	CBitSet *PbsNestloopJoinXforms(CMemoryPool *mp);

	// returuns a set containing all xforms that generate a plan with ant semi join not in
	static
	CBitSet *PbsAntSemiJoinNotInXforms(CMemoryPool *mp);
