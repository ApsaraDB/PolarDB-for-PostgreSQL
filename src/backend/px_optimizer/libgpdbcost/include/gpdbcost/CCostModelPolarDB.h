...
//		GPDB cost model
//---------------------------------------------------------------------------
#ifndef GPDBCOST_CCostModelPolarDB_H
#define GPDBCOST_CCostModelPolarDB_H
...
//
//---------------------------------------------------------------------------
class CCostModelPolarDB : public ICostModel
{
private:
	// memory pool
	CMemoryPool *m_mp;
	...
	CCostModelParamsPolarDB *m_cost_model_params;
	...
	// cost of share scan
	static
	CCost CostShareScan(CMemoryPool *mp, CExpressionHandle &exprhdl, const CCostModelPolarDB *pcmgpdb, const SCostingInfo *pci);
	...
	static CCost CostBitmapSmallNDV(const CCostModelPolarDB *pcmgpdb,
									const SCostingInfo *pci, CDouble dNDV);

	// cost of bitmap scan when the NDV is large
	static CCost CostBitmapLargeNDV(const CCostModelPolarDB *pcmgpdb,
									const SCostingInfo *pci, CDouble dNDV);

public:
	// ctor
	CCostModelPolarDB(CMemoryPool *mp, ULONG ulSegments,
				   CCostModelParamsPolarDB *pcp = nullptr);

	// dtor
	~CCostModelPolarDB() override;
	...
	Ecmt() const override
	{
		return ICostModel::EcmtPolarDBCalibrated;
	}

};	// class CCostModelPolarDB

}  // namespace gpdbcost

#endif	// !GPDBCOST_CCostModelPolarDB_H
