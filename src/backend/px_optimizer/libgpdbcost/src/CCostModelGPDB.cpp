							  ...
							  const SCostingInfo *pci)
{
	...
	const CDouble dShareIndexScanFactor = 
		pcmgpdb->GetCostModelParams()
			->PcpLookup(CCostModelParamsGPDB::EcpPolarShareIndexScanFactor)
			->Get();
	...
	)
{
	...
	// POLAR px: share scan factor
	const CDouble dTableShareScanFactor = pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDB::EcpPolarTableShareScanFactor)->Get();
