...
#include "px_optimizer_util/translate/CQueryMutators.h"
	...
	m_cteid_at_current_query_level_map(NULL)
{
	...
	// GPDB_95_MERGE_FIXME: Support grouping sets in PXOPT?
	...
	)
{
	...
	{
		unsupported_list = px::LAppendInt(unsupported_list, unsupported_features[ul].node_tag);
	}

	INT unsupported_node = px::FindNodes((Node *) query, unsupported_list);
	px::GPDBFree(unsupported_list);
	...
	INT non_default_collation = px::CheckCollation((Node *) query);
	...
	)
{
	...
	if ((NULL != query->jointree && 0 < px::ListLength(query->jointree->fromlist))
	...
	const
{
	...
	List *function_list =	px::ExtractNodesExpression(node, T_FuncExpr, true /*descendIntoSubqueries*/);
	ListCell *lc = NULL;
	...
	)
{
	...
	// GPDB_92_MERGE_FIXME: CTAS is a UTILITY statement after upstream
		...
		if (!px_optimizer_enable_ctas && query->parentStmtType == PARENTSTMTTYPE_CTAS)
		{
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("CTAS. Set px_optimizer_enable_ctas to on to enable CTAS with PXOPT"));
		}
		if (query->parentStmtType == PARENTSTMTTYPE_COPY)
		{
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("COPY. Copy select statement to file on segment is not supported with PXOPT"));
		}
		if (query->parentStmtType == PARENTSTMTTYPE_REFRESH_MATVIEW)
		{
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("Refresh matview is not supported with PXOPT"));
		}
	...
	)
{
	...
	ForEach (lc, query->rtable)
	{
		...
		if (rte->security_barrier || rte->securityQuals)
		{
			...
			// otherwise PXOPT most likely pushes potentially leaky filters down
...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	GPOS_ASSERT_IMP(NULL != m_query->setOperations, 0 == px::ListLength(m_query->windowClause));
	...
	else if (0 != px::ListLength(m_query->windowClause)) // translate window clauses
	{
...
CTranslatorQueryToDXL::TranslateUpdateQueryToDXL()
{
	...
	const RangeTblEntry *rte = (RangeTblEntry *) px::ListNth(
	...
	if (!px_optimizer_enable_dml_constraints &&
...
CTranslatorQueryToDXL::GetCtidAndSegmentId(ULONG *ctid, ULONG *segment_id)
{
	...
	*ctid = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation,
	...
	// segmentid column id
	mdid = CTranslatorUtils::GetSystemColType(m_mp, PxWorkerIdAttributeNumber);
	*segment_id = CTranslatorUtils::GetColId(
		m_query_level, m_query->resultRelation, PxWorkerIdAttributeNumber,
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	if (!px_optimizer_enable_dml_constraints && rel_has_constraints)
	{
		GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
	...
	const ULONG target_list_length = px::ListLength(m_query->targetList);
	...
	for (ULONG ul = 0; ul < num_table_columns; ul++)
	{
		...
		if (target_list_pos < target_list_length)
		{
			...
			TargetEntry *target_entry = (TargetEntry *) px::ListNth(
...
CTranslatorQueryToDXL::TranslateQueryToDXL()
{
	...
	switch (m_query->commandType)
	{
		case CMD_SELECT:
			...
			/* POLAR px: we don't support ctas */
#if 0
			else
			{
	...
	const
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		/* POLAR px: is_lead_func & is_lag are always false */
	...
	)
{
	...
	ForEach (lc, window_clause)
	{
		...
		if (0 < px::ListLength(wc->orderClause))
		{
...
{
	if (0 == px::ListLength(window_clause))
	{
	...
	ForEach (lc, target_list)
	{
		...
		if (!target_entry->resjunk)
		{
					...
					GPOS_NEW(m_mp) CDXLNode(
						...
						GPOS_NEW(m_mp) CDXLScalarIdent(
							m_mp, GPOS_NEW(m_mp) CDXLColRef(
									  ...
									  colid,
									  GPOS_NEW(m_mp) CMDIdGPDB(px::ExprType(
			...
			else
			{
				...
				omitted_target_entries = px::LAppend(omitted_target_entries, target_entry);
				resno_list = px::LAppendInt(resno_list, resno);
	...
	{
		px::GPDBFree(omitted_target_entries);
	}
...
{
	if (0 == px::ListLength(sort_clause) && NULL == limit_count && NULL == limit_offset_node)
	{
		return child_dxlnode;
	}
	...
	)
{
	...
	ForEach (lc, target_list)
	{
		...
		{
			if (IsA(target_entry->expr, Aggref) && ((Aggref *) target_entry->expr)->aggdistinct && !IsDuplicatePxaArg(dqa_list, (Aggref *) target_entry->expr))
			{
				dqa_list = px::LAppend(dqa_list, px::CopyObject(target_entry->expr));
				num_dqa++;
			}
	...
	if (1 < num_dqa && !px_optimizer_enable_multiple_distinct_aggs)
	{
...
/*-------------------------------------------------------------------------
*	@function:
*		CTranslatorQueryToDXL::IsDuplicatePxaArg
*
*	@doc:
*		Check if the argument of a PXA has already being used by another PXA
-------------------------------------------------------------------------*/
BOOL
CTranslatorQueryToDXL::IsDuplicatePxaArg
	(
	...
	)
{
	...
	if (NIL == dqa_list || 0 == px::ListLength(dqa_list))
	{
		return false;
	}
	...
	ForEach (lc, dqa_list)
	{
		...
		if (px::Equals(aggref->args, ((Aggref *) node)->args))
		{
			return true;
		}
	}
...
{
	const ULONG num_of_cols = px::ListLength(target_list) + 1;
...
CTranslatorQueryToDXL::DXLDummyConstTableGet() const
{
	...
	Const *const_expr = (Const*) px::MakeBoolConst(true /*value*/, false /*isnull*/);
	CDXLDatum *datum_dxl = m_scalar_translator->TranslateConstToDXL(const_expr);
	px::GPDBFree(const_expr);
	...
	const
{
	...
	for (ULONG ul = 0; ul < num_of_cols; ul++)
	{
		...
		TargetEntry *target_entry = (TargetEntry*) px::ListNth(output_target_list, target_list_pos);
	...
	const
{
	...
	GPOS_ASSERT(input_col_mdids->Size() <= px::ListLength(target_list)); // there may be resjunked columns
	...
	{
		TargetEntry *target_entry = (TargetEntry *) lfirst(lc);
		OID expr_type_oid = px::ExprType((Node*) target_entry->expr);
		if (!target_entry->resjunk)
		{
	...
	)
{
	...
	if (IsA(child_node, RangeTblRef))
	{
		...
		const RangeTblEntry *rte = (RangeTblEntry *) px::ListNth(m_query->rtable, rt_index - 1);
	...
	CHAR *temp_str = (CHAR*) px::NodeToString(const_cast<Node*>(child_node));
	...
	)
{
	...
	if (0 == px::ListLength(from_expr->fromlist))
	{
		dxlnode = DXLDummyConstTableGet();
	}
	else
	{
		if (1 == px::ListLength(from_expr->fromlist))
		{
			Node *node = (Node*) px::ListNth(from_expr->fromlist, 0);
			GPOS_ASSERT(NULL != node);
	...
	)
{
	...
	CHAR *sz = (CHAR*) px::NodeToString(const_cast<Node*>(node));
	...
	)
{
	...
	// 2. Legacy dqhash opclasses for the datatype
	// 3. Any other opclasses
	//
	// PXOPT doesn't know about hash opclasses attached to distribution
	// keys. So if a query involves two tables, with e.g. integer
	// datatype as distribution key, but with different opclasses,
	// PXOPT doesn'thinks they're nevertheless compatible, and will
	...
	// opclasses, or the legacy ones, PXOPT will produce a valid plan.
	...
	// for which the PXOPT-generated plan would in fact be OK, but
	...
	{
		px::RelationWrapper rel = px::GetRelation(rte->relid);
		...
		for (int i = 0; i < policy_nattrs; i++)
		{
			...
			{
				Oid default_opclass = px::GetDefaultDistributionOpclassForType(typeoid);
	...
	)
{
	...
	const ULONG num_of_tuples = px::ListLength(tuples_list);
	...
	ForEach (lc_tuple, tuples_list)
	{
		...
		GPOS_ASSERT(px::ListLength(tuple_list) == px::ListLength(col_names));
		...
		ForEach (lc_column, tuple_list)
		{
			...
			CHAR *col_name_char_array = (CHAR *) strVal(px::ListNth(col_names, col_pos_idx));
			...
			else
			{
				...
				if (0 == tuple_pos)
				{
					...
					CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp) CDXLColDescr(
						mdname, colid, col_pos_idx + 1 /* attno */,
						GPOS_NEW(m_mp) CMDIdGPDB(px::ExprType((Node *) expr)),
						px::ExprTypeMod((Node *) expr), false /* is_dropped */
					);
					dxl_col_descr_array->Append(dxl_col_descr);
				}
			}
	...
	)
{
	 ...
	 * PXOPT isn't prepared for that yet. See upstream commit 784e762e88.
	 */
	...
	m_context->set_has_distributed_tables(tvf_dxlop->IsGlobalFunc());
	...
	)
{
	...
	RangeTblEntry *rte = (RangeTblEntry *) px::ListNth(m_query->rtable, rtindex - 1);
	GPOS_ASSERT(NULL != rte);

	Alias *alias = rte->eref;
	GPOS_ASSERT(NULL != alias);
	GPOS_ASSERT(NULL != alias->colnames && 0 < px::ListLength(alias->colnames));
	GPOS_ASSERT(px::ListLength(rte->joinaliasvars) == px::ListLength(alias->colnames));
...
{
	BOOL is_groupby = (0 != px::ListLength(m_query->groupClause) || m_query->hasAggs);
	...
	ForEach(lc, target_list)
	{
		...
		{
			vars_list = px::ListConcat(vars_list, px::ExtractNodesExpression((Node *) target_entry->expr, T_Var, false /*descendIntoSubqueries*/));
		}
		else if (!IsA(target_entry->expr, Aggref))
		{
			omitted_te_list = px::LAppend(omitted_te_list, target_entry);
		}
	}
	...
	const
{
	...
	ForEach (lc, target_list)
	{
		...
		IMDId *mdid_type = GPOS_NEW(m_mp) CMDIdGPDB(px::ExprType( (Node*) target_entry->expr));
