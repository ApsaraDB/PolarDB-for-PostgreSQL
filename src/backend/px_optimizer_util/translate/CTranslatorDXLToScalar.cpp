	...
	)
{
	...
	case_expr->casecollid = px::TypeCollation(case_expr->casetype);
	...
	while (!is_leaf_else_stmt)
	{
		...
		case_expr->args = px::LAppend(case_expr->args,case_when);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	case_test_expr->collation = px::TypeCollation(case_test_expr->typeId);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	op_expr->inputcollid = px::ExprCollation((Node *) op_expr->args);
	op_expr->opcollid = px::TypeCollation(op_expr->opresulttype);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	array_op_expr->inputcollid = px::ExprCollation((Node *) array_op_expr->args);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	dist_expr->opcollid = px::TypeCollation(dist_expr->opresulttype);
	...
	)
{
	...
	switch(dxlop->GetDXLAggStage())
	{
		...
		case EdxlaggstageIntermediate:
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiPlStmt2DXLConversion,
				   GPOS_WSZ_LIT("POLARDB_MERGE_FIXME: Intermediate aggregate stage not implemented"));
			break;
	...
	{
		TargetEntry *new_target_entry = px::MakeTargetEntry((Expr *) lfirst(lc), attno + 1, NULL, false);
		Oid aggargtype = px::ExprType((Node *) lfirst(lc));
		/*
		 * Translate the aggdistinct bool set to true (in PXOPT),
		...
		if(dxlop->IsDistinct())
		{
			...
			gc->eqop = px::GetEqualityOp(px::ExprType((Node*) new_target_entry->expr));
			gc->sortop = px::GetOrderingOpForEqualityOp(gc->eqop, NULL);
			/*
			 * Since PXOPT doesn't yet support ordered aggregates, we are
			...
			aggref->aggdistinct = px::LAppend(aggref->aggdistinct, gc);
			sortgrpindex++;
		}
		aggref->args = px::LAppend(aggref->args, new_target_entry);
		aggref->aggargtypes = px::LAppendOid(aggref->aggargtypes, aggargtype);
	}
	...
	aggtranstype = px::GetAggIntermediateResultType(aggref->aggfnoid);

	/* extract argument types (ignoring any ORDER BY expressions) */
	numArguments = px::GetAggregateArgTypes(aggref, inputTypes);

	/* resolve actual type of transition state, if polymorphic */
	aggtranstype = px::ResolveAggregateTransType(aggref->aggfnoid,
						       aggtranstype,
						       inputTypes,
						       numArguments);
	...
	// GPDB_91_MERGE_FIXME: collation
	aggref->inputcollid = px::ExprCollation((Node *) exprs);
	aggref->aggcollid = px::TypeCollation(aggref->aggtype);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	window_func->wincollid = px::TypeCollation(window_func->wintype);
	window_func->inputcollid = px::ExprCollation((Node *) window_func->args);
	...
	)
{
	...
	GPOS_ASSERT(NULL != plan_child->targetlist && 1 <= px::ListLength(plan_child->targetlist));
	...
	)
{
	...
	args = px::LAppend(args, outer_arg_expr);
...
CDXLScalarIdent *inner_ident = nullptr;
	...
	args = px::LAppend(args, inner_expr);

	// also, add this param to subplan param ids before translating other params
	*param_ids = px::LAppendInt(*param_ids, param->paramid);
	...
	)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		subplan->parParam = px::LAppendInt(subplan->parParam, param->paramid);
		...
		subplan->args = px::LAppend(subplan->args, arg);
	}
	...
	)
{
	...
	subplan->plan_id = px::ListLength(dxl_to_plstmt_ctxt->GetSubplanEntriesList());
	subplan->plan_name = GetSubplanAlias(subplan->plan_id);
	subplan->is_initplan = false;
	subplan->firstColType = px::ExprType( (Node*) ((TargetEntry*) px::ListNth(plan->targetlist, 0))->expr);
	// GPDB_91_MERGE_FIXME: collation
	subplan->firstColCollation = px::TypeCollation(subplan->firstColType);
	...
	)
{
	...
	ULONG max_length = GPOS_WSZ_LENGTH(buf) * GPOS_SIZEOF(WCHAR) + 1;
	CHAR *result_plan_name = (CHAR *) px::GPDBAlloc(max_length);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	scalar_null_if_expr->opcollid = px::TypeCollation(scalar_null_if_expr->opresulttype);
	scalar_null_if_expr->inputcollid = px::ExprCollation((Node *) scalar_null_if_expr->args);

	return (Expr *) scalar_null_if_expr;
}

Expr *
CTranslatorDXLToScalar::TranslateDXLScalarCastWithChildExpr(const CDXLScalarCast *scalar_cast, Expr *child_expr)
{
	if (IMDId::IsValid(scalar_cast->FuncMdId()))
	{
		...
		func_expr->args = NIL;
		func_expr->args = px::LAppend(func_expr->args, child_expr);
	...
	// GPDB_91_MERGE_FIXME: collation
	relabel_type->resultcollid = px::TypeCollation(relabel_type->resulttype);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	coerce->resultcollid = px::TypeCollation(coerce->resulttype);
	...
	)
{
	...
	CaseTestExpr *case_test_expr = MakeNode(CaseTestExpr);
	Oid input_array_type = px::ExprType((Node *) child_expr);
	int32 input_array_elem_typmod = px::ExprTypeMod((Node *) child_expr);
	case_test_expr->typeId = px::GetElementType(input_array_type);
	...
	if (elemfuncid != 0)
	{
		...
		func_expr->funcresulttype = px::GetFuncRetType(elemfuncid);
	...
	else
	{
		...
		rt->resulttype = px::GetElementType(coerce->resulttype);
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	coalesce->coalescecollid = px::TypeCollation(coalesce->coalescetype);
	...
	)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		Expr *child_expr = TranslateDXLToScalar(child_dxl, colid_var);
		new_list = px::LAppend(new_list, child_expr);
	}
	...
	)
{
	...
	{
		constant->constvalue = px::DatumFromInt32(oid_datum_dxl->OidValue());
	}
	...
	)
{
	...
	{
		constant->constvalue = px::DatumFromInt16(datum_int2_dxl->Value());
	}
	...
	)
{
	...
	{
		constant->constvalue = px::DatumFromInt32(datum_int4_dxl->Value());
	}
	...
	)
{
	...
	{
		constant->constvalue = px::DatumFromInt64(datum_int8_dxl->Value());
	}
	...
	)
{
	...
	{
		constant->constvalue = px::DatumFromBool(datum_bool_dxl->GetValue());
	}
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	constant->constcollid = px::TypeCollation(constant->consttype);
	...
	{
		Datum val = px::DatumFromPointer(datum_generic_dxl->GetByteArray());
		ULONG length = (ULONG) px::DatumSize(val, false, constant->constlen);

		CHAR *str = (CHAR *) px::GPDBAlloc(length + 1);
		memcpy(str, datum_generic_dxl->GetByteArray(), length);
		str[length] = '\0';
		constant->constvalue = px::DatumFromPointer(str);
	}
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	expr->array_collid = px::TypeCollation(expr->array_typeid);
	...
	/*
	 * PXOPT doesn't know how to construct array constants, so it will
	 ...
	 * were passed to PXOPT. But there's little harm in trying).
	 */
	return (Expr *) px::EvalConstExpressions((Node *) expr);
}
	...
	)
{
	...
	// GPDB_91_MERGE_FIXME: collation
	array_ref->refcollid = px::TypeCollation(array_ref->refelemtype);
