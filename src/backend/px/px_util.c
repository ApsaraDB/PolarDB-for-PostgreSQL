...
static PxNodeConfigEntry   *px_node_all_configs = NULL;
static int px_node_all_configs_size = 0;
static int px_node_configs_size = 0;

static PxNodes *getPxNodes(void);
/* POLAR px */
static int		nextPXIdentifer(PxNodes *px_nodes, bool isRW);
/* POLAR end */
static void		GeneratePxNodeAllConfigs(void);
...
/* NB: should only be called from GeneratePxNodeConfigs */
static void
GeneratePxNodeAllConfigs(void)
{
	char	   *confstring;
	List	   *nodelist;
	ListCell   *lnode;
	int			idx;
	PxNodeConfigEntry *configs;
	PxNodeConfigEntry *config, *qc_config;
	int			array_size;

	if (polar_cluster_map == NULL || polar_cluster_map[0] == '\0')
	{
		elog(DEBUG1, "px cluster map is empty, name|ip|port, ...");
		return ;
	}
	 ...
	 */
	confstring = pstrdup(polar_cluster_map);
	(void)SplitIdentifierString(confstring, ',', &nodelist);
	array_size = list_length(nodelist);

	configs = palloc0(sizeof(PxNodeConfigEntry) * (array_size + 1));

	foreach_with_count(lnode, nodelist, idx)
	{
		char *name, *ip, *port;
		List *itemlist;
		int cport;
		...
		config->dop = 1;
		config->port = cport;
		config->hostip = pstrdup(ip);
		config->name = pstrdup(name);

		config->status = 'd';
		elog(DEBUG1, "px cluster map add one server %s|%c|%s|%d|%d", config->name,
				   config->role, config->hostip, config->port, config->dop);
	}
	...
	*/
	...
	px_node_all_configs = configs;
	px_node_all_configs_size = idx;
}

/* NB: should only be called from getPxNodes */
static void
GeneratePxNodeConfigs(void)
{
	...
	GeneratePxNodeAllConfigs();

	if (polar_px_nodes == NULL || polar_px_nodes[0] == '\0')
	{
		px_node_configs = px_node_all_configs;
		px_node_configs_size = px_node_all_configs_size;
		return;
	}
	...
	{
		int		i;
		char   *nodename= pstrdup((char *) lfirst(lnode));

		for (i = 0; i < px_node_all_configs_size - 1; i++)
		{
			if (strcmp(px_node_all_configs[i].name, nodename) == 0)
			{
				PxNodeConfigEntry *config = &configs[idx];
				config->dbid = idx;
				config->role = 'x';
				config->node_idx = idx;
				config->dop = 1;
				config->port = px_node_all_configs[i].port;
				config->hostip = pstrdup(px_node_all_configs[i].hostip);
				config->name = pstrdup(px_node_all_configs[i].name);
				break;
			}
		}
		if (i == px_node_all_configs_size - 1)
		{
			int j;
			elog(ERROR, "GeneratePxNodeConfig: invalid node in polar_px_nodes: %s", nodename);

			for (j = 0; j < idx; j++)
			{
				pfree(configs[j].hostip);
				pfree(configs[j].name);
			}
...
GeneratePxWorkerNames(void)
{
	...
	for (i = 0; i < px_node_all_configs_size - 1; i++)
	{
		appendStringInfo(names, "%s", px_node_all_configs[i].name);
		if (i < px_node_all_configs_size - 2)
			appendStringInfo(names, ",");
	}
	return names->data;
}
...
getPxNodes(void)
{
	...
	if (px_node_all_configs == NULL)
		elog(ERROR, "polar_cluster_map is not set");

	if (px_node_configs == NULL)
...
pxnode_destroyPxNodes(void)
{
	...
	if (px_node_configs)
	{
		if (px_node_configs != px_node_all_configs)
		{
			int i;
	...
	if (px_node_all_configs)
	{
		int i;
		for (i= 0; i < px_node_all_configs_size - 1; i++)
		{
			pfree(px_node_all_configs[i].hostip);
			pfree(px_node_all_configs[i].name);
		}

		pfree(px_node_all_configs);
		px_node_all_configs = NULL;
		px_node_all_configs_size = 0;
	}
}
