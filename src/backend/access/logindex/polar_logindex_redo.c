...
int			polar_write_logindex_active_table_delay = 500;
int			polar_wait_old_version_page_timeout = 30 * 1000;
double		polar_xlog_queue_data_keep_ratio = 0.6;
int			polar_invalid_page_forget_queue_size = 32768;
polar_node_status_t polar_parallel_replay_mode = {
	.primary_status = POLAR_DISABLE_PARALLEL_REPLAY,
	.standby_status = POLAR_PARALLEL_REPLAY_DEFAULT_MODE,
	.pitr_status = POLAR_PARALLEL_REPLAY_DEFAULT_MODE
};
...
/* Define the ratio of memory configured by polar_logindex_mem_size used for wal logindex */
#define WAL_LOGINDEX_MEM_RATIO          (1 - polar_fullpage_logindex_mem_ratio)
 ...
 */
static log_index_page_iter wal_page_iter = NULL;
static log_index_page_iter fullpage_page_iter = NULL;

static BufferDesc *polar_replaying_buffer = NULL;
static log_index_page_iter iter_context = NULL;
...
static void polar_only_replay_exists_buffer(polar_logindex_redo_ctl_t instance, XLogReaderState *state, log_index_lsn_info log_index_page);
static void polar_logindex_apply_one_page(polar_logindex_redo_ctl_t instance, BufferTag *tag, Buffer *buffer, log_index_page_iter iter);
 ...
 * so we have to clear its redo_state so that other backend
 * process can replay this buffer again. We only process interrupt
 * when replaying buffer with buffer IO lock, so current replaying
...
polar_logindex_abort_replaying_buffer(void)
{
	if (polar_replaying_buffer != NULL)
	{
		...
		/* remove dirty state */
		buf_state = LockBufHdr(polar_replaying_buffer);
		POLAR_ASSERT_PANIC(buf_state & BM_IO_IN_PROGRESS);
		POLAR_ASSERT_PANIC(!(buf_state & BM_VALID));
		buf_state &= ~(BM_DIRTY | BM_JUST_DIRTIED);
		UnlockBufHdr(polar_replaying_buffer, buf_state);

		/* clear redo_state */
		redo_state = polar_lock_redo_state(polar_replaying_buffer);
		redo_state &= (~POLAR_BUF_REDO_FLAG_MASK);
		polar_unlock_redo_state(polar_replaying_buffer, redo_state);

		/* zero buffer content */
		buf_block = BufHdrGetBlock(polar_replaying_buffer);
...
polar_extend_block_if_not_exist(BufferTag *tag)
{
	...
	rlocator.relNumber = tag->relNumber;
	...
	/*
	 * See: XLogReadBufferExtended.
	 */
	if (nblocks == 0 &&
		POLAR_RSC_ENABLED() && polar_rsc_optimize_drop_buffers)
	{
		nblocks = polar_rsc_update_entry(smgr,
										 tag->forkNum,
										 InvalidBlockNumber,
...
polar_read_vm_buffer(BufferTag *heap_tag)
{
	...
	RelFileLocator rlocator = {
		.spcOid = heap_tag->spcOid,
		.dbOid = heap_tag->dbOid,
		.relNumber = heap_tag->relNumber
	};
	Relation	rel = CreateFakeRelcacheEntry(rlocator);
							 ...
							 bool get_cleanup_lock, polar_page_lock_t * page_lock, bool vm_parse)
{
	...
	{
		POLAR_GET_VM_TAG(&vm_tag, heap_tag);
		tag = &vm_tag;
	}
	...
	if (buf_id >= 0)
	{
		 ...
		 * process complete buffer read and start to redo for buffer, it will
		 * redo to this lsn.
		 *
		 * We add logindex without acquiring buffer content lock in the
		 * following scenarios: 1. When backend process hasn't finished
		 * reading this buffer from storage, it will redo to this lsn when it
		 * starts to redo the buffer later. 2. When it is no need to acquire
		 * buffer cleanup lock during replaying the record, the mini
		 * transaction lock will guarantee the data consistency between
		 * buffers. 3. If buffer cleanup lock is needed, and the buffer is
		 * being replayed, and it's state is invalid. In this case we check
		 * buffer state during acquired redo state lock to guarantee that
		 * there won't be other backends access the buffer until current
		 * record is replayed.
		 */
			 ...
			 !(pg_atomic_read_u32(&buf_desc->state) & BM_VALID)))
		{
			...
			polar_logindex_mini_trans_set_page_added(instance->mini_trans, *page_lock);
		}
		else
		{
			polar_unlock_redo_state(buf_desc, redo_state);
			...
			/*
			 * We need to acquire buffer's cleanup lock here to avoid the
			 * buffer content being changed when other backends are accessing
			 * the buffer with buffer content lock or buffer pin. The buffer
			 * is valid, so it won't take long time to read this buffer.
			 */
			if (!vm_parse)
			{
				RelFileLocator rlocator = {
				...
				buffer = XLogReadBufferExtended(rlocator, tag->forkNum, tag->blockNum, RBM_NORMAL_NO_LOG, InvalidBuffer);
			}
			else
...
polar_logindex_save_vm_block(polar_logindex_redo_ctl_t instance, XLogReaderState *state, uint8 heap_block)
{
	...
	InitBufferTag(&vm_tag, &blk->rlocator, VISIBILITYMAP_FORKNUM, HEAPBLK_TO_MAPBLOCK(blk->blkno));

	POLAR_LOG_INDEX_ADD_LSN(instance->wal_logindex_snapshot, state, &vm_tag);
}

static void
polar_logindex_save_block_common(log_index_snapshot snapshot, XLogReaderState *state, uint8 block_id)
{
	BufferTag	tag;
	DecodedBkpBlock *blk;
	...
	InitBufferTag(&tag, &blk->rlocator, blk->forknum, blk->blkno);
...
polar_logindex_redo_vm_parse(polar_logindex_redo_ctl_t instance, XLogReaderState *record, uint8 heap_block)
{
	...
	rlocator = POLAR_REL_FILE_LOCATOR(&heap_tag);
	InitBufferTag(&vm_tag, &rlocator, VISIBILITYMAP_FORKNUM, HEAPBLK_TO_MAPBLOCK(heap_tag.blockNum));
						  ...
						  XLogRecPtr redo_start_lsn, XLogRecPtr *mini_trans_lsn)
{
	...
	*mini_trans_lsn = InvalidXLogRecPtr;
	...
	/*
	 * Save LSN into logindex table before parsing the record instead of after
	 * acquiring the mini transaction lock, which shortens the critical
	 * section of mini transaction lock and thus reduces lock contention.
	 */
	if (polar_idx_redo[rmid].rm_polar_idx_save != NULL)
		polar_idx_redo[rmid].rm_polar_idx_save(instance, state);

	/*
	 * POLAR: Only wal records whose start lsn is not lower than redo lsn need
	 * to be parsed.
	 */
	if (IsUnderPostmaster &&
		state->ReadRecPtr >= redo_start_lsn &&
		POLAR_ENABLE_LOGINDEX_PARSE())
	{
		...
		if (polar_idx_redo[rmid].rm_polar_idx_parse != NULL)
		{
			 ...
			 * transaction here, and one backend start to do buffer replay, it
			 * can not acquire mini transaction lock and  replay to
...
polar_logindex_apply_page_from(polar_logindex_redo_ctl_t instance, XLogRecPtr start_lsn, BufferTag *tag, Buffer *buffer, polar_page_lock_t page_lock)
{
	...
	/*
	 * Release the mini transaction page_lock after obtaining the replay end
	 * lsn instead of holding it until the page replay is finished, which can
	 * reduce the time the startup process is blocked by page replay of
	 * backend process.
	 */
	if (page_lock != POLAR_INVALID_PAGE_LOCK)
		polar_logindex_mini_trans_unlock(instance->mini_trans, page_lock);
...
polar_logindex_lock_apply_buffer(polar_logindex_redo_ctl_t instance, Buffer *buffer)
{
	...
	if (polar_redo_check_state(buf_desc, POLAR_REDO_OUTDATE))
	{
		...
		start_lsn = instance->bg_replayed_lsn;
		POLAR_SET_BACKEND_READ_MIN_LSN(start_lsn);
		SpinLockRelease(&instance->info_lck);

		/*
		 * Prevent interrupts while replaying to avoid inconsistent buffer
		 * redo state. It's ok to process interrupts while replaying buffer
		 * with buffer's IO lock, because other backend process and background
		 * replay worker will read the buffer from storage and do the replay
		 * work again when current replay work is aborted. But When we replay
		 * the buffer with buffer content lock, we should hold interrupts
		 * because the bg_replayed_lsn may have advanced when we abort current
		 * replay work. Other backend process will replay the buffer from
		 * bg_replayed_lsn next time, thus resulting the replay of some wal is
		 * skiped. The replaying process should be in the critical section so
		 * that we can force system panic rather than abort current replaying
		 * buffer if there is any error.
		 */
		START_CRIT_SECTION();
		polar_logindex_lock_apply_page_from(instance, start_lsn, &buf_desc->tag, buffer);
		END_CRIT_SECTION();
...
static void
polar_logindex_apply_one_page(polar_logindex_redo_ctl_t instance, BufferTag *tag, Buffer *buffer, log_index_page_iter iter)
{
	static XLogReaderState *state = NULL;
	log_index_lsn_info lsn_info;
	MemoryContext oldcontext;
	...
	while ((lsn_info = polar_logindex_page_iterator_next(iter)) != NULL)
	{
		...
		if (polar_injection_point_find("polar_delay_page_replay"))
...
polar_create_bg_redo_ctl(polar_logindex_redo_ctl_t instance, bool enable_processes_pool)
{
	...
	XLogRecPtr	bg_replayed_lsn = polar_get_bg_replayed_lsn(instance);
	...
	if (enable_processes_pool)
	{
		...
		elog(LOG, "Create and start PolarDB parallel replay process pool");
	}
...
polar_logindex_apply_xlog_background(polar_logindex_bg_redo_ctl_t *ctl)
{
	...
	for (replayed_count = 0; replayed_count < ctl->replay_batch_size; replayed_count++)
	{
		 ...
		 * bg_replayed_lsn
		 */
		polar_advance_bg_replayed_lsn(ctl->instance, current_ptr);
	}
...
polar_bg_redo_get_record_id(XLogReaderState *state, BufferTag *tag)
{
	...
	for (i = 0; i <= XLogRecMaxBlockId(state); i++)
	{
		...
		if (!XLogRecGetBlockTagExtended(state, i, &rlocator, &blk_tag.forkNum,
										&blk_tag.blockNum, NULL))
			continue;

		blk_tag.spcOid = rlocator.spcOid;
		blk_tag.dbOid = rlocator.dbOid;
		blk_tag.relNumber = rlocator.relNumber;

		if (BufferTagsEqual(&blk_tag, tag))
			return i;
	}
								...
								BufferTag *tag, Buffer *buffer)
{
	...
	if (BufferIsInvalid(*buffer))
	{
		...
		if (XLogRecBlockImageApply(state, blk_id))
		{
			...
			{
				*buffer = XLogReadBufferExtended(POLAR_REL_FILE_LOCATOR(tag),
												 tag->forkNum, tag->blockNum, RBM_ZERO_ON_ERROR,
...
polar_pin_buffer_for_replay(BufferTag *tag, XLogRecPtr lsn, uint32 bg_redo_state, bool *is_flushed)
{
	...
	if (buf_id >= 0)
	{
		...
		if (!valid)
		{
			 ...
			 * POLAR_REDO_REPLAYING flag is cleared but BM_VALID is not set,
 ...
 */
static void
polar_only_replay_exists_buffer(polar_logindex_redo_ctl_t instance, XLogReaderState *state, log_index_lsn_info log_index_page)
{
	Buffer		buffer;
...
static Size
polar_forget_queue_shmem_size(Size req_size)
{
	Size		size = offsetof(polar_invalid_page_forget_queue_t, reqs);

	size += mul_size(req_size, sizeof(polar_invalid_page_forget_req_t));
	return size;
}
...
polar_logindex_redo_shmem_size(void)
{
	...
	if (polar_fullpage_logindex_mem_ratio > 0)
	{
		size = add_size(size,
						polar_logindex_shmem_size(polar_logindex_convert_mem_tbl_size(polar_logindex_mem_size * polar_fullpage_logindex_mem_ratio),
												  polar_logindex_bloom_blocks * polar_fullpage_logindex_mem_ratio));
	...
	if (polar_parallel_replay_proc_num > 0 && polar_parallel_replay_task_queue_depth > 0)
	{
		...
		size = add_size(size, polar_forget_queue_shmem_size(polar_invalid_page_forget_queue_size));
	}
...
void
polar_wal_logindex_init_lwlock_array(log_index_snapshot logindex_snapshot)
{
	int			mem_tbl_size = polar_logindex_mem_tbl_size(logindex_snapshot);

	POLAR_ASSERT_PANIC(mem_tbl_size > 0);

	polar_logindex_init_lwlock_array(logindex_snapshot, LOG_INDEX_MEMTBL_LOCK_OFFSET,
									 mem_tbl_size, LWTRANCHE_WAL_LOGINDEX_MEM_TBL);

	polar_logindex_init_lwlock_array(logindex_snapshot, LOG_INDEX_HASH_LOCK_OFFSET,
									 LOG_INDEX_MEM_TBL_HASH_LOCK_NUM,
									 LWTRANCHE_WAL_LOGINDEX_HASH_LOCK);

	polar_logindex_init_lwlock_array(logindex_snapshot, LOG_INDEX_IO_LOCK_OFFSET,
									 1, LWTRANCHE_WAL_LOGINDEX_IO);

	polar_logindex_init_lwlock_array(logindex_snapshot, LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK_OFFSET,
									 1, LWTRANCHE_WAL_LOGINDEX_FLUSH_ACTIVE_TBL);
}

static polar_invalid_page_forget_queue_t *
polar_create_forget_queue(const char *name, Size req_size)
{
	bool		found;
	polar_invalid_page_forget_queue_t *queue;

	queue = (polar_invalid_page_forget_queue_t *) ShmemInitStruct(name, polar_forget_queue_shmem_size(req_size), &found);

	if (!IsUnderPostmaster)
	{
		POLAR_ASSERT_PANIC(!found);
		pg_atomic_init_u64(&queue->start, 0);
		pg_atomic_init_u64(&queue->end, 0);
		queue->total_size = req_size;

		MemSet(queue->reqs, 0, sizeof(polar_invalid_page_forget_req_t) * req_size);
	}
	else
		POLAR_ASSERT_PANIC(found);
...
polar_logindex_redo_shmem_init(void)
{
	...
	instance->wal_logindex_snapshot = polar_logindex_snapshot_shmem_init(WAL_LOGINDEX_DIR,
																		 ...
																		 polar_logindex_bloom_blocks * WAL_LOGINDEX_MEM_RATIO,
																		 polar_wal_logindex_init_lwlock_array,
																		 LWTRANCHE_WAL_LOGINDEX_BLOOM_LRU_BUFFER, LWTRANCHE_WAL_LOGINDEX_BLOOM_LRU);
	...
	if (polar_rel_size_cache_blocks > 0)
		...
		elog(FATAL, "%s: PolarDB relation size cache use wrong block size %d",
			 __func__, polar_rel_size_cache_blocks);
	...
	if (polar_xlog_queue_buffers > 0)
		...
		elog(FATAL, "%s: PolarDB xlog queue use wrong buffer size %d",
			 __func__, polar_xlog_queue_buffers);
	...
	if (polar_fullpage_logindex_mem_ratio > 0)
	{
		instance->fullpage_logindex_snapshot = polar_logindex_snapshot_shmem_init(FULLPAGE_LOGINDEX_DIR,
																				  polar_logindex_convert_mem_tbl_size(polar_logindex_mem_size * polar_fullpage_logindex_mem_ratio),
																				  polar_logindex_bloom_blocks * polar_fullpage_logindex_mem_ratio,
																				  polar_fullpage_logindex_init_lwlock_array,
																				  LWTRANCHE_FULLPAGE_LOGINDEX_BLOOM_LRU_BUFFER, LWTRANCHE_FULLPAGE_LOGINDEX_BLOOM_LRU);
	...
	pg_atomic_init_u64(&instance->oldest_replayed_lsn, InvalidXLogRecPtr);
	...
	if (polar_parallel_replay_proc_num > 0 && polar_parallel_replay_task_queue_depth > 0)
	{
		...
		instance->forget_queue = polar_create_forget_queue("polar_parallel_replay_forget_queue", polar_invalid_page_forget_queue_size);
	}

	pg_atomic_init_u32(&instance->startup_process_status, 0);
						 ...
						 TimeLineID checkpoint_tli, bool read_only)
{
	...
	if (instance)
	{
		...
		/* bg_replayed_lsn should never be lower than redo lsn. */
		polar_advance_bg_replayed_lsn(instance, GetRedoRecPtr());
	}
...
polar_logindex_bg_dispatch(polar_logindex_bg_redo_ctl_t *ctl, bool *can_hold)
{
	...
	XLogRecPtr	bg_replayed_lsn,
				backend_min_lsn,
				xlog_replayed_lsn = InvalidXLogRecPtr,
				oldest_replayed_lsn;
	int			replayed_count;
	...
	*can_hold = false;
	...
	if (polar_injection_point_find("test_skip_dispatch"))
	{
		*can_hold = true;
		return false;
	}
	...
	for (replayed_count = 0; replayed_count < ctl->replay_batch_size; replayed_count++)
	{
		...
		/* If no more item in logindex, hold for a while and then retry */
		if (!ctl->replay_page)
		{
			*can_hold = true;
			break;
		...
		/*
		 * If current xlog record is beyond replayed lsn, just hold for a
		...
		if (!XLogRecPtrIsInvalid(prev_replay_lsn) &&
			...
			polar_injection_point_find("polar_inject_panic"))
		{
		...
		POLAR_INIT_BUFFER_TAG(&node.tag, ctl->replay_page->tag);
		node.lsn = ctl->replay_page->lsn;
		node.prev_lsn = ctl->replay_page->prev_lsn;
		node.kind = POLAR_TASK_REPLAY_BUFFER;
	...
	if (replayed_count == ctl->replay_batch_size)
		dispatch_done = false;
	 ...
	 *
	 * Notice: we need to fetch GetXLogReplayRecPtr(NULL) at the very
	 * beginning, because WALs may be appended into logindex snapshot during
	 * our check. Forwarding to an improper RecPtr may skip the newly added
	 * WALs without replaying.
	 */
	if (dispatch_done)
	...
	if (dispatch_done)
	{
		XLogRecPtr	new_bg_replayed_lsn = Max(xlog_replayed_lsn, ctl->max_dispatched_lsn);

		POLAR_ASSERT_PANIC(ctl->instance != NULL);
		polar_advance_bg_replayed_lsn(ctl->instance, new_bg_replayed_lsn);
		elog(polar_trace_logindex(DEBUG3), "%s: forward bg_replayed_lsn to %X/%X", __func__, LSN_FORMAT_ARGS(new_bg_replayed_lsn));
	}

	bg_replayed_lsn = polar_get_bg_replayed_lsn(ctl->instance);
	pg_read_barrier();
	...
	if (!XLogRecPtrIsInvalid(backend_min_lsn) && backend_min_lsn < bg_replayed_lsn)
		oldest_replayed_lsn = backend_min_lsn;
	else
		oldest_replayed_lsn = bg_replayed_lsn;
	polar_advance_oldest_replayed_lsn(ctl->instance, oldest_replayed_lsn);

	return dispatch_done;
}

static bool
logindex_worker_handle_interrupts(void)
{
	bool		shutdown = false;
...
/*
 * logindex_worker_forget_invalid_pages: logindex worker broadcast
 * forget invalid page request to all the parallel replay subprocs.
 *
 * The broadcasted forget invalid requests's lsn must be lower than
 * current bg_replayed_lsn.
 */
static void
logindex_worker_forget_invalid_pages(polar_logindex_bg_redo_ctl_t *ctl, Latch *latch, bool may_send_all)
{
	polar_task_sched_ctl_t *sched_ctl = ctl->sched_ctl;
	polar_logindex_redo_ctl_t instance = ctl->instance;
	polar_invalid_page_forget_queue_t *forget_queue = instance->forget_queue;

	uint64		queue_start = pg_atomic_read_u64(&forget_queue->start);
	uint64		queue_end = pg_atomic_read_u64(&forget_queue->end);
	XLogRecPtr	bg_replayed_lsn = polar_get_bg_replayed_lsn(instance);

	/* bg_replayed_lsn may be the EndRecPtr of wal record */
	if (bg_replayed_lsn % XLOG_BLCKSZ == 0)
	{
		uint64		offset = XLogSegmentOffset(bg_replayed_lsn, wal_segment_size);

		bg_replayed_lsn += (offset == 0) ? SizeOfXLogLongPHD : SizeOfXLogShortPHD;
	}
	...
	{
		parallel_replay_task_node_t node;
		polar_invalid_page_forget_req_t *req = &forget_queue->reqs[queue_start % forget_queue->total_size];

		/* send forget requests whose lsn is not bigger than bg_replayed_lsn */
		if (req->lsn > bg_replayed_lsn)
			break;

		node.tag = req->buftag;
		node.lsn = req->lsn;
		node.prev_lsn = req->lsn;
		node.kind = POLAR_TASK_FORGET_INVALID_PAGE;

		if (polar_sched_broadcast_task(sched_ctl, (polar_task_node_t *) &node))
		{
			queue_start = pg_atomic_add_fetch_u64(&forget_queue->start, 1);
			if (POLAR_STARTUP_IN_STATUS(instance, POLAR_STARTUP_WAIT_FORGET_INVALID_PAGES))
				WakeupRecovery();
			continue;
		}

		/* failed to broadcast task, one of sub procs' queue must be full */

		/*
		 * don't wait if may_send_all is false and startup is not waiting
		 * forget queue
		 */
		if (!may_send_all && !POLAR_STARTUP_IN_STATUS(instance, POLAR_STARTUP_WAIT_FORGET_INVALID_PAGES))
			break;

		/* exit loop when shutdown request was received */
		if (logindex_worker_handle_interrupts())
			break;

		/* must wait for sub procs to finish tasks */
		WaitLatch(latch, WL_LATCH_SET | WL_TIMEOUT | WL_EXIT_ON_PM_DEATH, 10,
				  WAIT_EVENT_PARALLEL_REPLAY_BROADCAST);
...
/*
 * logindex_worker_check_invalid_pages: logindex worker broadcast
 * check invalid page request to all the parallel replay subprocs.
 *
 * Before broadcasting check invalid page request, we must make sure
 * that all the forget invalid page requests are broadcasted.
 */
static void
logindex_worker_check_invalid_pages(polar_logindex_bg_redo_ctl_t *ctl, Latch *latch)
{
	bool		finished = false;
	...
	{
		logindex_worker_forget_invalid_pages(ctl, latch, true);
		pg_memory_barrier();

		/* check again and enter into anothor round if needed */
		queue_start = pg_atomic_read_u64(&forget_queue->start);
		queue_end = pg_atomic_read_u64(&forget_queue->end);
		if (queue_start < queue_end)
			return;
	}

	MemSet(&node, 0, sizeof(parallel_replay_task_node_t));
	node.kind = POLAR_TASK_CHECK_INVALID_PAGES;

	while (!finished)
	{
		if (!broadcast && polar_sched_broadcast_task(sched_ctl, (polar_task_node_t *) &node))
			broadcast = true;
		...
		if (broadcast)
			finished = polar_sched_empty_running_task(sched_ctl);

		if (finished)
		{
			elog(LOG, "all %d sub procs succ to check all invalid pages", sched_ctl->sched->total_proc);
			POLAR_STARTUP_REMOVE_STATUS(ctl->instance, POLAR_STARTUP_WAIT_CHECK_INVALID_PAGES);
			WakeupRecovery();
			break;
		}
...
/*
 * logindex_worker_finish_parallel_replay: finish parallel replay and reset
 * bg_redo_state for instant recovery and online promote.
 *
 * We have to make sure the bg_replayed_lsn is bigger or equal than the current
 * lastReplayedEndRecPtr before finishing parallel replay. And it's better to
 * request one online checkpoint after finished parallel replay.
 */
static void
logindex_worker_finish_parallel_replay(polar_logindex_redo_ctl_t instance, char *replay_title)
{
	XLogRecPtr	bg_replayed_lsn = polar_get_bg_replayed_lsn(instance);
	XLogRecPtr	last_replayed_end_lsn = polar_get_xlog_replay_recptr_nolock();

	/* Exit parallel replay mode if needed */
	if (bg_replayed_lsn >= last_replayed_end_lsn &&
		XLogRecPtrIsInvalid(polar_get_backend_min_replay_lsn()))
	{
		...
		elog(LOG, "Background process finished parallel replay for %s, last_replayed_end_lsn=%X/%X",
			 replay_title, LSN_FORMAT_ARGS(polar_get_bg_replayed_lsn(instance)));
	}
}

static bool
polar_logindex_bg_parallel_replay(polar_logindex_bg_redo_ctl_t *ctl, bool *can_hold, Latch *latch)
{
	bool		dispatch_done = polar_logindex_bg_dispatch(ctl, can_hold);
	polar_logindex_redo_ctl_t instance = ctl->instance;

	if (dispatch_done || *can_hold ||
		POLAR_STARTUP_IN_STATUS(instance, POLAR_STARTUP_WAIT_FORGET_INVALID_PAGES))
		logindex_worker_forget_invalid_pages(ctl, latch, false);

	if ((dispatch_done || *can_hold) &&
		POLAR_STARTUP_IN_STATUS(instance, POLAR_STARTUP_WAIT_CHECK_INVALID_PAGES))
		logindex_worker_check_invalid_pages(ctl, latch);

	/* startup process has parsed all valid wal records in primary node */
	if (dispatch_done && polar_is_primary() &&
		POLAR_STARTUP_IN_STATUS(instance, POLAR_STARTUP_WAIT_FINISH_REPLAY))
		logindex_worker_finish_parallel_replay(instance, "instant recovery");
...
bool
polar_logindex_redo_bg_replay(polar_logindex_bg_redo_ctl_t *ctl, bool *can_hold, Latch *latch)
{
	uint32		state;
	...
	/* inject fault */
#ifdef USE_INJECTION_POINTS
	if (polar_injection_point_find("polar_logindex_bg_skip_replay"))
		return false;
#endif
	/* inject fault end */
	...
	*can_hold = false;
	...
	switch (state)
	{
		...
		case POLAR_BG_PARALLEL_REPLAYING:
			replay_done = polar_logindex_bg_parallel_replay(ctl, can_hold, latch);
			break;
...
bool
polar_logindex_finished_bg_replay(polar_logindex_redo_ctl_t instance)
{
...
polar_logindex_redo_get_min_replay_from_lsn(polar_logindex_redo_ctl_t instance, XLogRecPtr consist_lsn)
{
	...
	bg_replayed_lsn = polar_get_bg_replayed_lsn(instance);
...
{
#define POLAR_UPDATE_DATA_KEEP_REF_COUNT	(10000)
	DecodedXLogRecord *decoded;
	static XLogReaderState *state = NULL;
	static TimestampTz last_flush_time = 0;
	static uint64 pop_count = 0;
	static uint64 new_pread = POLAR_RINGBUF_MIN_PREAD;
	static bool need_update_pread = true;
	...
	while ((decoded = polar_xlog_send_queue_pop(instance->xlog_queue, &ref, state)))
	{
		if (polar_xlog_remove_payload(&decoded->header))
			polar_logindex_save_lsn(instance, state);

		/* update data_keep_ref */
		pop_count++;
		if (pop_count == POLAR_UPDATE_DATA_KEEP_REF_COUNT)
		{
			if (need_update_pread)
				new_pread = instance->xlog_queue->slot[ref.slot].pread;

			need_update_pread = polar_xlog_send_queue_keep_data(instance->xlog_queue, new_pread);
			pop_count = 0;
		}
	}
...
/*
 * POLAR: Reset background replayed lsn.
 */
void
polar_reset_bg_replayed_lsn(polar_logindex_redo_ctl_t instance, XLogRecPtr lsn)
{
	if (instance)
	{
		SpinLockAcquire(&instance->info_lck);
...
/*
 * POLAR: Advance background replayed lsn.
 */
void
polar_advance_bg_replayed_lsn(polar_logindex_redo_ctl_t instance, XLogRecPtr lsn)
{
	if (instance)
	{
		SpinLockAcquire(&instance->info_lck);
		if (lsn > instance->bg_replayed_lsn)
			instance->bg_replayed_lsn = lsn;
		SpinLockRelease(&instance->info_lck);
	}
}
 ...
 */
XLogRecPtr
polar_get_bg_replayed_lsn(polar_logindex_redo_ctl_t instance)
{
...
/*
 * POLAR: Only one process can advance oldest replayed lsn.
 */
void
polar_reset_oldest_replayed_lsn(polar_logindex_redo_ctl_t instance, XLogRecPtr lsn)
{
	if (instance)
		pg_atomic_write_u64(&instance->oldest_replayed_lsn, lsn);
}
 ...
 */
void
polar_advance_oldest_replayed_lsn(polar_logindex_redo_ctl_t instance, XLogRecPtr lsn)
{
	if (instance)
	{
		POLAR_ASSERT_PANIC(pg_atomic_read_u64(&instance->oldest_replayed_lsn) <= lsn);
		pg_atomic_write_u64(&instance->oldest_replayed_lsn, lsn);
	}
}

XLogRecPtr
polar_get_oldest_replayed_lsn(polar_logindex_redo_ctl_t instance)
{
	XLogRecPtr	lsn = InvalidXLogRecPtr;

	if (instance)
	{
		lsn = pg_atomic_read_u64(&instance->oldest_replayed_lsn);
		POLAR_ASSERT_PANIC(!XLogRecPtrIsInvalid(lsn));
	}
...
{
	if (instance && polar_get_bg_redo_state(instance) == POLAR_BG_REDO_NOT_START)
	{
		uint32		state = POLAR_BG_REDO_NOT_START;

		polar_advance_bg_replayed_lsn(instance, replayed_lsn);

		elog(LOG, "instance bg_replayed_lsn set to %X/%X", LSN_FORMAT_ARGS(replayed_lsn));

		if (POLAR_PARALLEL_REPLAY_IN_RECOVERY() ||
			POLAR_PARALLEL_REPLAY_IN_INSTANT_RECOVERY())
		{
			polar_advance_oldest_replayed_lsn(instance, replayed_lsn);
...
polar_logindex_require_backend_redo(polar_logindex_redo_ctl_t instance, ForkNumber fork_num, XLogRecPtr *replay_from)
{
	...
	*replay_from = InvalidXLogRecPtr;
	...
	else if (polar_get_bg_redo_state(instance) == POLAR_BG_PARALLEL_REPLAYING)
	{
		XLogRecPtr	redo_lsn = GetRedoRecPtr();
	...
	else if (polar_get_bg_redo_state(instance) == POLAR_BG_ONLINE_PROMOTE)
	{
...
polar_logindex_redo_set_valid_info(polar_logindex_redo_ctl_t instance, XLogRecPtr start_lsn)
{
	...
	if (POLAR_LOGINDEX_ENABLE_FULLPAGE()
		&& polar_logindex_check_state(instance->fullpage_logindex_snapshot, POLAR_LOGINDEX_STATE_WRITABLE)
...
static void
parallel_replay_task_finished(polar_task_sched_t *sched, parallel_replay_task_node_t *parallel_task, polar_logindex_redo_ctl_t instance)
{
	...
	if (head_lsn == parallel_task->lsn)
		polar_advance_bg_replayed_lsn(instance, parallel_task->lsn);
}
...
static bool
handle_replay_buffer(polar_task_sched_t *sched, parallel_replay_task_node_t *parallel_task)
{
	static XLogReaderState *state = NULL;
	...
	if (handled)
		parallel_replay_task_finished(sched, parallel_task, instance);

	return handled;
}

/*
 * handle_forget_invalid_pages: parallel replay subprocs handle forget
 * invalid pages' request.
 */
static bool
handle_forget_invalid_pages(polar_task_sched_t *sched, parallel_replay_task_node_t *parallel_task)
{
	BufferTag  *tag = &parallel_task->tag;

	if (OidIsValid(tag->dbOid) && !OidIsValid(tag->relNumber) &&
		!OidIsValid(tag->spcOid) && tag->forkNum == InvalidForkNumber &&
		!BlockNumberIsValid(tag->blockNum))
		polar_forget_invalid_pages_db(tag->dbOid);
	else
		polar_forget_invalid_pages(BufTagGetRelFileLocator(tag), tag->forkNum, tag->blockNum);

	return true;
}

/*
 * handle_check_invalid_pages: parallel replay subprocs handle check
 * invalid pages' request.
 */
static bool
handle_check_invalid_pages(polar_task_sched_t *sched, parallel_replay_task_node_t *parallel_task)
{
...
handle_parallel_replay_task(polar_task_sched_t *sched, polar_task_node_t *task)
{
	...
	switch (parallel_task->kind)
	{
		case POLAR_TASK_REPLAY_BUFFER:
			handled = handle_replay_buffer(sched, parallel_task);
			break;

		case POLAR_TASK_FORGET_INVALID_PAGE:
			handled = handle_forget_invalid_pages(sched, parallel_task);
			break;

		case POLAR_TASK_CHECK_INVALID_PAGES:
			handled = handle_check_invalid_pages(sched, parallel_task);
			break;

		default:
			elog(PANIC, "wrong parallel replay task kind: %d", parallel_task->kind);
	}
 ...
 */
void
polar_promote_reset_bg_replayed_lsn(polar_logindex_redo_ctl_t instance, XLogRecPtr oldest_redo_ptr)
{
	XLogRecPtr	bg_replayed_lsn;
	...
	polar_reset_bg_replayed_lsn(instance, oldest_redo_ptr);
	polar_reset_oldest_replayed_lsn(instance, oldest_redo_ptr);
...
static void
polar_logindex_promote_fullpage_snapshot(log_index_snapshot logindex_snapshot, XLogRecPtr last_replayed_lsn)
{
	char		path[MAXPGPATH];
	int			fd = -1;

	/*
	 * If you upgrade from an old version that does not support FPSI to a new
	 * version, the polar_fullpage directory and the logindex meta file will
	 * not exist during the online promote process, so they need to be created
	 * during the online promote process.
	 */
	snprintf(path, MAXPGPATH, "%s/%s", POLAR_DATA_DIR(), polar_get_logindex_snapshot_dir(logindex_snapshot));
	polar_validate_dir(path);

	snprintf(path, MAXPGPATH, "%s/%s/%s", POLAR_DATA_DIR(), polar_get_logindex_snapshot_dir(logindex_snapshot), LOG_INDEX_META_FILE);

	if ((fd = PathNameOpenFile(path, O_RDONLY | PG_BINARY)) < 0)
	{
		if (errno == ENOENT)
		{
			elog(LOG, "create fullpage logindex meta file \"%s\"", path);
			polar_logindex_set_start_lsn(logindex_snapshot, last_replayed_lsn);
		}
		else
			elog(FATAL, "failed to open fullpage logindex meta file \"%s\": %m", path);
	}
	else
		FileClose(fd);

	polar_logindex_update_promoted_info(logindex_snapshot, last_replayed_lsn);
}
...
polar_online_promote_data(polar_logindex_redo_ctl_t instance)
{
	...
	ereport(LOG, errmsg("The replica promote data at lastReplayedEndRecPtr %X/%X",
						LSN_FORMAT_ARGS(last_replayed_lsn)));
	...
	if (instance->fullpage_logindex_snapshot)
		polar_logindex_promote_fullpage_snapshot(instance->fullpage_logindex_snapshot, last_replayed_lsn);
...
polar_standby_promote_data(polar_logindex_redo_ctl_t instance)
{
	...
	ereport(LOG, errmsg("The standby promote data at lastReplayedEndRecPtr %X/%X",
						LSN_FORMAT_ARGS(last_replayed_lsn)));
...
void
polar_primary_promote_data(polar_logindex_redo_ctl_t instance)
{
	XLogRecPtr	last_replayed_lsn = GetXLogReplayRecPtr(NULL);

	ereport(LOG, errmsg("The primary promote data at lastReplayedEndRecPtr %X/%X",
						LSN_FORMAT_ARGS(last_replayed_lsn)));
...
void
polar_wait_logindex_worker_handle_promote(polar_logindex_redo_ctl_t instance)
{
	if (!instance)
		return;
	...
	while (polar_get_bg_redo_state(instance) != POLAR_BG_WAITING_RESET)
		polar_startup_wait_latch(10, WAIT_EVENT_RECOVERY_APPLY_DELAY);

	elog(LOG, "logindex worker handled online promote signal");
}

void
polar_wait_logindex_worker_finish_parallel_replay(polar_logindex_redo_ctl_t instance)
{
	if (!instance)
		return;
	...
	elog(LOG, "startup is waiting for logindex worker to finish replay");
	POLAR_STARTUP_SET_STATUS(instance, POLAR_STARTUP_WAIT_FINISH_REPLAY);

	/* Wait for logindex worker to finish all parallel replay work */
	while (polar_get_bg_redo_state(instance) != POLAR_BG_REDO_NOT_START)
		polar_startup_wait_latch(10, WAIT_EVENT_RECOVERY_APPLY_DELAY);

	POLAR_STARTUP_REMOVE_STATUS(instance, POLAR_STARTUP_WAIT_FINISH_REPLAY);
	elog(LOG, "logindex worker has finished parallel replay");
}

static void
logindex_replay_specific_pages(polar_logindex_redo_ctl_t instance, XLogRecPtr start_lsn,
							   log_index_lsn_info_cmp cmp, log_index_lsn_info target_info)
{
	log_index_lsn_info replay_info;
	log_index_lsn_iter lsn_iter;
	XLogReaderState *state;
	MemoryContext oldcontext;
	XLogRecPtr	end_lsn = InvalidXLogRecPtr;

	POLAR_ASSERT_PANIC(cmp && target_info);
	elog(LOG, "%s start with lsn %X/%X", __func__, LSN_FORMAT_ARGS(start_lsn));
	...
	MemoryContextSwitchTo(oldcontext);
	elog(LOG, "%s end with lsn %X/%X", __func__, LSN_FORMAT_ARGS(end_lsn));
}

static bool
dbnode_cmp(log_index_lsn_info info_a, log_index_lsn_info info_b)
{
	if (info_a->tag->dbOid == info_b->tag->dbOid)
		return true;
	return false;
}

void
polar_logindex_replay_db(polar_logindex_redo_ctl_t instance, Oid db_oid)
{
	BufferTag	target_tag = {0};
	log_index_lsn_info_t target_info = {0};
	XLogRecPtr	start_lsn = polar_get_bg_replayed_lsn(instance);

	target_tag.dbOid = db_oid;
...
{
	static XLogReaderState *state = NULL;
	log_index_lsn_info lsn_info = NULL;
...
retry:
	 ...
	 * read a future page
	 */
	replayed_lsn = polar_get_xlog_replay_recptr_nolock();
	if (!polar_is_future_page(GetBufferDescriptor(*buffer - 1), replayed_lsn))
		return false;

	pgstat_report_wait_start(WAIT_EVENT_LOGINDEX_WAIT_FULLPAGE);
	pg_atomic_add_fetch_u64(&instance->fullpage_ctl->stat.read_fullpage_count, 1);
	...
	if (lsn_info != NULL)
	{
		...
		polar_logindex_apply_one_record(instance, state, tag, buffer);
		success = true;
		pg_atomic_add_fetch_u64(&instance->fullpage_ctl->stat.restore_oldpage_count, 1);
	}
...
extern void
polar_parallel_replay_check_invalid_pages(void)
{
	polar_logindex_redo_ctl_t instance = polar_logindex_redo_instance;
	long		wait_latch_timeout = 10;

	if (!instance || !AmStartupProcess() ||
		polar_get_bg_redo_state(instance) != POLAR_BG_PARALLEL_REPLAYING)
		return;

	elog(LOG, "startup is waiting for logindex worker to check invalid pages");
	POLAR_STARTUP_SET_STATUS(instance, POLAR_STARTUP_WAIT_CHECK_INVALID_PAGES);

	NOTIFY_LOGINDEX_BG_WORKER(instance->bg_worker_latch);

	/* wait for all parallel replay procs to forget all invalid pages */
	while (POLAR_STARTUP_IN_STATUS(instance, POLAR_STARTUP_WAIT_CHECK_INVALID_PAGES))
		polar_startup_wait_latch(wait_latch_timeout, WAIT_EVENT_LOGINDEX_CHECK_INVALID_PAGE);

	elog(LOG, "logindex worker succeed to check invalid pages");
}

static void
parallel_replay_forget_invalid_pages(BufferTag *tag)
{
	polar_logindex_redo_ctl_t instance = polar_logindex_redo_instance;
	polar_invalid_page_forget_queue_t *forget_queue;
	long		wait_latch_timeout = 10;
	static uint64 wait_latch_count = 0;
	uint64		queue_start,
				queue_end;
	XLogRecPtr	cur_lsn;
	...
	forget_queue = instance->forget_queue;
	cur_lsn = polar_get_replay_read_recptr();
	queue_start = pg_atomic_read_u64(&forget_queue->start);
	queue_end = pg_atomic_read_u64(&forget_queue->end);
	POLAR_ASSERT_PANIC(queue_start <= queue_end);

	/* enter into POLAR_STARTUP_WAIT_FORGET_INVALID_PAGES when queue is full */
	while (queue_end - queue_start >= forget_queue->total_size)
	{
		wait_latch_count++;
		POLAR_STARTUP_SET_STATUS(instance, POLAR_STARTUP_WAIT_FORGET_INVALID_PAGES);
		elog(WARNING, "startup wait %ld times for forget invalid page at %X/%X, "
			 "start pos %ld, end pos %ld, queue length %ld",
			 wait_latch_count, LSN_FORMAT_ARGS(cur_lsn),
			 queue_start, queue_end, forget_queue->total_size);

		polar_startup_wait_latch(wait_latch_timeout, WAIT_EVENT_LOGINDEX_FORGET_INVALID_PAGE);

		queue_start = pg_atomic_read_u64(&forget_queue->start);
		queue_end = pg_atomic_read_u64(&forget_queue->end);
	}

	POLAR_STARTUP_REMOVE_STATUS(instance, POLAR_STARTUP_WAIT_FORGET_INVALID_PAGES);
	/* add invalid page forget request into queue */
	forget_queue->reqs[queue_end % forget_queue->total_size].buftag = *tag;
	forget_queue->reqs[queue_end % forget_queue->total_size].lsn = cur_lsn;
	pg_atomic_add_fetch_u64(&forget_queue->end, 1);
}

extern void
polar_parallel_replay_forget_invalid_pages(RelFileLocator locator, ForkNumber forkno, BlockNumber minblkno)
{
	BufferTag	buftag = (BufferTag) {
		.spcOid = locator.spcOid,
		.dbOid = locator.dbOid,
		.relNumber = locator.relNumber,
		.forkNum = forkno,
		.blockNum = minblkno
	};

	parallel_replay_forget_invalid_pages(&buftag);
}

extern void
polar_parallel_replay_forget_invalid_pages_db(Oid dbid)
{
	BufferTag	buftag = (BufferTag) {
		.dbOid = dbid,
...
/*
 * polar_replica_set_cascade_recovery: The replica can mark it self in cascade
 * recovery mode when controlfile's minRecoveryPoint isn't InvalidXLogRecPtr
 * and the database state is not DB_IN_PRODUCTION.
 *
 * The standby would update controlfile's minRecoveryPoint to the latest
 * replayed record's end lsn during recovery. The primary node would keep
 * controlfile's minRecoveryPoint to be InvalidXLogRecPtr in order to replay
 * all the existed wal records during crash recovery. Besides, the primary
 * node would set the database state of controlfile to DB_IN_PRODUCTION.
 */
void
polar_replica_set_cascade_recovery(void)
{
	...
	ControlFile = get_controlfile(polar_datadir, &crc_ok);
	...
	if (XLogRecPtrIsInvalid(ControlFile->minRecoveryPoint) ||
		...
		POLAR_STARTUP_REMOVE_STATUS(polar_logindex_redo_instance, POLAR_STARTUP_IN_CASCADE_REPLICA);
	else
	{
		WalRcvData *walrcv = WalRcv;

		/* try to update shared flushedUpto */
		SpinLockAcquire(&walrcv->mutex);
		if (walrcv->receiveStart == 0 ||
			(walrcv->receivedTLI == ControlFile->minRecoveryPointTLI &&
			 walrcv->flushedUpto < ControlFile->minRecoveryPoint))
		{
			walrcv->flushedUpto = ControlFile->minRecoveryPoint;
		}
		SpinLockRelease(&walrcv->mutex);
		POLAR_STARTUP_SET_STATUS(polar_logindex_redo_instance, POLAR_STARTUP_IN_CASCADE_REPLICA);
		ereport(LOG, errmsg("replica is in cascade recovery mode"));
	}
}
