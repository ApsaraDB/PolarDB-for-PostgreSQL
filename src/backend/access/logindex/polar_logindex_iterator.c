 ...
 *	  logindex wal iterator and page iterator.
 *
...
static uint64 log_index_iter_mem_size = 0;

#define LOG_INDEX_ITER_MEM_SIZE_ADD(size) \
	do \
	{ \
		log_index_iter_mem_size += (size); \
	} while (0)

#define LOG_INDEX_ITER_MEM_SIZE_SUB(size) \
	do \
	{ \
		Assert(log_index_iter_mem_size >= (size)); \
		log_index_iter_mem_size -= (size); \
	} while (0)

#define LOG_INDEX_ITER_CHECK_START() \
	do \
	{ \
		log_index_iter_mem_size = 0; \
	} while (0)

#define LOG_INDEX_ITER_CHECK_END(size) \
	do \
	{ \
		Assert(log_index_iter_mem_size == 0); \
	} while (0)

#else

#define LOG_INDEX_ITER_MEM_SIZE_ADD(size) ((void) true)
#define LOG_INDEX_ITER_MEM_SIZE_SUB(size) ((void) true)

#define LOG_INDEX_ITER_CHECK_START() ((void) true)
#define LOG_INDEX_ITER_CHECK_END() ((void) true)

#endif

#define UINT32_ACCESS_ONCE(var)      ((uint32)(*((volatile uint32 *)&(var))))

static log_index_table_t *log_index_get_table_data(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter, log_index_mem_table *mem_table);
static void log_index_set_start_order(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter);
...
log_index_tbl_stack_next_item(log_index_tbl_stack_lsn_t *stack, uint16 *idx)
{
	...
	if (stack->head == NULL || stack->head->item_size == LOG_INDEX_PAGE_STACK_ITEM_SIZE)
	{
		...
		stack->head = item;
		LOG_INDEX_ITER_MEM_SIZE_ADD(sizeof(log_index_page_lsn_t));
	}
	else
		item = stack->head;
...
static bool
log_index_tbl_stack_pop_lsn(log_index_tbl_stack_lsn_t *stack, log_index_lsn_info lsn_info)
{
	log_index_page_lsn_t *item;
	uint8		idx;
	...
	if (item->iter_pos == 0)
	{
		...
		pfree(item);
		LOG_INDEX_ITER_MEM_SIZE_SUB(sizeof(log_index_page_lsn_t));
	}
...
log_index_iter_push_tbl_stack(log_index_page_stack_lsn_t *stack)
{
	...
	LOG_INDEX_ITER_MEM_SIZE_ADD(sizeof(log_index_tbl_stack_lsn_t));
	return tbl_stack;
}
...
log_index_pop_tbl_stack(log_index_page_stack_lsn_t *stack)
{
	...
	if (tbl_stack != NULL)
	{
		...
		pfree(tbl_stack);
		LOG_INDEX_ITER_MEM_SIZE_SUB(sizeof(log_index_tbl_stack_lsn_t));
	}
}
...
static BufferTag *
log_index_get_seg_page_tag(log_index_table_t *table, log_seg_id_t seg_id)
{
...
static XLogRecPtr
log_index_get_seg_prev_lsn(log_index_table_t *table,
...
static void
log_index_page_iterate_push_lsn(log_index_page_iter iter,
								log_index_tbl_stack_lsn_t *stack,
								log_index_table_t *table,
...
static void
log_index_iterate_table_data(log_index_page_iter iter, log_index_table_t *table, log_item_head_t *item_head)
{
	log_item_seg_t *item;
...
static void
log_index_push_tbl_lsn(log_index_page_iter iter, log_index_table_t *table)
{
...
static bool
log_index_table_in_range(log_index_page_iter iter, log_index_mem_table table)
{
...
log_index_push_mem_tbl_lsn(log_index_snapshot logindex_snapshot, log_index_page_iter iter)
{
	uint32		mid;
...
static bool
log_index_check_hollow_table(log_index_snapshot logindex_snapshot, log_index_page_iter iter,
...
static bool
log_index_check_bloom_not_exists(log_index_snapshot logindex_snapshot, log_index_page_iter iter, log_idx_table_id_t tid)
{
	...
	LWLock	   *lock = SimpleLruGetBankLock(&logindex_snapshot->bloom_ctl, pageno);

	bloom_data = palloc(LOG_INDEX_FILE_TBL_BLOOM_SIZE);

	/*
	 * Notice: We will acquire bloom lru bank lock in log_index_get_tbl_bloom
	 * function
	 */
...
static void
log_index_push_file_tbl_lsn(log_index_snapshot logindex_snapshot, log_index_page_iter iter, log_idx_table_id_t tid)
{
	log_index_table_t *table;
		   ...
		   tid >= min_seg->min_idx_table_id && iter->state == ITERATE_STATE_FORWARD)
	{
		...
		log_index_mem_table mem_table = LOG_INDEX_MEM_TBL(mid);
...
log_index_page_iter
polar_logindex_create_page_iterator(log_index_snapshot logindex_snapshot, BufferTag *tag, XLogRecPtr min_lsn, XLogRecPtr max_lsn, bool before_promote)
{
	log_idx_table_id_t tid;
	log_index_page_iter iter = NULL;
	...
	iter = palloc0(sizeof(log_index_page_iter_t));

	LOG_INDEX_ITER_CHECK_START();
	LOG_INDEX_ITER_MEM_SIZE_ADD(sizeof(log_index_page_iter_t));
...
void
polar_logindex_release_page_iterator(log_index_page_iter iter)
{
	log_index_stack_release(&iter->lsn_stack);
	pfree(iter);

	LOG_INDEX_ITER_MEM_SIZE_SUB(sizeof(log_index_page_iter_t));
	LOG_INDEX_ITER_CHECK_END();
}

log_index_lsn_info
polar_logindex_page_iterator_next(log_index_page_iter iter)
{
	log_index_page_stack_lsn_t *stack = &(iter->lsn_stack);
	log_index_lsn_info lsn_info = &iter->lsn_info;
...
bool
polar_logindex_page_iterator_end(log_index_page_iter iter)
{
	return iter->lsn_stack.head == NULL;
}

log_index_iter_state_t
polar_logindex_page_iterator_state(log_index_page_iter iter)
{
	return iter->state;
}

static void
log_index_set_search_table(log_index_lsn_iter iter, log_index_table_t *table)
{
	...
	else if (iter->last_search_tid == LOG_INDEX_TABLE_INVALID_ID)
	{
		/*
		 * Just skip the current table and do not set last_search_tid when the
		 * first searched table is empty.
		 */
		return;
	}
...
XLogRecPtr
log_index_get_order_lsn(log_index_table_t *table, uint32 order, log_index_lsn_info lsn_info)
{
	log_seg_id_t seg_id;
	uint8		idx;
	log_item_seg_t *seg;
...
static void
log_index_set_search_start_order(log_index_lsn_iter iter, log_index_table_t *table)
{
	uint32		order = 0;
...
static void
log_index_set_start_order(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter)
{
	log_index_mem_table table = NULL;
...
static void
log_index_search_mem_tbl_lsn(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter)
{
	uint32		mid;
	log_index_mem_table table;
	log_idx_table_id_t tid;
...
static void
log_index_search_file_tbl_lsn(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter)
{
	log_idx_table_id_t tid;
	log_index_meta_t meta;
	bool		hollow;
	...
	while (tid != LOG_INDEX_TABLE_INVALID_ID
		   && tid >= min_seg->min_idx_table_id && iter->state == ITERATE_STATE_FORWARD)
	{
		log_index_mem_table mem_table = NULL;
		log_index_table_t *table_data = log_index_read_table(logindex_snapshot, tid, &mem_table);
...
log_index_lsn_iter
polar_logindex_create_lsn_iterator(log_index_snapshot logindex_snapshot, XLogRecPtr lsn)
{
	MemoryContext oldcontext = MemoryContextSwitchTo(polar_logindex_memory_context());
	log_index_lsn_iter iter = palloc0(sizeof(log_index_lsn_iter_t));
...
static log_index_table_t *
log_index_get_table_data(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter, log_index_mem_table *r_table)
{
	uint32		mid;
	log_index_mem_table table;
	log_index_meta_t meta;
...
static bool
log_index_lsn_iterator_update(log_index_lsn_iter iter, log_index_mem_table table, log_index_table_t *table_data)
{
...
log_index_lsn_info
polar_logindex_lsn_iterator_next(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter)
{
	log_index_table_t *table_data;
	log_index_mem_table mem_table;
	...
	POLAR_ASSERT_PANIC(!lsn_info || lsn_info->lsn >= iter->start_lsn);
	return lsn_info;
}

void
polar_logindex_release_lsn_iterator(log_index_lsn_iter iter)
{
	pfree(iter);
}

XLogRecPtr
polar_logindex_page_iterator_max_lsn(log_index_page_iter iter)
{
	return iter->max_lsn;
}

XLogRecPtr
polar_logindex_page_iterator_min_lsn(log_index_page_iter iter)
{
	return iter->min_lsn;
}

BufferTag *
polar_logindex_page_iterator_buf_tag(log_index_page_iter iter)
{
	return &iter->tag;
}
