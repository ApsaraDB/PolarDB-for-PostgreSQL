...
void
polar_record_rel_size(polar_rel_size_cache_t cache, XLogRecPtr lsn, RelFileLocator *rlocator, ForkNumber fork, BlockNumber rel_size)
{
	polar_rel_size_table_t *active_table;
	int			pos;
	...
	if (unlikely(polar_enable_debug))
		ereport(LOG,
				errmsg("polar_record_rel_size lsn=%X/%X,spc=%d,db=%d,rel=%d,fork=%d,rel_size=%d",
					   LSN_FORMAT_ARGS(lsn), rlocator->dbOid, rlocator->relNumber, rlocator->spcOid, fork, rel_size));
}

void
polar_record_rel_size_with_lock(polar_rel_size_cache_t cache, XLogRecPtr lsn, RelFileLocator *rlocator, ForkNumber fork, BlockNumber rel_size)
{
	LWLockAcquire(POLAR_REL_SIZE_CACHE_LOCK(cache), LW_EXCLUSIVE);

	polar_record_rel_size(cache, lsn, rlocator, fork, rel_size);
						   ...
						   XLogRecPtr lsn, BufferTag *tag, polar_rel_search_result_t *result)
{
	...
	while (pos > 0)
	{
		...
		if (tag->spcOid == rel.rlocator.spcOid &&
			tag->dbOid == rel.rlocator.dbOid &&
			tag->relNumber == rel.rlocator.relNumber &&
			tag->forkNum == rel.fork &&
			tag->blockNum >= rel.rel_size)
		{
			result->lsn = rel.lsn;
						   ...
						   BufferTag *tag, polar_rel_search_result_t *result)
{
	...
	if (result->state == POLAR_DB_NOTFOUND)
		polar_search_db_state(table, lsn, tag->spcOid, tag->dbOid, result);
