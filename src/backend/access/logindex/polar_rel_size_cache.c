...
polar_save_rel_size_table(polar_rel_size_cache_t cache, polar_rel_size_table_t *table)
{
	...
	fd = PathNameOpenFile(path, O_CREAT | O_RDWR | PG_BINARY, false);

	if (fd < 0)
		elog(FATAL, "Could not open file \"%s\" for write, %m", path);

	ret = FileWrite(fd, (char *)table, POLAR_REL_CACHE_TABLE_SIZE, WAIT_EVENT_REL_SIZE_CACHE_WRITE);
...
polar_get_rel_size_table(polar_rel_size_cache_t cache, uint64 tid)
{
	...
	ret = FileRead(fd, (char *)table, POLAR_REL_CACHE_TABLE_SIZE, WAIT_EVENT_REL_SIZE_CACHE_READ);
