...
static bool
_nblocks_less_than(BlockNumber newval, BlockNumber cached)
{
...
polar_record_truncate_heap_info(SMgrRelation reln, polar_logindex_redo_ctl_t instance, XLogRecPtr lsn, xl_smgr_truncate *xlrec)
{
	...
	if (instance->rel_size_cache)
		polar_record_rel_size_with_lock(instance->rel_size_cache, lsn, &xlrec->rlocator, fork, xlrec->blkno);

	/*
	 * Get rid of any buffers for the about-to-be-deleted blocks. bufmgr will
	 * just drop them without bothering to write the contents.
	 */
	DropRelationBuffers(reln, &fork, 1, &(xlrec->blkno));

	/*
	 * POLAR RSC: update nblocks after dropping buffers.
	 */
	if (POLAR_RSC_REPLICA_ENABLED())
		polar_rsc_update_if_exists(&xlrec->rlocator, fork, xlrec->blkno,
								   _nblocks_less_than);
	/* POLAR end */
}
...
polar_record_truncate_vm_info(SMgrRelation reln, polar_logindex_redo_ctl_t instance, XLogRecPtr lsn, xl_smgr_truncate *xlrec)
{
	...
	if (instance->rel_size_cache)
		polar_record_rel_size_with_lock(instance->rel_size_cache, lsn, &xlrec->rlocator, fork, new_blocks);

	/*
	 * Get rid of any buffers for the about-to-be-deleted blocks. bufmgr will
	 * just drop them without bothering to write the contents.
	 */
	DropRelationBuffers(reln, &fork, 1, &new_blocks);

	/*
	 * POLAR RSC: update nblocks after dropping buffers.
	 */
	if (POLAR_RSC_REPLICA_ENABLED())
		polar_rsc_update_if_exists(&xlrec->rlocator, fork, new_blocks,
								   _nblocks_less_than);
	/* POLAR end */
}
