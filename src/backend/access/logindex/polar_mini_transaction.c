...
{
	info->tag.spcOid = tag->spcOid;
	info->tag.dbOid = tag->dbOid;
	BufTagSetRelForkDetails(&info->tag, tag->relNumber, tag->forkNum);
	info->tag.blockNum = tag->blockNum;
