...
polar_btree_xlog_delete_parse(polar_logindex_redo_ctl_t instance, XLogReaderState *record)
{
	...
	if (InHotStandby && POLAR_LOGINDEX_ENABLE_RESOLVE_CONFLICT())
	{
		...
		ResolveRecoveryConflictWithSnapshot(xlrec->snapshotConflictHorizon, xlrec->isCatalogRel, rlocator);
	}
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			if (!posting)
			{
								...
								false, false) == InvalidOffsetNumber)
				{
					...
					elog(PANIC, "%s: failed to add item", __func__);
				}
			}
			else
			{
...
polar_btree_xlog_dedup(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	if (BufferTagsEqual(tag, &dedup_tag))
	{
		char	   *ptr = XLogRecGetBlockData(record, 0, NULL);
		Page		page;
		BTPageOpaque opaque;
