...
int			polar_wait_xlog_meta_timeout = 0;
static bool xlog_queue_catch_up = false;

static bool polar_xlog_queue_decode(XLogReaderState *state, DecodedXLogRecord *decoded, XLogRecord *record,
...
check_xlog_queue_size(Size xlog_queue_size)
{
	...
	double		free_ratio = 1 - POLAR_XLOG_QUEUE_MAX_DATA_KEEP_RATIO;

	POLAR_ASSERT_PANIC(XLOGbuffers > 0);
	xlog_buffer_size = (Size) XLOGbuffers * XLOG_BLCKSZ;
	xlog_queue_size -= offsetof(polar_ringbuf_data_t, data);
...
polar_xlog_push_heap2_data(polar_ringbuf_t queue, size_t rbuf_pos, int offset, uint8 info)
{
	...
	switch (info & XLOG_HEAP_OPMASK)
	{
		...
		case XLOG_HEAP2_PRUNE_VACUUM_SCAN:
		case XLOG_HEAP2_PRUNE_VACUUM_CLEANUP:
			/* the length of main data is variable. */
			len = polar_push_main_data(queue, rbuf_pos, offset);
			break;
						   ...
						   XLogRecPtr end_lsn, uint32 xlog_len)
{
	...
	if (unlikely(polar_enable_debug))
		elog(LOG, "%s lsn=%X/%X", __func__, LSN_FORMAT_ARGS(end_lsn - xlog_len));
...
polar_standby_xlog_send_queue_push(polar_ringbuf_t queue, XLogReaderState *xlogreader)
{
	...
	polar_xlog_send_queue_keep_data(queue, POLAR_RINGBUF_MIN_PREAD);
	...
	{
		if (unlikely(polar_enable_debug))
			ereport(LOG, (errmsg("%s push %X/%X to queue", __func__, LSN_FORMAT_ARGS(EndPos))));
	}
...
polar_xlog_send_queue_next_lsn(polar_ringbuf_ref_t *ref, size_t *len)
{
	...
	if (polar_ringbuf_next_pkt_type(ref, &pktlen) != POLAR_RINGBUF_PKT_INVALID_TYPE)
	{
								   ...
								   uint8 *data, size_t size, XLogRecPtr *max_lsn)
{
	...
	XLogRecPtr	lsn = InvalidXLogRecPtr;
	static XLogRecPtr flush_lsn = InvalidXLogRecPtr;
	...
	while (polar_ringbuf_next_pkt_type(ref, &pktlen) != POLAR_RINGBUF_PKT_INVALID_TYPE)
	{
		...
		if (unlikely(polar_enable_debug))
		{
			...
			elog(LOG, "%s lsn=%X/%X", __func__, LSN_FORMAT_ARGS(lsn - xlog_len));
		}
...
polar_xlog_send_queue_check(polar_ringbuf_ref_t *ref, XLogRecPtr start_point)
{
	 ...
	 */
	while (!ref->strong && !polar_ringbuf_set_ref_strong_property(ref, true))
	...
	do
	{
		...
		if ((type = polar_ringbuf_next_pkt_type(ref, &pktlen)) == POLAR_RINGBUF_PKT_INVALID_TYPE)
			break;
	...
	if (!ref->strong)
		polar_ringbuf_set_ref_strong_property(ref, false);
...
polar_xlog_queue_remove_outdate(polar_ringbuf_ref_t *ref, XLogReaderState *state)
{
	...
	while (polar_ringbuf_next_pkt_type(ref, &pktlen) == POLAR_RINGBUF_PKT_WAL_META);
...
polar_xlog_queue_pop_record(polar_ringbuf_ref_t *ref, uint32 pktlen, XLogReaderState *state, bool decode_payload)
{
	...
	do
	{
		...
		/*
		 * The read_rec_ptr is exactly the first byte of wal record, but the
	...
	while (decode_record == NULL &&
		   polar_ringbuf_next_pkt_type(ref, &pktlen) == POLAR_RINGBUF_PKT_WAL_META);
	return decode_record;
}

static void
polar_startup_wait_flush_upto(XLogRecPtr end_lsn)
{
	static XLogRecPtr flush_upto = InvalidXLogRecPtr;

	while (POLAR_STARTUP_IN_STATUS(polar_logindex_redo_instance, POLAR_STARTUP_IN_CASCADE_REPLICA))
	{
		if (flush_upto < end_lsn)
			flush_upto = GetWalRcvFlushRecPtr(NULL, NULL);
		...
		if (!WalRcvStreaming())
			polar_keep_wal_receiver_up(end_lsn);

		polar_startup_wait_latch(polar_wait_xlog_meta_timeout, WAIT_EVENT_RECOVERY_WAL_STREAM);
	}
}
...
polar_xlog_queue_ref_pop_ahead(polar_ringbuf_ref_t *ref, XLogReaderState *state, bool decode_payload)
{
	...
	pkt_type = polar_ringbuf_next_pkt_type(ref, &pktlen);
		...
		case POLAR_RINGBUF_PKT_WAL_META:
			{
				if (unlikely(!xlog_queue_catch_up))
				{
					...
					pg_atomic_fetch_add_u64(&(ref->rbuf->prs.recv_phys_io_cnt), 1);

					if (decode_record == NULL)
						break;
					 ...
					 * file
					 */
					if (decode_record->lsn == catch_up_ptr)
					{
						polar_ringbuf_update_ref(ref);
					...
					else if (decode_record->lsn > catch_up_ptr)
						need_wait = true;
				}
				else
	...
	if (decode_record && need_wait)
		polar_startup_wait_flush_upto(state->EndRecPtr);

	if (unlikely(polar_enable_debug))
		elog(LOG, "%s lsn=%X/%X", __func__, LSN_FORMAT_ARGS(state->DecodeRecPtr));
...
#define COPY_CONTENT(_dst, _size) \
	...
	if (polar_xlog_remove_payload(record))
	{
		...
		while (offset < record_meta_len)
		{
			...
			{
				COPY_CONTENT(&blk->rlocator, sizeof(RelFileLocator));
...
DecodedXLogRecord *
polar_xlog_send_queue_pop(polar_ringbuf_t queue, polar_ringbuf_ref_t *ref, XLogReaderState *state)
{
	uint32		pktlen;
	uint8		pkt_type;
	char	   *errormsg = NULL;
...
bool
polar_xlog_send_queue_keep_data(polar_ringbuf_t queue, uint64 new_pread)
{
	...
	return polar_ringbuf_ref_keep_data(&polar_data_ref, polar_xlog_queue_data_keep_ratio, new_pread);
}
...
polar_xlog_recv_queue_push_storage_begin(polar_ringbuf_t queue, polar_interrupt_callback callback)
{
	...
	while (polar_ringbuf_free_size(queue) < POLAR_RINGBUF_PKTHDRSIZE)
		polar_ringbuf_free_up(queue, POLAR_RINGBUF_PWRITE(queue), POLAR_RINGBUF_PKTHDRSIZE, callback);
...
polar_xlog_recv_queue_push(polar_ringbuf_t queue, char *buf, size_t len, polar_interrupt_callback callback)
{
	...
	do
	{
		...
		while (polar_ringbuf_free_size(queue) < POLAR_RINGBUF_PKT_SIZE(pktlen))
			polar_ringbuf_free_up(queue, POLAR_RINGBUF_PWRITE(queue), POLAR_RINGBUF_PKT_SIZE(pktlen), callback);
...
polar_xlog_recv_queue_pop_ahead(polar_ringbuf_t queue, XLogReaderState *state)
{
		   ...
		   (state->DecodeRecPtr < state->currRecPtr))
	{
		...
		if (decoded == NULL)
		{
			...
			while (AmStartupProcess() && !IsPromoteSignaled() &&
				   polar_injection_point_find("test_wait_for_promote_signal"))
				pg_usleep(1000);
#endif
			...
			/* double check xlog recv queue before WaitLatch */
			if (polar_ringbuf_avail_size(&ref) <= 0)
				polar_startup_wait_latch(polar_wait_xlog_meta_timeout, WAIT_EVENT_RECOVERY_WAL_STREAM);
		}
	}
...
polar_max_sendable_lsn(void)
{
	if (!RecoveryInProgress())
		...
		return Max(polar_get_xlog_replay_recptr_nolock(), polar_get_sync_ddl_lsn());
}
