...
polar_heap_clear_vm(XLogReaderState *record, RelFileLocator rlocator,
...
polar_heap_prune_parse(polar_logindex_redo_ctl_t instance, XLogReaderState *record)
{
	...
	if ((xlrec->flags & XLHP_HAS_CONFLICT_HORIZON) != 0 &&
		POLAR_LOGINDEX_ENABLE_RESOLVE_CONFLICT())
	{
		TransactionId snapshot_conflict_horizon;

		/* memcpy() because snapshot_conflict_horizon is stored unaligned */
		memcpy(&snapshot_conflict_horizon, XLogRecGetData(record) + SizeOfHeapPrune, sizeof(TransactionId));
	...
	if (xlrec->flags & XLHP_CLEANUP_LOCK)
...
polar_heap_visible_parse(polar_logindex_redo_ctl_t instance, XLogReaderState *record)
{
	 ...
	 */
	if (InHotStandby && POLAR_LOGINDEX_ENABLE_RESOLVE_CONFLICT())
	{
		xl_heap_visible *xlrec = (xl_heap_visible *) XLogRecGetData(record);
		RelFileLocator rlocator;

		XLogRecGetBlockTag(record, 1, &rlocator, NULL, NULL);
...
/*
 * Handles HEAP2_PRUNE_* record type
 */
static XLogRedoAction
polar_heap_xlog_prune_freeze(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
...
polar_heap_xlog_visible(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	if (BufferTagsEqual(tag, &tag0))
	{
		ReadBufferMode mode = BufferIsValid(*buffer) ? RBM_NORMAL_VALID_NO_LOG : RBM_ZERO_ON_ERROR;
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			polar_visibilitymap_set(tag1.blockNum, *buffer, vmbits);
...
polar_heap_xlog_multi_insert(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	if (BufferTagsEqual(&vm_tag, tag))
	{
		...
		if (xlrec->flags & XLH_INSERT_ALL_VISIBLE_CLEARED)
			action = polar_heap_clear_vm(record, POLAR_REL_FILE_LOCATOR(&heap_tag),
										 heap_tag.blockNum, buffer,
										 VISIBILITYMAP_VALID_BITS);
		return action;
	}

	if (BufferTagsEqual(&heap_tag, tag))
	{
		if (isinit)
		{
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			for (i = 0; i < xlrec->ntuples; i++)
			{
				...
				{
					POLAR_LOG_REDO_INFO(page, record);
					elog(PANIC, "invalid max offset number, page_max_off=%d, offnum=%d",
...
polar_heap_xlog_lock_updated(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	if (BufferTagsEqual(tag, &heap_tag))
	{
		action = POLAR_READ_BUFFER_FOR_REDO(record, 0, buffer);
...
polar_heap_xlog_update(XLogReaderState *record, BufferTag *tag, Buffer *buffer, bool hotupdate)
{
	...
	if (oldblk != newblk)
	{
		if (xlrec->flags & XLH_UPDATE_OLD_ALL_VISIBLE_CLEARED)
		{
			POLAR_GET_VM_TAG(&tag3, &tag1);
			old_vm_tag = &tag3;
		}

		if (xlrec->flags & XLH_UPDATE_NEW_ALL_VISIBLE_CLEARED)
		{
			POLAR_GET_VM_TAG(&tag2, &tag0);
			new_vm_tag = &tag2;
		}
	}
	else
	...
	if (old_vm_tag != NULL && BufferTagsEqual(old_vm_tag, tag))
	{
		/*
		 * The visibility map may need to be fixed even if the heap page is
		 * already up-to-date.
		 */
		if ((xlrec->flags & XLH_UPDATE_OLD_ALL_VISIBLE_CLEARED))
			action = polar_heap_clear_vm(record, POLAR_REL_FILE_LOCATOR(old_tag),
										 old_tag->blockNum,
										 buffer, VISIBILITYMAP_VALID_BITS);
	}

	if (new_vm_tag != NULL && BufferTagsEqual(new_vm_tag, tag))
	{
		/*
		 * The visibility map may need to be fixed even if the heap page is
		 * already up-to-date.
		 */
		if (xlrec->flags & XLH_UPDATE_NEW_ALL_VISIBLE_CLEARED)
			action = polar_heap_clear_vm(record, POLAR_REL_FILE_LOCATOR(new_tag),
										 new_tag->blockNum,
										 buffer, VISIBILITYMAP_VALID_BITS);
	}

	if (BufferTagsEqual(tag, old_tag))
	{
		/*
	...
	if (BufferTagsEqual(tag, new_tag))
	{
		/*
...
polar_heap2_idx_redo(polar_logindex_redo_ctl_t instance, XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	switch (info & XLOG_HEAP_OPMASK)
	{
		...
		case XLOG_HEAP2_PRUNE_VACUUM_SCAN:
		case XLOG_HEAP2_PRUNE_VACUUM_CLEANUP:
			return polar_heap_xlog_prune_freeze(record, tag, buffer);
