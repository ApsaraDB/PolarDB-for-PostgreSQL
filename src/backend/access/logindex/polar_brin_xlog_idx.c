...
polar_brin_xlog_createidx(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	if (!BufferTagsEqual(&meta_tag, tag))
		return BLK_NOTFOUND;
							  ...
							  xl_brin_insert *xlrec, BufferTag *tag, Buffer *buffer)
{
	...
	if (BufferTagsEqual(tag, &index_tag))
	{
		/*
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			POLAR_ASSERT_PANIC(tuple->bt_blkno == xlrec->heapBlk);
			...
			if (PageGetMaxOffsetNumber(page) + 1 < offnum)
			{
				...
				elog(PANIC, "polar_brin_xlog_insert_update: invalid max offset number, page_max_off=%d, offnum=%d",
	...
	if (BufferTagsEqual(tag, &revmap_tag))
	{
		/* update the revmap */
...
polar_brin_xlog_samepage_update(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	if (!BufferTagsEqual(&page_tag, tag))
		return action;
...
polar_brin_xlog_revmap_extend(XLogReaderState *record, BufferTag *tag, Buffer *buffer)
{
	...
	POLAR_ASSERT_PANIC(xlrec->targetBlk == targetBlk);
	...
	if (BufferTagsEqual(tag, &meta_tag))
	{
		...
		if (action == BLK_NEEDS_REDO)
		{
			...
			POLAR_ASSERT_PANIC(metadata->lastRevmapPage == xlrec->targetBlk - 1);
