...
log_index_table_flushable_hook_type polar_logindex_table_flushable_hook = NULL;

int			polar_logindex_table_batch_size = 100;
...
static void log_index_insert_new_item(log_index_lsn_info lsn_info, log_index_mem_table table, uint32 key, log_seg_id_t new_item_id);
static void log_index_insert_new_seg(log_index_mem_table table, log_seg_id_t head, log_seg_id_t seg_id, log_index_lsn_info lsn_info);

bool
polar_logindex_check_state(log_index_snapshot logindex_snapshot, uint32 state)
{
...
XLogRecPtr
log_index_item_max_lsn(log_index_table_t *table, log_item_head_t *item)
{
	log_item_seg_t *seg;
...
log_index_mem_tbl_shmem_size(uint64 logindex_mem_tbl_size)
{
	...
	size = add_size(size, mul_size(sizeof(log_index_mem_table_t), logindex_mem_tbl_size));
...
static bool
log_index_table_saved_before_promote(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
...
bool
polar_logindex_table_flushable(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
	if (!RecoveryInProgress())
		return GetFlushRecPtr(NULL) > table->data.max_lsn;
	else
		return polar_get_xlog_replay_recptr_nolock() > table->data.max_lsn;

}

static bool
log_index_table_flushable(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
	if (polar_logindex_table_flushable_hook)
		return polar_logindex_table_flushable_hook(logindex_snapshot, table);
	else
		return polar_logindex_table_flushable(logindex_snapshot, table);
}

static bool
log_index_flush_table(log_index_snapshot logindex_snapshot, XLogRecPtr checkpoint_lsn, bool flush_active)
{
	log_index_mem_table table;
	LWLock	   *lock;
	...
	do
	{
			 ...
			 !LOG_INDEX_MEM_TBL_IS_NEW(table)))
		{
			...
			if ((LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_INACTIVE ||
				 LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_ACTIVE)
				&& log_index_table_flushable(logindex_snapshot, table))
			{
				/*
...
static bool
log_index_primary_bg_write(log_index_snapshot logindex_snapshot)
{
	return log_index_flush_table(logindex_snapshot, InvalidXLogRecPtr, false);
}

void
polar_logindex_flush_table(log_index_snapshot logindex_snapshot, XLogRecPtr checkpoint_lsn, bool flush_active)
{
	log_index_flush_table(logindex_snapshot, checkpoint_lsn, flush_active);
}

void
polar_logindex_invalid_bloom_cache(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid)
{
...
static void
polar_logindex_truncate_local_cache(log_index_snapshot logindex_snapshot, uint64 segno)
{
...
static void
log_index_update_replica_table_state(log_index_snapshot logindex_snapshot, log_idx_table_id_t max_saved_tid)
{
	uint32		mid,
				start_mid;
	log_index_mem_table table;
...
static bool
log_index_replica_bg_write(log_index_snapshot logindex_snapshot)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
...
bool
polar_logindex_bg_write(log_index_snapshot logindex_snapshot)
{
...
void
polar_logindex_init_lwlock_array(log_index_snapshot logindex_snapshot, int offset, int size, int tranche_id)
{
...
static void
polar_logindex_snapshot_remove_data(log_index_snapshot logindex_snapshot)
{
...
static void
logindex_snapshot_init_promoted_info(log_index_snapshot logindex_snapshot)
{
...
static XLogRecPtr
polar_logindex_snapshot_base_init(log_index_snapshot logindex_snapshot, XLogRecPtr checkpoint_lsn,
								  TimeLineID replay_tli)
{
	...
	MemSet(logindex_snapshot->mem_table, 0,
		   sizeof(log_index_mem_table_t) * logindex_snapshot->mem_tbl_size);
	 ...
	 */
	pg_atomic_write_u64(&logindex_snapshot->bloom_ctl.shared->latest_page_number, UINT64_MAX);
...
XLogRecPtr
polar_logindex_snapshot_init(log_index_snapshot logindex_snapshot, XLogRecPtr checkpoint_lsn,
...
static bool
log_index_page_precedes(int64 page1, int64 page2)
{
	return page1 < page2;
}

log_index_snapshot
polar_logindex_snapshot_shmem_init(const char *name, uint64 logindex_mem_tbl_size, int bloom_blocks,
...
#define LOGINDEX_BLOOM_SUFFIX "_bloom"
	...
	log_index_snapshot logindex_snapshot = NULL;
	bool		found_snapshot;
	bool		found_locks;
	Size		size;
	...
	StaticAssertStmt(LOG_INDEX_FILE_TBL_BLOOM_SIZE > sizeof(log_index_table_bloom_t),
					 "LOG_INDEX_FILE_TBL_BLOOM_SIZE is not enough for log_index_table_bloom_t");

	snprintf(item_name, POLAR_MAX_SHMEM_NAME, "%s%s", name, LOGINDEX_SNAPSHOT_SUFFIX);

	logindex_snapshot = (log_index_snapshot)
	...
	if (!IsUnderPostmaster)
	{
		...
		init_lwlock_func(logindex_snapshot);

		logindex_snapshot->max_allocated_seg_no = 0;
		pg_atomic_init_u64(&logindex_snapshot->flush_active_table_lsn, InvalidXLogRecPtr);
	...
	SimpleLruInit(&logindex_snapshot->bloom_ctl, item_name,
				  bloom_blocks, 0, name,
				  tranche_buffer_id, tranche_bank_id,
				  SYNC_HANDLER_NONE, true, true);
...
bool
log_index_get_meta(log_index_snapshot logindex_snapshot, log_index_meta_t *meta)
{
	int			r;
...
XLogRecPtr
polar_logindex_start_lsn(log_index_snapshot logindex_snapshot)
{
	XLogRecPtr	start_lsn = InvalidXLogRecPtr;
...
static void
log_index_write_meta(log_index_snapshot logindex_snapshot, log_index_meta_t *meta, bool update)
{
	File		fd;
	char		meta_path[MAXPGPATH];
 ...
 */
void
log_index_force_save_table(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
	...
	{
		if (log_index_table_flushable(logindex_snapshot, table))
		{
			bool		succeed = false;
...
static void
log_index_wait_active(log_index_snapshot logindex_snapshot, log_index_mem_table table, XLogRecPtr lsn)
{
	LWLock	   *lock;
	bool		end = false;
	...
	for (;;)
	{
		...
		if (LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_FLUSHED)
			MemSet(table, 0, sizeof(log_index_mem_table_t));
...
log_index_mem_tbl_exists_page(BufferTag *tag,
							  log_index_table_t *table, uint32 key)
{
...
static bool
log_index_mem_seg_full(log_index_mem_table table, log_seg_id_t head)
{
	log_item_head_t *item;
	log_item_seg_t *seg;
...
static void
log_index_insert_new_item(log_index_lsn_info lsn_info,
						  log_index_mem_table table, uint32 key,
...
static void
log_index_insert_new_seg(log_index_mem_table table, log_seg_id_t head,
						 log_seg_id_t seg_id, log_index_lsn_info lsn_info)
{
...
static uint8
log_index_append_lsn(log_index_mem_table table, log_seg_id_t head, log_index_lsn_info lsn_info)
{
	log_item_head_t *item;
	log_item_seg_t *seg;
	uint8		idx;
...
static log_seg_id_t
log_index_next_free_seg(log_index_snapshot logindex_snapshot, XLogRecPtr lsn, log_index_mem_table *active_table)
{
	log_seg_id_t dst = LOG_INDEX_TBL_INVALID_SEG;
	log_index_mem_table active;
	int			next_mem_id = -1;
...
static log_index_table_bloom_t *
log_index_get_bloom_lru(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid, int *slot)
{
	int			pageno = LOG_INDEX_TBL_BLOOM_PAGE_NO(tid);
	...
	return (log_index_table_bloom_t *) (shared->page_buffer[*slot] + offset);
}


log_index_table_bloom_t *
log_index_get_tbl_bloom(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid)
{
	int			pageno = LOG_INDEX_TBL_BLOOM_PAGE_NO(tid);
	int			offset = LOG_INDEX_TBL_BLOOM_PAGE_OFFSET(tid);
	SlruShared	shared = logindex_snapshot->bloom_ctl.shared;
...
static void
log_index_calc_bloom(log_index_table_t *table_data, log_index_table_bloom_t *bloom)
{
	bloom_filter *filter;
	int			i;
...
static bool
log_index_save_table(log_index_snapshot logindex_snapshot, log_index_table_t *table, File fd, log_index_table_bloom_t *bloom)
{
	int			ret = -1;
	...
	table->crc = 0;
	table->crc = log_index_calc_crc((unsigned char *) table, sizeof(log_index_table_t));
...
static void
log_index_get_bloom_data(log_index_table_t *table_data, log_idx_table_id_t tid, log_index_table_bloom_t *bloom)
{
	...
	bloom->buf_size = LOG_INDEX_FILE_TBL_BLOOM_SIZE -
		offsetof(log_index_table_bloom_t, bloom_bytes);
...
static void
log_index_save_bloom(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid, log_index_table_bloom_t *bloom)
{
	int			slot;
	log_index_table_bloom_t *lru_bloom;
	SlruShared	shared = logindex_snapshot->bloom_ctl.shared;
	int			pageno = LOG_INDEX_TBL_BLOOM_PAGE_NO(tid);
	LWLock	   *lock;

	lock = SimpleLruGetBankLock(&logindex_snapshot->bloom_ctl, pageno);
	LWLockAcquire(lock, LW_EXCLUSIVE);
...
static int
log_index_open_table_file(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid, bool readonly, int elevel)
{
	char		path[MAXPGPATH];
	File		fd;
	int			flag;
...
bool
log_index_read_table_data(log_index_snapshot logindex_snapshot, log_index_table_t *table, log_idx_table_id_t tid, int elevel)
{
	File		fd;
	pg_crc32	crc;
	int			bytes;
	...
	bytes = FileRead(fd, (char *) table, sizeof(log_index_table_t), offset, WAIT_EVENT_LOGINDEX_TBL_READ);

	if (bytes != sizeof(log_index_table_t))
	{
...
static bool
log_index_write_table_data(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
	bool		ret = false;
	File		fd;
	log_index_table_bloom_t *bloom;
	log_idx_table_id_t tid = table->data.idx_table_id;
	log_index_table_t *table_data = NULL;
	...
	{
		table_data = palloc(sizeof(log_index_table_t));
		LWLockAcquire(LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK, LW_EXCLUSIVE);
		memcpy(table_data, &table->data, sizeof(log_index_table_t));
		LWLockRelease(LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK);
	}
	else
		table_data = &table->data;

	bloom = (log_index_table_bloom_t *) palloc0(LOG_INDEX_FILE_TBL_BLOOM_SIZE);

	log_index_get_bloom_data(table_data, tid, bloom);
...
static void
log_index_report_io_error(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid)
{
	uint64		segno = LOG_INDEX_FILE_TABLE_SEGMENT_NO(tid);
...
static bool
log_index_read_seg_file(log_index_snapshot logindex_snapshot, log_table_cache_t *cache, uint64 segno)
{
	int			bytes;
	int			i;
	...
	table_data = (log_index_table_t *) cache->data;
	...
	i = (bytes / sizeof(log_index_table_t)) - 1;
	...
	while (i >= 0)
	{
		table_data = (log_index_table_t *) (cache->data + sizeof(log_index_table_t) * i);
 ...
 */
log_index_table_t *
log_index_read_table(log_index_snapshot logindex_snapshot, log_idx_table_id_t tid, log_index_mem_table *mem_table)
{
	static log_table_cache_t table_cache;
	int			mid = (tid - 1) % logindex_snapshot->mem_tbl_size;
	log_index_mem_table table = LOG_INDEX_MEM_TBL(mid);
	LWLock	   *table_lock = LOG_INDEX_MEM_TBL_LOCK(table);
	log_index_table_t *data;
	...
	*mem_table = NULL;
	...
	{
		if (LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_FLUSHED && tid != LOG_INDEX_MEM_TBL_TID(table))
			memcpy(&table->data, data, sizeof(log_index_table_t));
...
bool
log_index_write_table(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
	bool		succeed = false;
	uint64		segment_no;
	...
	if (polar_injection_point_find("test_skip_write_logindex"))
		return true;
#endif
	...
	if (tid == meta->max_idx_table_id + 1)
	{
		...
		{
			/* Flush active memtable, update corresponding lsn */
			pg_atomic_write_u64(&logindex_snapshot->flush_active_table_lsn, table->data.max_lsn);
			succeed = true;
		}
		else
		{
...
static void
log_index_update_min_segment(log_index_snapshot logindex_snapshot)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	log_index_file_segment_t *min_seg = &meta->min_segment_info;
	...
	{
		log_index_table_t *table = palloc(sizeof(log_index_table_t));
...
static bool
log_index_rename_segment_file(log_index_snapshot logindex_snapshot, char *old_file)
{
...
static void
log_index_truncate_tbl(log_index_snapshot logindex_snapshot, uint64 min_segment_no, uint64 max_segment_no)
{
	char		path[MAXPGPATH];
...
static void
log_index_truncate_bloom(log_index_snapshot logindex_snapshot, log_idx_table_id_t max_unused_tid)
{
	uint64		bloom_page;
...
static void
log_index_truncate(log_index_snapshot logindex_snapshot, XLogRecPtr lsn)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	log_index_file_segment_t *min_seg = &meta->min_segment_info;
...
void
polar_logindex_truncate(log_index_snapshot logindex_snapshot, XLogRecPtr lsn)
{
...
static bool
log_index_exists_in_saved_table(log_index_snapshot logindex_snapshot, log_index_lsn_info lsn_info)
{
	uint32		mid;
	uint32		i;
	BufferTag	tag;
	log_index_lsn_info_t saved_lsn;
	log_index_mem_table table;
	...
	for (i = table->data.last_order; i > 0; i--)
	{
		...
		if (BufferTagsEqual(lsn_info->tag, saved_lsn.tag))
			return true;
	}
...
void
log_index_insert_lsn(log_index_snapshot logindex_snapshot, log_index_lsn_info lsn_info, uint32 key)
{
	log_index_mem_table active = NULL;
	bool		new_item;
	...
	{
		log_seg_id_t dst;
		log_index_mem_table old_active = active;
...
void
polar_logindex_add_lsn(log_index_snapshot logindex_snapshot, BufferTag *tag, XLogRecPtr prev, XLogRecPtr lsn)
{
	uint32		key = LOG_INDEX_MEM_TBL_HASH_PAGE(tag);
	log_index_meta_t *meta = NULL;
	log_index_lsn_info_t lsn_info;
	...
	{
		log_index_mem_table active = LOG_INDEX_MEM_TBL_ACTIVE();
...
XLogRecPtr
polar_get_logindex_snapshot_max_lsn(log_index_snapshot logindex_snapshot)
{
 ...
 */
void
polar_load_logindex_snapshot_from_storage(log_index_snapshot logindex_snapshot, XLogRecPtr start_lsn)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	log_idx_table_id_t new_max_idx_table_id = LOG_INDEX_TABLE_INVALID_ID;
	log_index_mem_table active = LOG_INDEX_MEM_TBL_ACTIVE();
	log_index_mem_table mem_tbl = NULL;
	static log_index_table_t table;
	int			mid = 0;
	...
	if (new_max_idx_table_id >= min_tid)
	{
		...
		while (tid >= min_tid)
		{
			...
			memcpy(&mem_tbl->data, &table, sizeof(log_index_table_t));
		...
		table.idx_table_id == LOG_INDEX_MEM_TBL_TID(active))
	{
		...
		if (table.max_lsn > active->data.max_lsn)
		{
			...
			memcpy(&active->data, &table, sizeof(log_index_table_t));
...
uint64
polar_logindex_mem_tbl_size(log_index_snapshot logindex_snapshot)
{
	return logindex_snapshot != NULL ? logindex_snapshot->mem_tbl_size : 0;
}

uint64
polar_logindex_used_mem_tbl_size(log_index_snapshot logindex_snapshot)
{
...
{
	return (mem_size * 1024L * 1024L) / (sizeof(log_index_mem_table_t) + sizeof(LWLockPadded));
}

const char *
polar_get_logindex_snapshot_dir(log_index_snapshot logindex_snapshot)
{
	return logindex_snapshot->dir;
}

XLogRecPtr
polar_get_logindex_snapshot_storage_min_lsn(log_index_snapshot logindex_snapshot)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	log_index_file_segment_t *min_seg = &meta->min_segment_info;
	log_index_table_t *table = NULL;
	XLogRecPtr	min_lsn = InvalidXLogRecPtr;

	table = palloc(sizeof(log_index_table_t));
...
void
polar_replica_promote_logindex(log_index_snapshot logindex_snapshot, bool missing_ok)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	SlruShared	shared = logindex_snapshot->bloom_ctl.shared;
	log_index_promoted_info_t *promoted_info = &logindex_snapshot->promoted_info;
...
XLogRecPtr
polar_logindex_check_valid_start_lsn(log_index_snapshot logindex_snapshot)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	XLogRecPtr	start_lsn = InvalidXLogRecPtr;
...
void
polar_logindex_set_start_lsn(log_index_snapshot logindex_snapshot, XLogRecPtr start_lsn)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
...
void
polar_logindex_set_bgworker_latch(log_index_snapshot logindex_snapshot, struct Latch *latch)
{
...
void
polar_logindex_update_promoted_info(log_index_snapshot logindex_snapshot, XLogRecPtr last_replayed_lsn)
{
	log_index_promoted_info_t *info = &logindex_snapshot->promoted_info;
	...
	ereport(LOG, errmsg("update logindex snapshot %s's promoted info as"
						" max saved lsn %X/%X at saved tid %ld,"
						" max inserted lsn %X/%X at tid %ld for old primary",
						logindex_snapshot->dir,
						LSN_FORMAT_ARGS(info->old_primary_saved_max_lsn),
						info->old_primary_saved_max_tid,
						LSN_FORMAT_ARGS(info->old_primary_max_inserted_lsn),
						info->old_primary_max_tid));
}

static void
polar_logindex_local_cache_seg2str(char *seg_str, uint64 segno, bool long_segment_names)
{
	LOG_INDEX_LOCAL_CACHE_SEG_NAME(seg_str, segno);
}

void
polar_logindex_create_local_cache(log_index_snapshot logindex_snapshot, const char *cache_name, uint32 max_segments)
{
	...
	logindex_snapshot->segment_cache = polar_create_local_cache(cache_name, logindex_snapshot->dir,
																max_segments, LOG_INDEX_TABLE_NUM_PER_FILE * sizeof(log_index_table_t),
																LWTRANCHE_POLAR_LOGINDEX_LOCAL_CACHE, io_permission, false,
																polar_logindex_local_cache_seg2str);
 ...
 */
uint64
polar_get_estimate_fullpage_segment_size(log_index_snapshot logindex_snapshot)
{
	uint64		estimate_total_segment_size = 0;
	log_idx_table_id_t max_tid = 0,
				min_tid = 0;

	/* read without logindex snapshot spinlock */
	min_tid = pg_atomic_read_u64((pg_atomic_uint64 *) &(logindex_snapshot->meta.min_segment_info.min_idx_table_id));
	max_tid = pg_atomic_read_u64((pg_atomic_uint64 *) &(logindex_snapshot->max_idx_table_id));

	if (unlikely(max_tid < min_tid))
		return 0;
