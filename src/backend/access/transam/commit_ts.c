...
int			polar_committs_buffer_slot_size = 256;
...
CommitTsShmemBuffers(void)
{
	...
	{
		/*
		 * POLAR: we can enlarge committs max size because we optimize clog
		 * search time complexity to O(1)
		 */
		int			max_committs_slot_size = Max(4, NBuffers / 256 * 8);
		int			committs_slot_size = Min(polar_committs_buffer_slot_size, max_committs_slot_size);

		ereport(LOG, errmsg("commit timestamp buffer max slot size is %d, and it will set to %d",
...
CommitTsShmemInit(void)
{
	...
	SimpleLruInit(CommitTsCtl, "CommitTs", CommitTsShmemBuffers(), 0,
				  ...
				  SYNC_HANDLER_COMMIT_TS, (polar_enable_shared_storage_mode && !polar_is_replica()));
