...
CreateSharedBackendStatus(void)
{
	...
	/* Create or attach to the shared Proxy Stats buffer */
	...
	for (procNumber = 0; procNumber < NumBackendStatSlots; procNumber++)
	{
		...
		for (;;)
		{
			...
			if (localentry->backendStatus.st_procpid > 0)
			{
				...
				localentry->backendStatus.backendid = procNumber;
				/* POLAR end */
			}
...
polar_proxy_set_sid(int proxy_sid, PgBackendStatus *lbeentry)
{
	...
	polar_proxy_log_beentry_state(lbeentry, __func__);
	if (polar_enable_debug_proxy)
		elog(LOG, "[Proxy] %s: new proxy sid: %d", __func__, proxy_sid);
	...
	for (i = 1; i <= MaxBackends; i++)
	{
		...
		{
			polar_proxy_log_beentry_state(beentry, __func__);
...
polar_proxy_set_cancel_key(int32 cancel_key, PgBackendStatus *lbeentry)
{
	...
	polar_proxy_log_beentry_state(lbeentry, __func__);
	if (polar_enable_debug_proxy)
		elog(LOG, "[Proxy] %s: new cancel key: %d", __func__, cancel_key);
...
polar_proxy_get_pid(int proxy_sid, int32 cancel_key, bool auth_cancel_key)
{
	...
	if (polar_enable_debug_proxy)
		elog(LOG, "[Proxy] %s, proxy_sid: %d, cancel_key: %d, auth_cancel_key: %d",
			 __func__, proxy_sid, cancel_key, auth_cancel_key);
...
polar_proxy_get_sid(int pid, int32 *cancel_key)
{
	...
	polar_proxy_log_beentry_state(my_beentry, __func__);
	if (polar_enable_debug_proxy)
		elog(LOG, "[Proxy] %s, pid: %d", __func__, pid);
