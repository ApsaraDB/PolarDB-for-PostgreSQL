...
polar_mount_and_extra_load_polar_settings(const bool polar_is_show_guc_mode)
{
	...
	if (!polar_enable_shared_storage_mode || polar_setting_file_global_to_local())
	{
		if (polar_enable_shared_storage_mode)
			elog(DEBUG5, "\"%s\" is copied from sharedstorage success, additional load it.", POLAR_SETTING_FILENAME);
		else
			elog(DEBUG5, "\"%s\" is in non-sharedstorage mode, additional load it.", POLAR_SETTING_FILENAME);
...
polar_setting_file_global_to_local(void)
{
	...
	if (polar_make_absolute_file_path(GlobalAllSettingFileName))
		elog(DEBUG5, "Change \"%s\" absolute file path to %s", POLAR_SETTING_FILENAME, GlobalAllSettingFileName);
