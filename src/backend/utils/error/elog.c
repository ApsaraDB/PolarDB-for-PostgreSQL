...
bool		polar_enable_coredump_print = true;
bool		polar_enable_coredump_cleanup = true;
int			polar_save_stack_info_level = FATAL;
char	   *polar_ignore_coredump_functions;
char	   *polar_ignore_coredump_function_list;
int			polar_ignore_coredump_level = FATAL;
...
static void set_stack_entry_location(ErrorData *edata,
									 ...
									 const char *funcname);
static bool matches_backtrace_functions(char *symbol_list, const char *funcname);
...
errfinish(const char *filename, int lineno, const char *funcname)
{
	...
	/*
	 * Collect backtrace, if enabled and we didn't already POLAR: save stack
	 * according to info level
	 */
	if ((!edata->backtrace &&
		 edata->funcname &&
		 backtrace_functions &&
		 matches_backtrace_functions(backtrace_function_list, edata->funcname)) ||
...
polar_program_error_handler(SIGNAL_ARGS)
{
	...
	POLAR_IGNORE_COREDUMP_BACKTRACE(postgres_signal_arg, 4);
