...
GetBackendTypeDesc(BackendType backendType)
{
	...
	switch (backendType)
	{
		...
		case B_WAL_WRITER:
			...
			break;
		case B_BG_LOGINDEX:
			backendDesc = "logindex background worker";
			break;
	}
...
AddToDataDirLockFile(int target_line, const char *str)
{
	...
	fd = polar_open(DIRECTORY_LOCK_FILE, O_RDWR | PG_BINARY, 0);
	if (fd < 0)
	{
		ereport(LOG,
				(errcode_for_file_access(),
	...
	len = polar_read(fd, srcbuffer, sizeof(srcbuffer) - 1);
	pgstat_report_wait_end();
	if (len < 0)
	{
		ereport(LOG,
...
RecheckDataDirLockFile(void)
{
	...
	len = polar_read(fd, buffer, sizeof(buffer) - 1);
	pgstat_report_wait_end();
	if (len < 0)
	{
		ereport(LOG,
