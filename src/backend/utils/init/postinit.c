			 ...
			 char *out_dbname)
{
	 ...
	 */
	if (!am_superuser &&		/* POLAR: AmRegularBackendProcess() && */
 ...
 * - Auxiliary proc: in AuxiliaryProcessMainCommon
 *
 * So we need to borrow codes from InitPostgres/AuxiliaryProcessMainCommon and we'd
...
polar_init_dynamic_bgworker_in_backends(void)
{
	...
	if (MyProcNumber > MaxBackends || MyProcNumber <= 0)
		elog(FATAL, "bad backend ID: %d", MyProcNumber);
