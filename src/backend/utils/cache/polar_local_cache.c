...
static void
polar_default_convert_seg2str(char *buf, uint64 segno, bool long_segment_names)
{
	if (long_segment_names)
		sprintf(buf, "%015llX", (long long unsigned int) segno);
	else
...
#define LOCAL_CACHE_SUFFIX "_local_cache"
	...
	if (!IsUnderPostmaster)
	{
		...
		cache->long_segment_names = long_segment_names;
						...
						uint32 offset, char *buf, uint32 size, polar_cache_io_error * io_error)
{
	...
	if (polar_cache_copy_local_not_exists(cache, segno, io_error))
		write_size = polar_cache_file_write(cache, POLAR_LOCAL_CACHE_FILE, io_seg->segno, offset, buf, size, io_error);
