...
/* Register background writer workers. */
void
...
polar_parallel_bgwriter_worker_main(Datum main_arg)
{
	...
	for (;;)
	{
		...
		switch (task_type)
		{
				 ...
				 * current consistent_lsn lag. Enable parallel bgwriter to go
				 * into low-power hibernation mode when flush dispatcher is
				 * disabled and consistent lsn lag is low, otherwise the lock
				 * contention between parallel bgwriters will be serious,
		...
		/*
		 * Report pending statistics to the cumulative stats system, including
		 * WAL stats and I/O stats.
		 */
...
polar_shutdown_parallel_bgwriter_workers(int workers)
{
	...
	for (i = 0; i < workers; i++)
	{
		...
		WaitForBackgroundWorkerShutdown(&polar_parallel_bgwriter_info->polar_flush_work_info[used_slot].handle);
