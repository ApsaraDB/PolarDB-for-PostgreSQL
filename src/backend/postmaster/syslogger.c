...
SysLoggerMain(char *startup_data, size_t startup_data_len)
{
...
#ifdef EXEC_BACKEND
	{
		...
		MyLoggerIndex = slsdata->loggerIndex;
	}
#else
	{
		MyLoggerIndex = *(int *) startup_data;
	}
#endif
...
SysLogger_Start(int loggerIndex)
{
	...
	{
		memset(syslogChannels, -1, sizeof(syslogChannels));
	...
	startup_data.loggerIndex = loggerIndex;
	sysloggerPid = postmaster_child_launch(B_LOGGER, (char *) &startup_data, sizeof(startup_data), NULL);
#else
	sysloggerPid = postmaster_child_launch(B_LOGGER, (char *) &loggerIndex, sizeof(loggerIndex), NULL);
#endif							/* EXEC_BACKEND */
