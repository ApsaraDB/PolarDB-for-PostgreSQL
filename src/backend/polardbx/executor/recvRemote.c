 ...
 *
 * IDENTIFICATION
 *      src/backend/polardbx/recvRemote.c
 *
 *-------------------------------------------------------------------------
 */
...
static void CopyDataRowTupleToSlot(ResponseCombiner *combiner,
                                   TupleTableSlot *slot);

static void HandleCommandComplete(ResponseCombiner *combiner, char *msg_body,
                                  size_t len, PGXCNodeHandle *conn);

static bool HandleRowDescription(ResponseCombiner *combiner, char *msg_body,
                                 size_t len);

static bool HandleDataRow(ResponseCombiner *combiner, char *msg_body,
                          size_t len, Oid node);

static void HandleError(ResponseCombiner *combiner, char *msg_body, size_t len,
                        PGXCNodeHandle *conn);

static void HandleDatanodeCommandId(ResponseCombiner *combiner, char *msg_body,
                                    size_t len);

static void add_error_message_from_combiner(PGXCNodeHandle *handle,
                                            void *combiner_input);
 ...
 */
void pgxc_node_report_error(ResponseCombiner *combiner)
{ // #lizard forgives
    ...
    if (combiner->errorMessage)
    {
        ...
        if ((combiner->errorDetail == NULL) && (combiner->errorHint == NULL))
            ereport(ERROR, (errcode(MAKE_SQLSTATE(code[0], code[1], code[2], code[3],
                                                  code[4])),
                            errmsg("node:%s, backend_pid:%d, %s", combiner->errorNode,
                                   combiner->backend_pid, combiner->errorMessage)));
 ...
 */
void InitResponseCombiner(ResponseCombiner *combiner, int node_count,
...
/*
 * Parse out row count from the command status response and convert it to
 * integer
 */
static int parse_row_count(const char *message, size_t len, uint64 *rowcount)
{
...
static void HandleTimestamp(ResponseCombiner *combiner, char *msg_body,
                            size_t len, PGXCNodeHandle *conn)
{
    uint64 tmp;
...
#ifdef _PG_REGRESS_
                                elog(LOG,
                                     "RESPONSE_COMPLETE_2 set node %s, remote pid %d "
                                     "DN_CONNECTION_STATE_IDLE",
                                     conn->nodename, conn->backend_pid);
#endif
                            }
                            /* There is a consistency issue in the database with the
                            * replicated table */
 ...
 */
TupleDesc create_tuple_desc(char *msg_body, size_t len)
{
    TupleDesc result;
    int i, nattr;
    uint16 n16;
                                 ...
                                 size_t len)
{
    ...
    if (combiner->request_type != REQUEST_TYPE_QUERY)
    {
        ...
        ereport(ERROR, (errcode(ERRCODE_DATA_CORRUPTED),
                        errmsg("Unexpected response from the Datanodes for 'T' "
                               "message, current request type %d",
...
{ // #lizard forgives
    ...
    if (combiner->request_type != REQUEST_TYPE_QUERY)
    {
        ...
        data_buf[len] = 0;
        ereport(ERROR, (errcode(ERRCODE_DATA_CORRUPTED),
                        errmsg("Unexpected response from the data nodes for 'D' "
                               "message, current request type %d, data %s",
                      ...
                      combiner->errorCode[4]) == ERRCODE_PRODUCER_ERROR)
    {
        ...
        if (message)
        {
            ...
            snprintf(message_trans, strlen(message) + APPEND_LENGTH,
                     "nodename:%s,backend_pid:%d,message:%s", conn->nodename,
                     conn->backend_pid, message);
        }
...
static void
add_error_message_from_combiner(PGXCNodeHandle *handle,
                                void *combiner_input)
{ // #lizard forgives
    ...
    if (handle->error[0] && combiner->errorMessage)
    {
        ...
        if (combiner->errorMessage && combiner->errorDetail &&
            combiner->errorHint)
        {
            snprintf(handle->error + offset, MAX_ERROR_MSG_LENGTH - offset,
                     "%s:%s:%s", combiner->errorMessage, combiner->errorDetail,
                     combiner->errorHint);
        }
...
#ifdef _PG_REGRESS_
        elog(LOG,
             "add_error_message node:%s, running with pid %d non first time error: "
             "%s, error ptr:%lx",
             handle->nodename, handle->backend_pid, handle->error,
             (uint64)handle->error);
#endif
    }
    else if (combiner->errorMessage)
...
#ifdef _PG_REGRESS_
        elog(LOG,
             "add_error_message node:%s, running with pid %d first time error: %s, "
             "ptr:%lx",
             handle->nodename, handle->backend_pid, handle->error,
             (uint64)handle->error);
#endif
 ...
 */
int handle_response(PGXCNodeHandle *conn,
                    ResponseCombiner *combiner)
{ // #lizard forgives
    char *msg;
    int msg_len;
    ...
    for (;;)
    {
        ...
        */
        ...
        {
#ifdef _PG_REGRESS_
            elog(LOG, "RESPONSE_COMPLETE_1 from node %s, remote pid %d",
                 conn->nodename, conn->backend_pid);
#endif
        ...
        elog(DEBUG5,
             "handle_response - received message %c, node %s, "
             "current_state %d",
             msg_type, conn->nodename, conn->state);
        ...
        */
            ...
            ereport(
                ...
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("Received messy message from node:%s host:%s port:%d pid:%d, "
                        ...
                        conn->nodename, conn->nodehost, conn->nodeport,
                        conn->backend_pid, conn->inBuffer, conn->inSize,
                        conn->inStart, conn->inEnd, conn->inCursor, msg_len)));
        }
...
#ifdef _PG_REGRESS_
            elog(LOG, "RESPONSE_COMPLETE_2 from node %s, remote pid %d",
                 conn->nodename, conn->backend_pid);
#endif
            return RESPONSE_COMPLETE;
...
#ifdef _PG_REGRESS_
            elog(LOG, "ReadyForQuery from node %s, remote pid %d", conn->nodename,
                 conn->backend_pid);
#endif
        ...
        case 'I': /* EmptyQuery */
#ifdef _PG_REGRESS_
            elog(LOG, "RESPONSE_COMPLETE_3 from node %s, remote pid %d",
                 conn->nodename, conn->backend_pid);
#endif
            return RESPONSE_COMPLETE;
        ...
        default:
            ...
            PGXCNodeSetConnectionState(conn, DN_CONNECTION_STATE_ERROR_FATAL);
            /* stop reading */
            elog(LOG, "RESPONSE_COMPLETE_4 from node %s, remote pid %d",
                 conn->nodename, conn->backend_pid);
            return RESPONSE_COMPLETE;
        }
    }
 ...
 * successfully
 */
bool validate_combiner(ResponseCombiner *combiner)
{ // #lizard forgives
    ...
    {
        elog(LOG,
             "validate_combiner request_type is %d, command_complete_count:%d not "
             "equal node_count:%d",
             combiner->request_type, combiner->command_complete_count,
             combiner->node_count);
        return false;
    }
    ...
    {
        elog(LOG,
             "validate_combiner request_type is REQUEST_TYPE_QUERY, "
             "description_count:%d not equal node_count:%d",
             combiner->description_count, combiner->node_count);
        return false;
    }
    ...
    {
        elog(LOG,
             "validate_combiner request_type is REQUEST_TYPE_COPY_IN, "
             "copy_in_count:%d not equal node_count:%d",
             combiner->copy_in_count, combiner->node_count);
        return false;
    }
    ...
    {
        elog(LOG,
             "validate_combiner request_type is REQUEST_TYPE_COPY_OUT, "
             "copy_out_count:%d not equal node_count:%d",
             combiner->copy_out_count, combiner->node_count);
        return false;
    }
 ...
 */
void CloseCombiner(ResponseCombiner *combiner)
{ // #lizard forgives
 ...
 */
bool ValidateAndCloseCombiner(ResponseCombiner *combiner)
{
 ...
 */
void pgxc_connections_cleanup(ResponseCombiner *combiner)
{ // #lizard forgives
    int32 ret = 0;
   ...
   */
        ...
        if (combiner->connections[i] && combiner->connections[i]->needSync)
        {
            if (pgxc_node_send_sync(combiner->connections[i]))
            {
                ereport(LOG, (errcode(ERRCODE_INTERNAL_ERROR),
                              errmsg("Failed to sync msg to node:%s pid:%d when clean",
     ...
     */
        ...
        {
            elog(LOG, "pgxc_connections_cleanup is different, remove connection:%s",
                 conn->nodename);
            REMOVE_CURR_CONN(combiner);
            continue;
        }
        ...
        {
            elog(LOG,
                 "Failed to read response from data node:%s pid:%d when ending query "
                 "for ERROR, node status:%d",
        ...
        if (RESPONSE_EOF == res)
        {
            /* Continue to consume the data, until all connections are abosulately
            * done. */
            if (pgxc_node_is_data_enqueued(conn) && !proc_exit_inprogress)
            {
                ...
                {
                    elog(LOG,
                         "Failed to read response from data node:%u when ending query "
                         "for ERROR, state:%d",
            ...
            else if (!proc_exit_inprogress)
            {
                if (conn->state == DN_CONNECTION_STATE_QUERY)
                {
                    combiner->current_conn =
                        (combiner->current_conn + 1) % combiner->conn_count;
                    continue;
                }
            }
        }
...
#ifdef _PG_REGRESS_
                ereport(LOG, (errcode(ERRCODE_INTERNAL_ERROR),
                              errmsg("Succeed to sync msg to node:%s pid:%d when clean",
                                     conn->nodename, conn->backend_pid)));
#endif
                conn->needSync = false;
            }
                                   ...
                                   TupleTableSlot *slot)
{
    ...
    datarow = (RemoteDataRow)palloc(sizeof(RemoteDataRowData) +
...
void BufferConnection(PGXCNodeHandle *conn)
{
    ...
    elog(DEBUG2, "Buffer connection %u to step %s", conn->nodeoid,
         combiner->cursor);
    /*
    ...
    */
    oldcontext =
        MemoryContextSwitchTo(combiner->ss.ps.ps_ResultTupleSlot->tts_mcxt);
    ...
    if (!combiner->dataRowBuffer)
    {
        combiner->dataRowBuffer =
            (Tuplestorestate **)palloc0(sizeof(Tuplestorestate *) * node_cnt);
    }
    ...
    if (!combiner->dataRowBuffer[node_index])
    {
        combiner->dataRowBuffer[node_index] =
            tuplestore_begin_datarow(false, work_mem, NULL);
    }
    ...
    while (true)
    {
        ...
        */
        ...
        if (res == RESPONSE_DATAROW)
        {
            ...
            tuplestore_puttupleslot(combiner->dataRowBuffer[node_index],
                                    combiner->tmpslot);
            pfree(combiner->currentRow);
            combiner->currentRow = NULL;
            ...
            */
            ...
            if (conn->state == DN_CONNECTION_STATE_ERROR_FATAL)
            {
                ereport(
                    ...
                    (errcode(ERRCODE_INTERNAL_ERROR),
                     errmsg(
                         "Unexpected FATAL ERROR on Connection to Datanode %s pid %d",
                         conn->nodename, conn->backend_pid)));
            }
        }
...
static bool FetchFromTupleStore(ResponseCombiner *combiner)
{
    ...
    if (tuplestore_gettupleslot(combiner->dataRowBuffer[node_index], true, true,
                                combiner->tmpslot))
    {
 ...
 *         Function may wait while more data arrive from the data nodes. If
 there
 * is a locally executed subplan function advance it and buffer resulting rows
 * instead of waiting.
 */
TupleTableSlot *FetchTuple(ResponseCombiner *combiner)
{ // #lizard forgives
    PGXCNodeHandle *conn;
...
READ_NEXT_ROW:
    ...
    */
    ...
    while (conn)
    {
        ...
        */
        if (combiner->extended_query && conn->state == DN_CONNECTION_STATE_IDLE)
        {
        ...
        switch (res)
        {
            ...
            case RESPONSE_EOF:
            {
                ...
                if (pgxc_node_receive(1, &conn, NULL))
                    ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
                                    errmsg("Failed to receive more data from data node %u",
                                        conn->nodeoid)));
                break;
            }
            case RESPONSE_COMPLETE:
            {
                if (combiner->extended_query)
                {
                    if (combiner->merge_sort)
                    {
                        ...
                        conn = NULL;
                        if (combiner->dataRowBuffer &&
                            combiner->dataRowBuffer[combiner->current_conn])
                            goto READ_NEXT_ROW;
                    }
                    else
                    {
 ...
 */
int pgxc_node_receive_responses(
...
{ // #lizard forgives
    ...
    /*
    * Read results.
    * Note we try and read from Datanode connections even if there is an error on
    * one, so as to avoid reading incorrect results on the next statement. Other
    * safegaurds exist to avoid this, however.
    */
    while (count > 0)
    {
        int i = 0;
        ...
        if (DNStatus_ERR == ret)
        {
            ...
            has_errmsg = true;
            elog(LOG,
                 "pgxc_node_receive_responses pgxc_node_receive data from node "
                 "number:%d failed",
                 count);
            /* mark has error */
            func_ret = -1;
        ...
        while (i < count)
        {
...
#ifdef _PG_REGRESS_
            elog(LOG, "Received response %d on connection to node %s", result,
                 to_receive[i]->nodename);
#else
            elog(DEBUG5, "Received response %d on connection to node %s", result,
                 to_receive[i]->nodename);
#endif
            switch (result)
            {
            ...
            case RESPONSE_READY:
                ...
                UpdateLocalTwoPhaseState(result, to_receive[i], connection_index,
                                         combiner->errorMessage);
            case RESPONSE_COPY:
                ...
                if (nbytes)
                {
                    ...
                    elog(DEBUG1,
                         "Pending response %d bytes on connection to node %s, pid %d try "
                         "to read again. ",
                         nbytes, to_receive[i]->nodename, to_receive[i]->backend_pid);
                    ...
                    if (ret)
                    {
                        ...
                        {
                            elog(LOG,
                                 "pgxc_node_receive Pending data %d bytes from node:%s "
                                 "pid:%d failed for ERROR:%s. ",
                                 nbytes, to_receive[i]->nodename, to_receive[i]->backend_pid,
                                 strerror(errno));
                            break;
                        }
            ...
            default:
                ...
                add_error_message(to_receive[i],
                                  "Unexpected response from the Datanodes");
                elog(DEBUG1,
                     "Unexpected response from the Datanodes, result = %d, request "
                     "type %d",
