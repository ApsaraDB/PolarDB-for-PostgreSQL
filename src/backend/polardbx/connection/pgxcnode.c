...
#ifdef POLARDB_X
/*
 * Obsolete function. Use sql: select polardbx_finish_global_transation('$gid') instead.
 * Send the cleanup twophase command down to the PGXC node.
 */
int pgxc_node_send_cleanup_2pc_file_command(PGXCNodeHandle *handle, char *gid)
{
    int msglen = 4 + strlen(gid) + 1;

    if (enable_log_remote_query)
    {
        elog(LOG, "send cleanup 2pc file command to server %s", handle->nodename);
    }
    if (strlen(gid) == 0)
        elog(ERROR, "gid is not valid for sending");
    /* Invalid connection state, return error */
    if (handle->state != DN_CONNECTION_STATE_IDLE)
    {
        elog(LOG, "pgxc_node_send_cleanup_2pc_file_command datanode:%u invalid stauts:%d, no need to send data, return NOW", handle->nodeoid, handle->state);
        return EOF;
    }
