...
void ConsensusLOGTruncateForward(uint64 log_index, bool force)
{
	...
	easy_info_log("Consensus log truncate forward, log index: %ld, cutoffPage: %ld", log_index, cutoffPage);
...
consensus_meta_flush_internal(void)
{
	...
	if (write(fd, meta_buffer, meta_size) != meta_size)
	{
		if (errno == 0)
			errno = ENOSPC;
		close(fd);
...
ConsensusLOGGetLength(void)
{
	...
	return consensus_log_get_current_index() - purge_index;
}
