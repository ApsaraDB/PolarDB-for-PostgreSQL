...
polar_pfsd_preadv(int fd, const struct iovec *iov, int iovcnt, off_t offset)
{
	...
	if (iovcnt == 1)
	{
		return polar_pfsd_pread(fd, iov[0].iov_base, iov[0].iov_len, offset);
	}
	for (i = 0; i < iovcnt; ++i)
	{
		count += iov[i].iov_len;
	}
	buf = aligned_malloc(count, PG_IO_ALIGN_SIZE);
	if (buf != NULL)
	{
		char	   *src = (char *) buf;

		part = polar_pfsd_pread(fd, buf, count, offset);
		count = 0;
		if (part < 0)
		{
			aligned_free(buf);
