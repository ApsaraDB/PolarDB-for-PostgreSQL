...
static int	vfs_fdatasync(int file);
...
vfs_write(int file, const void *buf, size_t len)
{
	...
	if (unlikely(polar_vfs_debug))
	{
		if (vfdP->kind == POLAR_VFS_LOCAL_DIO &&
			!POLAR_DIRECTIO_IS_ALIGNED(buf))
			elog(LOG, "path %s vfs directio write is not aligned", vfdP->file_name);
	}
...
vfs_preadv(int file, const struct iovec *iov, int iovcnt, off_t offset)
{
	...
	if (polar_vfs_io_after_hook)
		polar_vfs_io_after_hook(vfdP, res, VFS_PREADV);
...
vfs_pwrite(int file, const void *buf, size_t len, off_t offset)
{
	...
	if (unlikely(polar_vfs_debug))
	{
		if (vfdP->kind == POLAR_VFS_LOCAL_DIO &&
			!POLAR_DIRECTIO_IS_ALIGNED(buf))
			elog(LOG, "path %s vfs directio pwrite is not aligned", vfdP->file_name);
	}
...
vfs_posix_fallocate(int file, off_t offset, off_t len)
{
	...
	if (polar_vfs_io_before_hook)
		polar_vfs_io_before_hook(vfdP, 0, VFS_POSIX_FALLOCATE);

	rc = vfs[vfdP->kind]->vfs_posix_fallocate(vfdP->fd, offset, len);
	save_errno = errno;

	if (polar_vfs_io_after_hook)
		polar_vfs_io_after_hook(vfdP, 0, VFS_POSIX_FALLOCATE);
...
vfs_fdatasync(int file)
{
	...
	return vfs[vfdP->kind]->vfs_fdatasync(vfdP->fd);
}
