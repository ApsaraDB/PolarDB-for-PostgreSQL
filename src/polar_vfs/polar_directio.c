...
{
	/*
	 * Calculate the total size we need to allocate, taking the alignment into
	 * account.
	 */
	void	   *ptr = malloc(required_bytes + alignment - 1 + sizeof(void *));
	...
	aligned_address = (void *) (((size_t) ptr + sizeof(void *) + alignment - 1) & ~(alignment - 1));
	/* Store the original pointer just before the aligned address. */
	((void **) aligned_address)[-1] = ptr;
	return aligned_address;
}

static void
aligned_free(void *aligned_alloc_ptr)
{
	/* Retrieve the original pointer and free it. */
	void	   *original_ptr = ((void **) aligned_alloc_ptr)[-1];
...
polar_directio_preadv(int fd, const struct iovec *iov, int iovcnt, off_t offset)
{
	...
	if (iovcnt == 1)
	{
		if (POLAR_DIRECTIO_IS_ALIGNED(iov[0].iov_base) &&
			POLAR_DIRECTIO_IS_ALIGNED(iov[0].iov_len) &&
			POLAR_DIRECTIO_IS_ALIGNED(offset))
			return pread(fd, iov[0].iov_base, iov[0].iov_len, offset);
		else
			return polar_directio_pread(fd, iov[0].iov_base,
										iov[0].iov_len, offset);
	}
	for (i = 0; i < iovcnt; ++i)
	{
		bytes_read += iov[i].iov_len;
	}
	buf = aligned_malloc(bytes_read, PG_IO_ALIGN_SIZE);
	if (buf != NULL)
	{
		char	   *src = (char *) buf;

		part = pread(fd, buf, bytes_read, offset);
		bytes_read = 0;
		if (part < 0)
		{
			aligned_free(buf);
		...
		for (i = 0; i < iovcnt; ++i)
		{
			if (part >= bytes_read + iov[i].iov_len)
			{
				memcpy((char *) iov[i].iov_base, src, iov[i].iov_len);
				bytes_read += iov[i].iov_len;
				src += iov[i].iov_len;
			}
		}
	...
	else
	{
		...
		for (i = 0; i < iovcnt; ++i)
		{
			if (POLAR_DIRECTIO_IS_ALIGNED(iov[i].iov_base) &&
				POLAR_DIRECTIO_IS_ALIGNED(iov[i].iov_len) &&
				POLAR_DIRECTIO_IS_ALIGNED(offset))
				part = pread(fd, iov[i].iov_base,
							 iov[i].iov_len, offset);
			else
				part = polar_directio_pread(fd, iov[i].iov_base,
											iov[i].iov_len, offset);
			if (part < 0)
			{
				if (i == 0)
