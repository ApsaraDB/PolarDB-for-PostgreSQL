...
polar_vfs_init(void)
{
	...
	elog(LOG, "begin mount pfs name %s id %d pid %d backendid %d",
		 polar_disk_name, polar_hostid, MyProcPid, MyProcNumber);
		...
		case VFS_RENAME:
			if (vfdp->kind == POLAR_VFS_LOCAL_DIO)
			{
				...
				if (polar_is_replica() && !polar_vfs_is_writable())
					elog(PANIC, "replica counld not write in shared storage by dio");
#endif
			}
...
#ifndef FRONTEND
				if (polar_is_replica() && !polar_vfs_is_writable())
					elog(WARNING, "replica counld not write in shared storage by pfs");
#endif
			}
			break;
		default:
