...
command_checks_all(
	[ 'pg_controldata', $node_replica1->data_dir ],
	0,
	[qr/./],
	[
		qr/warning: calculated CRC checksum does not match value stored in control file/,
...
# standby
$pg_control = $node_standby1->polar_get_datadir . '/global/pg_control';
destroy_control_file($pg_control);
command_checks_all(
	[ 'pg_controldata', $node_standby1->data_dir ],
	0,
	[qr/./],
	[
		qr/warning: calculated CRC checksum does not match value stored in control file/,
