...
logindex_table_main(int argc, char **argv)
{
	...
	table = malloc(sizeof(log_index_table_t));
	if (!table)
		goto end;
	...
	while ((ret = fread(table, 1, sizeof(log_index_table_t), fp)) == sizeof(log_index_table_t))
	{
		crc = table->crc;
		table->crc = 0;
	...
	if ((ret > 0 && ret != sizeof(log_index_table_t)) || errno != 0)
	{
