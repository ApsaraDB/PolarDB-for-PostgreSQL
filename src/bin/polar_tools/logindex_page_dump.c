...
static void
logindex_search_table(BufferTag *tag, log_index_table_t *table, uint32 key)
{
...
logindex_page_main(int argc, char **argv)
{
	...
	while ((option = getopt_long(argc, argv, "p:?s:d:r:f:b:", long_options, &optindex)) != -1)
	{
		switch (option)
		{
			case 'p':
				...
				break;
			case 's':
				tag.spcOid = atoi(optarg);
				break;
			case 'd':
				tag.dbOid = atoi(optarg);
				break;
			case 'r':
				tag.relNumber = atoi(optarg);
				break;
			case 'f':
	...
	{
		table = (data + offset);
		logindex_search_table(&tag, (log_index_table_t *) table, key);
		offset += sizeof(log_index_table_t);
	}
