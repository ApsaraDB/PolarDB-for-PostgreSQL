...
usage(void)
{
	...
	printf(_("      --polar_disk_home=disk  polar_disk_home for polardb\n"));
	printf(_("      --polar_host_id=host_id  polar_host_id for polardb\n"));
	printf(_("      --polar_storage_cluster_name=cluster_name  polar_storage_cluster_name for polardb\n"));
	printf(_("\nReport bugs to <%s>.\n"), PACKAGE_BUGREPORT);
					 ...
					 pg_compress_specification *compress)
{
	...
	if (format == 'p')
	{
		 ...
		 */
		if (spclocation == NULL)
			directory = is_polar_data ? datadir : basedir;
	...
	else
	{
		...
		{
			need_append_polar_data = has_polar_data && !is_polar_data && strcmp(basedir, "-") == 0;
			streamer = bbstreamer_tar_archiver_new(streamer, need_append_polar_data);
		}
	...
	if (spclocation == NULL && writerecoveryconf)
	{
		...
		is_injected_polar_data = is_polar_data && strcmp(basedir, "-") == 0;
...
main(int argc, char **argv)
{
	static struct option long_options[] = {
		...
		{"polardata", required_argument, NULL, 9},
		{"polar_disk_name", required_argument, NULL, 10},
		{"polar_host_id", required_argument, NULL, 11},
		{"polar_storage_cluster_name", required_argument, NULL, 12},
		{NULL, 0, NULL, 0}
	};
	int			c;
