...
my $excludes  ||= "$code_base/src/tools/polar_coverage_exclude_patterns"
  if $code_base && -f "$code_base/src/tools/polar_coverage_exclude_patterns";
...
sub compare()
{
	...
	my $reg_no_cover_begin =
	  '\/\* no cover begin \*\/$';                    #"/*no cover begin*/"
...
# pattern in 'polar_coverage_exclude_patterns'
#-----------------------------------------------------
sub process_exclude
{
	if ($excludes && @valid_file)
	{
		open(my $eh, '<', $excludes)
		...
		while (my $line = <$eh>)
		{
			...
			next if $line =~ m/^\s*$/;
			my $rgx = qr!$line!;
			@valid_file = grep { $_ !~ /$rgx/ } @valid_file if $rgx;
		}
		close($eh);
	}
	return;
