...
#	  Fixed the lack of period in short desc of GUC.
#
...
sub process_guc_short_desc
{
	...
	foreach $a (@array)
	{
		...
		if (index($a, "struct config_bool ConfigureNamesBool[]") > -1)
		{
			$begin_process = 1;
		}
		if ($begin_process == 1)
...
process_guc_short_desc("src/backend/utils/misc/guc_tables.c");
