...
my @common_typo = (
	'cleard', 'confict', 'conficts', 'destory',
	'enalbe', 'exactlly', 'exsits', 'flie',
	'funcition', 'poalr', 'recognisable', 'rumtime',
	'sucess', 'truacte', 'trucate', 'wirte',
	'wrod',);
...
sub c_format_check
{
	...
	if ($check_spelling)
	{
		for (my $i = 0; $i < @lines; $i++)
		{
			...
			# /* POLAR :
			# /* POLAR:xxx
			if (   $lines[$i] =~ m!\*\s+POLAR\s+:!
				|| $lines[$i] =~ m!\*\s+POLAR:\S+!)
			{
				print
				  "  $logger_error Ugly comment format detected at line $i: $lines[$i].\n";
				$lines[$i] = $lines[$i] . '<<<';
			}
		}
	}
...
{
	return '
polar_copyright_check.pl [-s] [ -f file | -p prev_commit ]
