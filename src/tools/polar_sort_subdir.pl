...
#	  Sort lines in ASCII order in unit of hunks of a specific file.
#
...
#--------------------------------------------------------------
# Input parameters
#     -f: sort lines for a specific file
#     -h: help
#--------------------------------------------------------------
...
if (!-e $file_abs_path)
{
	print "File '$file_abs_path' does not exist, skipped.\n";
	exit 0;
}
...
foreach my $pattern (keys %patterns)
{
	...
	# Process and sort lines as hunk
	my @sorted_lines;
	my @current_hunk;
	my $in_sorting_section = 0;
	my %seen;
	foreach my $line (@lines)
	{
		if ($line =~ /^$pattern/)
		{
			$in_sorting_section = 1;
			push @current_hunk, $line;
		}
		elsif ($in_sorting_section)
		{
			$in_sorting_section = 0;
	...
	if ($in_sorting_section)
	{
		push @sorted_lines, sort { lc($a) cmp lc($b) } grep !$seen{$_}++,
		  @current_hunk;
	}

	# Deduplicate lines in the whole file
