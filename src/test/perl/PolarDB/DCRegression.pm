...
sub new
{
	...
	if (defined $ENV{DCCheckEnvFile})
	{
		open my $fd, '<', $ENV{DCCheckEnvFile}
		  or polar_die "failed to open $ENV{DCCheckEnvFile}: $!";
		foreach my $line (<$fd>)
		{
...
{
	my ($self, $node_replica_ref, $test_case) = @_;
	...
	$node_primary->safe_psql($regress_db,
		"select pg_logical_emit_message(false, 'dccheck', 'replay_check_all');"
	);

	print "($test_case) Waiting for replica nodes' replay_lsn to pass "
	  . $insert_lsn . " on "
	  . $node_primary->name . "\n";

	foreach my $node_replica (@{$node_replica_ref})
	{
		my $query = qq[SELECT '$insert_lsn' <= pg_last_wal_replay_lsn();];

		$start_time = [gettimeofday];
		$node_replica->poll_query_until('postgres', $query)
		  or polar_die(
			"timed out waiting for replay_check_all " . $node_replica->name);
...
sub polar_online_promote
{
	...
	$self->replay_check_all($node_replica_ref, "polar_online_promote");
	...
	{
		polar_die "PANIC: Wrong node_type: $new_primary_origin_type\n";
	}
	...
	my @replicas = ($old_primary);
	$self->replay_check_all(\@replicas, "polar_online_promote");
...
sub polar_promote
{
	...
	$self->replay_check_all($node_replica_ref, "polar_promote");
	...
	# replay check for old_primary
	my @replicas = ($old_primary);
	$self->replay_check_all(\@replicas, "polar_promote");
	# start the left nodes
	foreach my $node_replica (@{$node_replica_ref})
	{
		if ($node_replica ne $old_primary)
...
sub sql
{
	...
	polar_die "PANIC: params is not expected: " . @{$params} . "\n"
...
sub pgbench_worker_main
{
	...
	open my $INIT, '+>', $pgbench_init_file
	  or polar_die "failed to open $pgbench_init_file: $!";
	truncate($INIT, 0)
	  or polar_die "failed to truncate $pgbench_init_file: $!";
	...
	while ($request_shutdown eq 0)
	{
		...
		print "pgbench($total) node $name\n";
		if ($node->polar_is_in_recovery eq 0)
		{
			$node->pgbench_test(
				dbname => $regress_db,
				client => 5,
			...
			&& @ready_pipes eq 1)
		{
			...
			polar_die
			  "PANIC: Unexpected pipe: $temp_pipe, it should be $pipe!\n"
			  if $temp_pipe != $pipe;
			$command = <$pipe>;
...
sub misc_worker_main
{
	my ($self, $all_nodes, $pipe, $database, $ppid) = @_;
	my ($total, $cost_time, $sql_time) = (0, 0, 0);
	my $regress_db = $self->regress_db;
	my $total_timeout = 3600;
	my $sfd = IO::Select->new($pipe);
	my @ready_pipes;
	my $request_shutdown = 0;
	my $switch_wal_sql = 'select pg_switch_wal();';

	while ($request_shutdown eq 0)
	{
		my ($node_primary, $name);

		# find the right primary node
		for (my $i = 0; $i < scalar(@{$all_nodes}); $i += 1)
		{
			$node_primary = @{$all_nodes}[$i];
			$name = $node_primary->name;
			if ($node_primary->polar_is_in_recovery eq 0)
			{
				last;
			}
		}
		...
		my $start_time = [gettimeofday];
		my $timeout = 50 + int(rand(20));

		print "misc worker ($total) node $name\n";

		local %ENV = $node_primary->_get_env();
		my ($stdout, $stderr) = ('', '');
		my $ret = $node_primary->psql(
			$regress_db, $switch_wal_sql,
			stdout => \$stdout,
			stderr => \$stderr,
			on_error_die => 0,
			on_error_stop => 0);

		if ($ret ne 0 or ($stdout eq '' and $stderr ne ''))
		{
			note "misc worker ($$) failed to switch wal for $name: $stderr\n";
		}

		$total    += 1;
		$sql_time += tv_interval($start_time, [gettimeofday]);
			...
			&& @ready_pipes eq 1)
		{
			my $command = "";
			my $temp_pipe = $ready_pipes[0];
			polar_die "Unexpected pipe: $temp_pipe, it should be $pipe!\n"
			  if $temp_pipe != $pipe;
			$command = <$pipe>;
		...
		$cost_time += $sql_time + $timeout;

		if ($cost_time > $total_timeout)
		{
			note "misc worker $$ has been running for too long!\n";
			last;
		}
		elsif (getppid() != $ppid)
		{
			note "misc worker $$ found parent process exited!\n";
			last;
		}

		if ($total % 10 == 0)
		{
			note "running misc worker($$) for $name: "
			  . "$total times, sql time $sql_time s\n";
		}
	}
	note "finish misc worker($$): $total times, sql time $sql_time s\n";
	exit;
}

sub start_child_workers
{
	my ($self, $node_replica_ref) = @_;
	my $self_pid = $$;
	my $regress_db = $self->regress_db;
	my $node_primary = $self->node_primary;
	my @all_nodes = ($node_primary, @{$node_replica_ref});
	my @child_workers = ();
	...
	if ($pid == 0)
	{
		$0 = "misc worker";
		$pipe->reader;
		$self->misc_worker_main(\@all_nodes, $pipe, $regress_db, $self_pid);
		polar_die "Never be here!";
	}
	elsif ($pid > 0)
	{
		$pipe->writer;
		note "--->Created misc worker process $pid\n";
		push @child_workers, (undef, $pid, $pipe);
	}
	else
	{
		polar_die "Failed to fork misc worker!";
	}
	...
	foreach my $node (@all_nodes)
	{
		...
		if ($pid == 0)
		{
			$0 = "pgbench worker for " . $node->name;
			$pipe->reader;
		...
		elsif ($pid > 0)
		{
			...
			note "--->Created pgbench worker process($pid) for "
			  . $node->name . "\n";
			push @child_workers, ($node, $pid, $pipe);
		}
		else
		{
			polar_die "Failed to fork pgbench worker!";
		}
	}
	return @child_workers;
}

sub terminate_child_workers
{
	my ($self, $child_workers) = @_;

	polar_die "Unexpected pgbench workers!" if (!defined $child_workers);

	# send shutdown command
	for (my $index = 0; $index < scalar(@{$child_workers}); $index += 3)
	{
		my $node = @{$child_workers}[$index];
		my $pid = @{$child_workers}[ $index + 1 ];
		my $pipe = @{$child_workers}[ $index + 2 ];
		...
		{
			note "send shutdown command to misc worker\n";
		}
	}
	...
	for (my $index = 0; $index < scalar(@{$child_workers}); $index += 3)
	{
		...
		{
			note "wait misc worker $pid to exit\n";
		}

		my $wpid = waitpid($pid, 0);
		if ($wpid == $pid)
		{
			note "child worker $pid has been exited!\n";
		}
		elsif ($wpid < 0)
		{
			note "child worker $pid may have already exited!\n";
		}
		else
		{
			polar_die "Unexpected waitpid's result for child worker $pid\n";
		}
	}
}
...
sub test
{
	...
	open my $schedule, '<', "$schedule_file"
	  or polar_die "Could not open $schedule_file: $!";
	my $failed = 0;
	my $task_count = 0;
	my $regress_db = $self->regress_db;
	my @child_workers = undef;
	...
	$self->replay_check_all($node_replica_ref, "test all");

	@child_workers = $self->start_child_workers($node_replica_ref);
			...
			while ($live_children > 0 or $left_tasks > 0)
			{
				...
				if ($left_tasks > 0)
				{
					...
					if ($pid == 0)
					{
						...
						$0 = "dccheck worker $child_index test for $task";
	...
	$self->terminate_child_workers(\@child_workers);
...
our $polar_explain = "-- POLAR_EXPLAIN";    # PolarDB explain sql block
our $polar_border_ro_explain =
  "-- POLAR_END_RO_EXPLAIN"
  ;    # PolarDB end border of ro transaction sql block with explain
our $polar_border_rw_explain =
  "-- POLAR_END_RW_EXPLAIN"
  ;    # PolarDB end border of rw transaction sql block with explain
...
sub is_sql_border
{
	...
	elsif ($sql =~ qr/$polar_border_ro_explain/i
		|| $sql =~ qr/$polar_border_rw_explain/i)
	{
		return 2;
	}
	else
...
sub execute_sql
{
	...
	if ($output->{_ret} ne 0)
	{
		my $msg = "PANIC: ## $test_name Unexpected ret with $name node!"
...
sub check_results
{
	...
	foreach my $replica_output (@{$replica_output_array})
	{
		if (   $polar_tag =~ $polar_border_rw
			|| $polar_tag =~ $polar_border_rw_explain)
		{
			...
			|| $polar_tag =~ $polar_border_ro_explain
			|| $polar_tag =~ $polar_db_user_change)
		{
			if ($primary_output->{_stdout} ne $replica_output->{_stdout})
			{
					...
					&& $polar_tag =~ $polar_border_ro_explain)
				{
					$failed = $DC_RES{'SUCC'};
					$$note_msg .=
					  "recheck pass: explain sql could make the result different.\n";
				}
			}
		...
		{
			polar_die "Wrong polar_tag: $polar_tag!";
		}
...
sub save_diff_files
{
	my ($self, $node_primary, $node_replica_ref,
	  ...
	  = @_;
	my ($primary_fname, $replica_fname, $diff_fname);
	my ($primary_fd, $replica_fd, $diff_fd);
	my $diff_log_path = $PostgreSQL::Test::Utils::log_path;
	my $msg = "diff files:\n";

	$primary_fname = "$diff_log_path/$test_case\_$$." . $node_primary->name;
	$self->build_parent_dir($primary_fname);
	$diff_fname = "$diff_log_path/$test_case\_$$.diff";

	open $primary_fd, '+>', $primary_fname
	  or polar_die "failed to open $primary_fname: $!";
	...
	for (my $i = 0; $i < scalar @{$replica_output_array}; $i++)
	{
		...
		$replica_fname =
		  "$diff_log_path/$test_case\_$$." . $node_replica->name;
		open $replica_fd, '+>', $replica_fname
		  or polar_die "failed to open $replica_fname: $!";
		...
		system(
			"diff -c \"$primary_fname\" \"$replica_fname\" >> \"$diff_fname\""
		);
	}

	open $diff_fd, '<', $diff_fname
	  or polar_die "failed open $diff_fname!";
	$msg .= join('', <$diff_fd>);
	close $diff_fd;
...
{
	my ($self, $node_primary, $node_replica_ref,
		$test_case, $regress_db, $sql,
		$line_num, $polar_tag, $regress_out) = @_;
	...
	{
		polar_die "POLAR_TAG: $polar_tag is incorrect\n";
	}
	...
	if (   $polar_tag =~ $polar_border_rw
		|| $polar_tag =~ $polar_border_rw_explain)
	{
		$self->replay_check_all($node_replica_ref, $test_case);
	}
...
sub test_one_case
{
	...
	open my $regress_out, '>', $regress_out_file
	  or polar_die "Failed to open $regress_out_file";
	open my $INPUT, '<', $sql_file
	  or polar_die "Failed to open $sql_file: $!";
	my @all_sql = <$INPUT>;
	close $INPUT;
	my $line_num = 0;
	my $polar_tag;
	...
	for (my $index = 0; $index < @all_sql; $index++)
	{
		...
		my $replica_stderr = '';
		$res =
		  $self->test_some_sql($node_primary, $node_replica_ref, $test_case,
			$regress_db, $sql, $index + 1, $all_sql[$index], $regress_out);
...
sub try_rewrite_sql_file
{
	...
	my $rewrite = 0;
	open my $dst_sql_fd, '>>', $dst_file
	  or polar_die "Failed to open $dst_file";
	open my $src_sql_fd, '<', $src_file
	  or polar_die "Failed to open $src_file";
...
sub is_explain_sql
{
	...
	if (   $sql =~ qr/^explain\s*\([^\)]+\)\s*/i
		|| $sql =~ qr/\nexplain\s*\([^\)]+\)\s*/i
		|| $sql =~ qr/^explain\s+/i
		|| $sql =~ qr/\nexplain\s+/i
		|| $sql =~ qr/explain_analyze_without_memory/i
		|| $sql =~ qr/explain_memoize/i)
	{
		return 1;
	}
	else
...
sub prepare_one_case_phase_1
{
	...
	{
		polar_die
		  "PANIC: ($test_case,$$) Only one primary node could be needed to do this worker!("
		  . @{$node_replica_ref} . ")\n";
	}
	...
	open my $src_sql_fd, '+>>', $sql_rewrite
	  or polar_die "failed to open $sql_rewrite";
	...
	open my $stderr_fd, '>', $primary_stderr_file
	  or polar_die "failed to open $primary_stderr_file";
	...
	while ($node_primary->psql_pumpable)
	{
		...
		polar_die
		  "PANIC: ($test_case,$$) psql_step_forward failed:\nstdout:\n$primary_ret->{_stdout}\nstderr:\n$primary_ret->{stderr}\n"
				...
				|| !$node_primary->psql_pumpable))
		{
			...
			if ($self->is_explain_sql($cur_trans_sql) > 0)
			{
				push @dst_sql_array, "$polar_explain\n";
			}
	...
	polar_die "PANIC: Maybe there's some sqls left in $sql_rewrite: "
	  . "$src_sql_array_index vs "
	  . @src_sql_array . "\n"
	  if ($src_sql_array_index < @src_sql_array);

	polar_die "PANIC: Unexpected result for $sql_rewrite during phase one: "
	  . @dst_sql_array . "!\n"
	  if (@dst_sql_array <= 0);

	open my $dst_sql_fd, '>', $sql_input
	  or polar_die "failed to open $sql_input";
...
our @only_ro_error_patterns = (
	...
	[ '55000', [qr/\s*is not yet defined in this session\s*/i] ],);
...
sub is_trans_readonly
{
	...
	if (@errorcode_array > 0 && $errorcodes eq "")
	{
		...
		foreach my $obj (@temp_objs)
		{
			if ($replica_stderr =~ qr/[^a-zA-Z_]$obj[^a-zA-Z_]/i)
			{
				note
				  "Maybe there's some temp object $obj in replica_stderr of sql:$replica_stderr\n";
				return $ReadWrite;
			}
			elsif ($sql =~ qr/[^a-zA-Z_]$obj[^a-zA-Z_]/i)
			{
				note
				  "Maybe there's some temp object $obj in sql($line_number):$sql\nreplica stderr:$replica_stderr\n";
				return $ReadWrite;
			}
		}
...
sub prepare_one_case_phase_2
{
	...
	{
		polar_die
		  "PANIC: ($test_case,$$) At least one Primary node and one Replica node to do this work!("
		  . @{$node_replica_ref} . ")\n";
	}
	...
	open my $primary_stderr_fd, '>', $primary_error_file
	  or polar_die "Failed to open $primary_error_file!";
	$node_primary->psql_connect($regress_db, $global_timeout,
		output_file => $primary_temp_file);
	...
	open my $replica_stderr_fd, '>', $replica_error_file
	  or polar_die "Failed to open $replica_error_file!";
	$node_replica->psql_connect($regress_db, $global_timeout,
		output_file => $replica_temp_file);
	## fetch all content from original input sql file
	open my $src_sql_fd, '<', $sql_input
	  or polar_die "failed to open $sql_input";
	my @src_sql_array = <$src_sql_fd>;
	close $src_sql_fd;
	my @dst_sql_array;
	...
	my $cur_trans_sql_with_explain = 0;
	## replay check all
	$self->replay_check_all($node_replica_ref, $test_case);
	for (
		my $src_sql_array_index = 0;
		...
		$src_sql_array_index++)
	{
			...
			qr/$polar_db_user_change/i)
		{
			...
			if ($src_sql_array[$src_sql_array_index] =~ qr/$polar_explain/i)
			{
				$cur_trans_sql_with_explain += 1;
			}
			next;
		}
		...
		{
			polar_die "primary error sql should be ignored by replica nodes!"
			  if $errorcodes ne "";
			if ($cur_trans_sql_with_explain > 0)
			{
				push @dst_sql_array, "$polar_border_ro_explain\n";
			}
			else
			{
		...
		elsif ($trans_state eq $ReadWrite || $trans_state eq $ROCommit)
		{
			...
			if ($cur_trans_sql_with_explain > 0)
			{
				$rw_border = $polar_border_rw_explain;
			}
			else
			{
				$rw_border = $polar_border_rw;
			}
		...
		{
			polar_die "Wrong result: $trans_state!\n";
		}
		$cur_trans_sql = "";
		$exec_trans_sql = $set_verbosity_sql;
		$cur_trans_sql_with_explain = 0;
	}
	...
	polar_die "PANIC: Unexpected result for $sql_input during phase two!\n"
	  if (@dst_sql_array <= 0);
	...
	{
		polar_die
		  "PANIC: ##($test_case,$$) Unexpected end: $dst_sql_array[$index]";
	}
	while ($index >= 0)
	{
		if ($self->is_sql_border($dst_sql_array[$index]) > 0)
		{
	...
	open my $dst_sql_fd, '>', $sql_target
	  or polar_die "failed to open $sql_target";
	foreach my $line (@dst_sql_array)
	{
		if (defined $line)
		{
