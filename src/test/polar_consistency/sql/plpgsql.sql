...
drop table master, slave;

--
-- Test proper snapshot handling in simple expressions
--

create table users1(login text, id serial);

create function sp_id_user(a_login text) returns int as $$
declare x int;
begin
  select into x id from users1 where login = a_login;
  if found then return x; end if;
  return 0;
end$$ language plpgsql stable;
-- POLAR_END_FUNC

insert into users1 values('user1');
...
begin
  ...
  END IF;
  INSERT INTO users1 ( login ) VALUES ( a_login );
  my_id_user = sp_id_user( a_login );
  IF  my_id_user = 0 THEN
...
create table foo2 (f1 int, f2 int);
...
begin
  -- should work
  insert into foo2 values(5,6) returning * into x;
...
begin
  ...
  insert into foo2 values(7,8),(9,10) returning * into x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- should work
  execute 'insert into foo2 values(5,6) returning *' into x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  ...
  execute 'insert into foo2 values(7,8),(9,10) returning *' into x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- should work
  select * from foo2 where f1 = 3 into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- should fail, no rows
  select * from foo2 where f1 = 0 into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- should work
  execute 'select * from foo2 where f1 = 3' into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- should fail, no rows
  execute 'select * from foo2 where f1 = 0' into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- no rows
  select * from foo2 where f1 = p1 and f1::text = p3 into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- too many rows
  select * from foo2 where f1 > p1 or f1::text = p3  into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- no rows
  execute 'select * from foo2 where f1 = $1 or f1::text = $2' using 0, 'foo' into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
...
begin
  -- too many rows
  execute 'select * from foo2 where f1 > $1' using 1 into strict x;
  raise notice 'x.f1 = %, x.f2 = %', x.f1, x.f2;
end$$ language plpgsql;
-- POLAR_END_FUNC
