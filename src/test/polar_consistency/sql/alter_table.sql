...
-- Note: these tables are TEMP_* to avoid name conflicts when this test
-- is run in parallel with foreign_key.sql.
-- Test that ALTER CONSTRAINT updates trigger deferrability properly

CREATE TABLE TEMP_PKTABLE (ptest1 int primary key);
CREATE TABLE TEMP_FKTABLE (ftest1 int);

ALTER TABLE TEMP_FKTABLE ADD CONSTRAINT fknd FOREIGN KEY(ftest1) REFERENCES temp_pktable
  ON DELETE CASCADE ON UPDATE NO ACTION NOT DEFERRABLE;
ALTER TABLE TEMP_FKTABLE ADD CONSTRAINT fkdd FOREIGN KEY(ftest1) REFERENCES temp_pktable
  ON DELETE CASCADE ON UPDATE NO ACTION DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE TEMP_FKTABLE ADD CONSTRAINT fkdi FOREIGN KEY(ftest1) REFERENCES temp_pktable
  ON DELETE CASCADE ON UPDATE NO ACTION DEFERRABLE INITIALLY IMMEDIATE;

ALTER TABLE TEMP_FKTABLE ADD CONSTRAINT fknd2 FOREIGN KEY(ftest1) REFERENCES temp_pktable
  ON DELETE CASCADE ON UPDATE NO ACTION DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE TEMP_FKTABLE ALTER CONSTRAINT fknd2 NOT DEFERRABLE;
ALTER TABLE TEMP_FKTABLE ADD CONSTRAINT fkdd2 FOREIGN KEY(ftest1) REFERENCES temp_pktable
  ON DELETE CASCADE ON UPDATE NO ACTION NOT DEFERRABLE;
ALTER TABLE TEMP_FKTABLE ALTER CONSTRAINT fkdd2 DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE TEMP_FKTABLE ADD CONSTRAINT fkdi2 FOREIGN KEY(ftest1) REFERENCES temp_pktable
  ON DELETE CASCADE ON UPDATE NO ACTION NOT DEFERRABLE;
ALTER TABLE TEMP_FKTABLE ALTER CONSTRAINT fkdi2 DEFERRABLE INITIALLY IMMEDIATE;
SELECT conname, tgfoid::regproc, tgtype, tgdeferrable, tginitdeferred
FROM pg_trigger JOIN pg_constraint con ON con.oid = tgconstraint
WHERE tgrelid = 'temp_pktable'::regclass
ORDER BY 1,2,3;
SELECT conname, tgfoid::regproc, tgtype, tgdeferrable, tginitdeferred
FROM pg_trigger JOIN pg_constraint con ON con.oid = tgconstraint
WHERE tgrelid = 'temp_fktable'::regclass
ORDER BY 1,2,3;

-- drop TEMP_* tables.
DROP TABLE TEMP_FKTABLE;
DROP TABLE TEMP_PKTABLE;
...
create table foo1 (f1 text, f2 mytype, f3 text);

insert into foo1 values('bb','cc','dd');
select * from foo1;

drop domain mytype cascade;

select * from foo1;
insert into foo1 values('qq','rr');
select * from foo1;
update foo1 set f3 = 'zz';
select * from foo1;
select f3,max(f1) from foo1 group by f3;

-- Simple tests for alter table column type
alter table foo1 alter f1 TYPE integer; -- fails
alter table foo1 alter f1 TYPE varchar(10);
...
create table temp_temp_part_parent (a int) partition by list (a);
create table perm_part_child (a int);
create table temp_temp_part_child (a int);
alter table temp_temp_part_parent attach partition perm_part_child default; -- error
alter table perm_part_parent attach partition temp_temp_part_child default; -- error
alter table temp_temp_part_parent attach partition temp_temp_part_child default; -- ok
drop table perm_part_parent cascade;
drop table temp_temp_part_parent cascade;
drop table temp_temp_part_child cascade;
