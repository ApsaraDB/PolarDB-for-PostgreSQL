...
SET SESSION AUTHORIZATION regress_priv_user1;
CREATE TABLE t11 (c1 int, c2 int, c3 int check (c3 < 5), primary key (c1, c2));
GRANT SELECT (c1) ON t11 TO regress_priv_user2;
GRANT INSERT (c1, c2, c3) ON t11 TO regress_priv_user2;
GRANT UPDATE (c1, c2, c3) ON t11 TO regress_priv_user2;

-- seed data
INSERT INTO t11 VALUES (1, 1, 1);
INSERT INTO t11 VALUES (1, 2, 1);
INSERT INTO t11 VALUES (2, 1, 2);
INSERT INTO t11 VALUES (2, 2, 2);
INSERT INTO t11 VALUES (3, 1, 3);

SET SESSION AUTHORIZATION regress_priv_user2;
INSERT INTO t11 (c1, c2) VALUES (1, 1); -- fail, but row not shown
UPDATE t11 SET c2 = 1; -- fail, but row not shown
INSERT INTO t11 (c1, c2) VALUES (null, null); -- fail, but see columns being inserted
INSERT INTO t11 (c3) VALUES (null); -- fail, but see columns being inserted or have SELECT
INSERT INTO t11 (c1) VALUES (5); -- fail, but see columns being inserted or have SELECT
UPDATE t11 SET c3 = 10; -- fail, but see columns with SELECT rights, or being modified
...
select has_table_privilege(current_user,t11.oid,'rule')
from (select oid from pg_class where relname = 'pg_authid') as t11;
select has_table_privilege(current_user,t11.oid,'references')
from (select oid from pg_class where relname = 'pg_authid') as t11;

select has_table_privilege(t2.oid,t11.oid,'select')
from (select oid from pg_class where relname = 'pg_authid') as t11,
  (select oid from pg_roles where rolname = current_user) as t2;
select has_table_privilege(t2.oid,t11.oid,'insert')
from (select oid from pg_class where relname = 'pg_authid') as t11,
  (select oid from pg_roles where rolname = current_user) as t2;
...
select has_table_privilege(t11.oid,'select')
from (select oid from pg_class where relname = 'pg_authid') as t11;
select has_table_privilege(t11.oid,'trigger')
from (select oid from pg_class where relname = 'pg_authid') as t11;
...
select has_table_privilege(current_user,t11.oid,'references')
from (select oid from pg_class where relname = 'pg_class') as t11;
...
select has_table_privilege(current_user,t11.oid,'references')
from (select oid from pg_class where relname = 'atest1') as t11;
...
CREATE SCHEMA testns;
CREATE TABLE testns.t11 (f1 int);
CREATE TABLE testns.t2 (f1 int);

SELECT has_table_privilege('regress_priv_user1', 'testns.t11', 'SELECT'); -- false

GRANT ALL ON ALL TABLES IN SCHEMA testns TO regress_priv_user1;

SELECT has_table_privilege('regress_priv_user1', 'testns.t11', 'SELECT'); -- true
