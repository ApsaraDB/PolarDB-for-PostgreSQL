...
SELECT nextval('foo_seq_new');
-- POLAR: Not each fetching is loged, so we compare totoal value
SELECT (last_value+log_cnt) AS seq_max, is_called FROM foo_seq_new;
...
-- sometines last_value is -33 in replica or standby during in parellel mode.
