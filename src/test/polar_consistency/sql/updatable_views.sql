...
SELECT count(*) FROM v1 WHERE a=3;
SELECT * FROM v1 WHERE a=3; -- should not see anything
SELECT count(*) FROM v1 WHERE a=8;
...
SELECT count(*) FROM v1 WHERE b=8;
...
create table sometable1 (a int, b text);
insert into sometable1 values (1, 'a'), (2, 'b');
create view wcowrtest_v2 as
    select *
      from wcowrtest r
      where r in (select s from sometable1 s where r.a = s.a)
with check option;

-- WITH CHECK qual will be processed with wcowrtest2's
-- rowtype after tuple-routing
insert into wcowrtest_v2 values (2, 'no such row in sometable1');

drop view wcowrtest_v, wcowrtest_v2;
drop table wcowrtest, sometable1;
