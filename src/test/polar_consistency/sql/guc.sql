...
-- Test DISCARD TEMP
--
CREATE TABLE temp_reset_test ( data text ) ON COMMIT DELETE ROWS;
SELECT relname FROM pg_class WHERE relname = 'temp_reset_test';
DISCARD TEMP;
DROP table temp_reset_test;
SELECT relname FROM pg_class WHERE relname = 'temp_reset_test';
...
CREATE TABLE temp_tmp_foo (data text) ON COMMIT DELETE ROWS;
...
SHOW vacuum_cost_delay;
SELECT relname from pg_class where relname = 'temp_tmp_foo';
SELECT current_user = 'regress_guc_user';
-- discard everything
DISCARD ALL;
DROP table temp_tmp_foo;
-- look again
-- POLAR_TAG: REPLICA_IGNORE
SELECT pg_listening_channels();
SELECT name FROM pg_prepared_statements;
