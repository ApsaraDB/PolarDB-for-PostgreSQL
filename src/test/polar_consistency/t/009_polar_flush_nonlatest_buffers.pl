...
my @replicas = ($node_standby);
$regress->replay_check_all(\@replicas, "dccheck");

# tell standby's logindex bg writer to skip replay
$res = $node_standby->safe_psql('postgres',
	"select injection_points_attach('polar_logindex_bg_skip_replay', 'notice')"
);
print "attach logindex bg skip replay fault inject:$res\n";
...
$res = $node_standby->safe_psql('postgres',
	"select injection_points_detach('polar_logindex_bg_skip_replay')");
print "detach logindex bg skip replay fault inject:$res\n";
