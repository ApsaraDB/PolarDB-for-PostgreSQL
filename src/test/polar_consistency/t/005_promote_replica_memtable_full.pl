...
# push forward the logindex's max_idx_table_id bigger than zero
$node_primary->start;

$node_primary->polar_create_slot($node_replica->name);
$node_replica->start;

# create injection_points extension
$node_primary->safe_psql('postgres', 'create extension injection_points;');
# enable 'test_skip_write_logindex'
my $result = $node_primary->safe_psql('postgres',
	"select injection_points_attach('test_skip_write_logindex', 'notice');");
print "attach fault inject: $result\n";
# attach 'test_ignore_ro_latency'
# in case the whole buffer pool is dirty after replica promotion with old replica's slot
$result = $node_primary->safe_psql('postgres',
	"select injection_points_attach('test_ignore_ro_latency', 'notice');");
print "attach fault inject: $result\n";

# sigstop the replica's logindex background worker
$node_replica->stop_child('logindex background worker');
my $out_loop = 600;
my $in_loop = 30;
...
$node_primary->stop('i');
# sigcont the replica's logindex background worker
$node_replica->resume_child('logindex background worker');
