# 104_default_off_gucs_consistent_test.pl
#	  Test case: one primary node, one replica node and one standby node, enable gucs those default value is off.
#
...
#
# IDENTIFICATION
#	  src/test/polar_consistency/t/104_default_off_gucs_consistent_test.pl
use strict;
use warnings;
...
foreach my $schedule (@prepared_schedules)
{
	...
	# enable gucs those default value is off
	$node_primary->append_conf('postgresql.conf',
		'polar_enable_strategy_reject_buffer=on');
	$node_primary->append_conf('postgresql.conf', 'full_page_writes=on');
	$node_replica1->append_conf('postgresql.conf',
		'polar_enable_strategy_reject_buffer=on');
	$node_replica1->append_conf('postgresql.conf', 'full_page_writes=on');
	$node_standby1->append_conf('postgresql.conf',
		'polar_enable_strategy_reject_buffer=on');
	$node_standby1->append_conf('postgresql.conf', 'full_page_writes=on');
	...
	ok($failed == 0, "Polar regression test for [$schedule]");
	...
	if ($failed)
	{
		...
		sleep(86400);
		die "Polar regression test for [$schedule]";
	}
