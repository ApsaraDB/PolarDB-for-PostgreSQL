...
my $ret = $node_primary->psql_execute('select pg_backend_pid();',
	$wait_timeout, $psql);
my $backend = $ret->{_stdout};
$backend =~ s/^\s+|\s+$//g;
print "primary backend pid: $backend\n";
die "Unexpected backend pid $backend"
  if (!defined $backend || $backend eq 0 || $backend eq "");
