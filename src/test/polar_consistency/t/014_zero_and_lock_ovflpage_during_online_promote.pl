...
# attach fault inject in ro
$res = $node_replica->safe_psql('postgres',
	"select injection_points_attach('polar_delay_wal_replay', 'wait');");
print "attach delay wal replay fault inject:$res\n";
...
$res = $node_replica->safe_psql('postgres',
	"select injection_points_wakeup('polar_delay_wal_replay')");
print "detach delay wal replay:$res\n";
