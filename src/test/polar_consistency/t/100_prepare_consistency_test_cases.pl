...
{
	my ($all) = @_;
	my @excluded_test_cases = (
		'polar_cluster_settings', 'collate.windows.win1252',
		'tablespace', 'event_trigger_login',
		# which use package which may report outdated error in RO
		'polar_unpexted_query_list_in_function_out', 'polar_package_synonym',
		# which change guc in global mode to make bulk extend in different size
		'polar_index_bulk_extend_for_coverage',
		# which contains polar_parted_column_consistency_check to make deadlock in RO
