# 013_ro_missing_some_wal_before_promote.pl
#	  Test promote replica when it hasn't read all the WAL.
#
# Copyright (c) 2024, Alibaba Group Holding Limited
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# IDENTIFICATION
#	  src/test/polar_consistency/t/013_ro_missing_some_wal_before_promote.pl

use strict;
use warnings;
use PostgreSQL::Test::Cluster;
use PostgreSQL::Test::Utils;
use Test::More;

if ($ENV{enable_injection_points} eq 'no')
{
	plan skip_all => 'Fault injector not supported by this build';
}

my $db = 'postgres';

my $node_primary = PostgreSQL::Test::Cluster->new('primary');
$node_primary->polar_init_primary;
$node_primary->append_conf('postgresql.conf', 'synchronous_commit=on');
$node_primary->append_conf('postgresql.conf',
	'polar_table_multi_insert_min_rows=0');

my $node_replica = PostgreSQL::Test::Cluster->new('replica1');
$node_replica->polar_init_replica($node_primary);
$node_replica->append_conf('postgresql.conf', 'logging_collector=off');
$node_replica->append_conf('postgresql.conf',
	'polar_table_multi_insert_min_rows=0');

$node_primary->start;

$node_primary->safe_psql($db, 'create extension injection_points;');
$node_primary->safe_psql($db, 'create table test(id int);');
$node_primary->safe_psql($db, 'insert into test values(1);');

$node_replica->start;

# use fault injector to force make lazy checkpoint after promote
my $result = $node_replica->safe_psql($db,
	"select injection_points_attach('test_wait_for_promote_signal', 'notice');"
);
print "attach fault inject: $result\n";

my $pid = $node_replica->find_child('startup');
BAIL_OUT('startup process is gone!') if $pid eq 0;

my ($time, $out, $err);
for ($time = 0; $time < 60; $time = $time + 1)
{
	($out, $err) = $node_replica->run_command([ 'pstack', $pid ]);
	note "stdout: $out\nstderr: $err\n";
	# make sure that target process is stopped at right stack
	if ($out =~ /pg_usleep/)
	{
		note "startup process ($pid)'s stack is expected.\n";
		last;
	}
	my $walsender_pid = $node_primary->find_child('walsender');
	note "try to terminate walsender: $walsender_pid\n";
	if ($walsender_pid != 0)
	{
		PostgreSQL::Test::Utils::system_or_bail('kill', '-SIGTERM',
			$walsender_pid);
	}
	sleep 1;
}

BAIL_OUT("startup process ($pid)'s stack is unexpected.")
  if ($out !~ /pg_usleep/);

$node_primary->safe_psql($db, 'insert into test values(2);');
$node_primary->stop('i');

$node_replica->promote_async;

# wait for replica to shutdown
for ($time = 0; $time < 60; $time = $time + 1)
{
	if (kill(0, $node_replica->{_pid}) != 0)
	{
		sleep 1;
	}
	else
	{
		last;
	}
}

if (kill(0, $node_replica->{_pid}) != 0)
{
	BAIL_OUT("replica does not shutdown in $time seconds\n");
}
else
{
	print "replica shutdown after $time seconds\n";
}

my $panic_log = qr/is not the last valid record's EndRecPtr/;
my $log = PostgreSQL::Test::Utils::slurp_file($node_replica->logfile, 0);
like($log, $panic_log, "found panic log");

$node_replica->_update_pid(-1);
$node_replica->start;
# wait for replica to do crash recovery
$node_replica->polar_wait_for_startup(30);

$node_replica->promote;
# wait for new primary to startup
$node_replica->polar_wait_for_startup(30);

$result = $node_replica->safe_psql($db, "select count(*) from test;");
chomp($result);
ok($result == 2, "Check data consistency for promoted replica");

$node_replica->stop;

my $node_primary2 = PostgreSQL::Test::Cluster->new('primary2');
$node_primary2->polar_init_primary(extra => ['--wal-segsize=1']);
$node_primary2->append_conf('postgresql.conf', 'synchronous_commit=on');
$node_primary2->append_conf('postgresql.conf',
	'polar_table_multi_insert_min_rows=0');

$node_primary2->start;
$node_primary2->safe_psql($db, 'create table test(id int);');
$node_primary2->stop_child('checkpointer');

my $start_lsn = $node_primary2->lsn('insert');
local %ENV = $node_primary2->_get_env();

my $target_wal_file = '';

my $i;
for ($i = 0; $i < 1024; $i++)
{
	$node_primary2->safe_psql($db,
		"insert into test select generate_series(1, 10000);");
	my $cur_wal_file = $node_primary2->safe_psql($db,
		"select pg_walfile_name(pg_current_wal_lsn());");
	my $stdout;

	run_log(
		[
			'pg_waldump', '-p', $node_primary2->polar_get_datadir . '/pg_wal',
			$cur_wal_file, '-n', '1'
		],
		'>',
		\$stdout);

	print "[$i]: $stdout";
	if ($stdout =~ /^rmgr:.*,\s*lsn:\s*([0-9A-F\/]+),\s*/)
	{
		my $first_lsn = $1;
		my @arr = split('/', $first_lsn);
		print "first wal record lsn: $first_lsn, $arr[1]\n";
		if (hex($arr[1]) % 8192 == 0x28)
		{
			$target_wal_file = $cur_wal_file;
			last;
		}
	}
}

# zero the whole file to avoid blocks with bigger lsn than $first_lsn in storage.
my $target_db_oid = $node_primary2->safe_psql($db,
	"select oid from pg_database where datname = '$db';");
my $target_relfilenode = $node_primary2->safe_psql($db,
	"select relfilenode from pg_class where relname = 'test';");
my $target_file = $node_primary2->polar_get_datadir
  . "/base/$target_db_oid/$target_relfilenode";

$node_primary2->stop('i');

my $target_file_size = -s $target_file;
PostgreSQL::Test::Utils::system_or_bail('truncate', '-s', 0, $target_file);
PostgreSQL::Test::Utils::system_or_bail('truncate', '-s', $target_file_size,
	$target_file);

print "remove target wal file $target_wal_file\n";
PostgreSQL::Test::Utils::system_or_bail('rm', '-f',
	$node_primary2->polar_get_datadir . '/pg_wal/' . $target_wal_file);

my $node_replica2 = PostgreSQL::Test::Cluster->new('replica2');
$node_replica2->polar_init_replica($node_primary2,
	extra => ['--wal-segsize=1']);
$node_replica2->append_conf('postgresql.conf', 'restart_after_crash=off');
$node_replica2->append_conf('postgresql.conf',
	'polar_table_multi_insert_min_rows=0');

$node_replica2->start;

$node_replica2->promote();
$node_replica2->polar_wait_for_startup(30);

$result = $node_replica2->safe_psql($db, "select count(*) from test;");
chomp($result);
ok($result >= 10000 * $i,
	"Check data consistency for promoted replica2: $result");

# immediate shutdown to avoid flush buffer whose lsn is in removed wal file
$node_replica2->stop('i');

# cleanup nodes to avoid checksums for them
$node_primary2->clean_node;
$node_replica2->clean_node;

done_testing();
