...
#define LOGINDEX_SNAPSHOT_NAME "test_logindex_snapshot"

static pid_t bgwriter_pid = 0;
static BackgroundWorkerHandle *bgwriter_handle;
static bool shutdown_requested = false;

static log_index_snapshot test_logindex_snapshot = NULL;
...
static log_index_table_flushable_hook_type prev_flushable_hook = NULL;

PGDLLEXPORT void test_logindex_bgwriter_main(Datum main_arg);

static bool
test_logindex_table_flushable(log_index_snapshot logindex_snapshot, log_index_mem_table table)
{
	if (strcmp(logindex_snapshot->dir, LOGINDEX_SNAPSHOT_NAME) == 0)
		return true;
	else if (prev_flushable_hook)
		return prev_flushable_hook(logindex_snapshot, table);
	else
		return polar_logindex_table_flushable(logindex_snapshot, table);
}

static void
test_insert_lsn_to_mem(log_index_snapshot logindex_snapshot, BufferTag *tag, XLogRecPtr max_lsn, uint32 key)
{
...
static void
test_insert_one_mem_table_full(log_index_snapshot logindex_snapshot, BufferTag *tag)
{
...
static void
test_lsn_iterator(log_index_snapshot logindex_snapshot, log_index_lsn_iter iter, XLogRecPtr start, XLogRecPtr end)
{
	XLogRecPtr	i;
...
static void
test_lsn_iterate_from_invalid(log_index_snapshot logindex_snapshot)
{
	log_index_lsn_iter lsn_iter = polar_logindex_create_lsn_iterator(logindex_snapshot, InvalidXLogRecPtr);
...
static void
test_lsn_iterate_from_super_max(log_index_snapshot logindex_snapshot)
{
	log_index_lsn_info lsn_info;

	log_index_lsn_iter lsn_iter = polar_logindex_create_lsn_iterator(logindex_snapshot, test_max_lsn + 1);
...
static void
test_lsn_iterate_from_table_interval(log_index_snapshot logindex_snapshot)
{
	log_index_lsn_info lsn_info;
	BufferTag	tag;
...
static void
test_lsn_iterate_from_file(log_index_snapshot logindex_snapshot, XLogRecPtr start)
{
	log_index_lsn_iter lsn_iter = polar_logindex_create_lsn_iterator(logindex_snapshot, start);
...
static void
test_insert_lsn_after_force_flush(log_index_snapshot logindex_snapshot)
{
	BufferTag	tag;
	uint32		total = LOG_INDEX_MEM_TBL_SEG_NUM * LOG_INDEX_ITEM_SEG_LSN_NUM;
	log_index_lsn_iter lsn_iter = polar_logindex_create_lsn_iterator(logindex_snapshot, test_max_lsn + LSN_TEST_STEP);
	XLogRecPtr	plsn = InvalidXLogRecPtr;
	log_index_mem_table table = LOG_INDEX_MEM_TBL_ACTIVE();
	log_index_mem_table next_table;
...
static void
test_insert_file_full(log_index_snapshot logindex_snapshot)
{
	BufferTag	tag;
	uint32		total = LOG_INDEX_MEM_TBL_SEG_NUM * LOG_INDEX_ITEM_SEG_LSN_NUM;
	int			i,
...
static void
test_insert_lsn(log_index_snapshot logindex_snapshot)
{
	BufferTag	tag;
...
static void
test_iterate_non_exists_block_lsn(log_index_snapshot logindex_snapshot, BlockNumber block)
{
	log_index_page_iter iter;
	BufferTag	tag;
...
static void
test_iterate_in_table0(log_index_snapshot logindex_snapshot)
{
	BufferTag	tag;
	log_index_page_iter iter;
	int			i = LSN_TEST_STEP;
...
static void
test_iterate_release(log_index_snapshot logindex_snapshot)
{
	BufferTag	tag;
	log_index_page_iter iter;
...
static void
test_iterate_only_two_lsn(log_index_snapshot logindex_snapshot)
{
	BufferTag	tag;
	log_index_page_iter iter;
	log_index_lsn_info lsn_info;
...
static void
test_iterate_lsn(log_index_snapshot logindex_snapshot)
{
...
static void
test_save_memtable(log_index_snapshot logindex_snapshot)
{
	sleep(1);
...
static void
test_force_flush_table(log_index_snapshot logindex_snapshot)
{
...
static void
test_check_saved(log_index_snapshot logindex_snapshot, BufferTag *tag)
{
	log_index_mem_table table;
...
static void
test_iterate_after_truncate(log_index_snapshot logindex_snapshot, XLogRecPtr start_lsn)
{
	BufferTag	tag;
	log_index_page_iter iter;
	log_index_lsn_info lsn_info;
...
static void
test_truncate_log(log_index_snapshot logindex_snapshot)
{
	int			i;
...
static void
test_change_lsn_prefix(log_index_snapshot logindex_snapshot)
{
	log_index_mem_table table,
				next_table;
	BufferTag	tag;
...
test_logindex_bgwriter_main(Datum main_arg)
{
	...
	logindex_snapshot = polar_logindex_snapshot_shmem_init(LOGINDEX_SNAPSHOT_NAME, 3, 32,
...
static void
test_force_flush_full_table(log_index_snapshot logindex_snapshot, BufferTag *tag)
{
	log_index_mem_table table;
	LWLock	   *lock;
...
{
	XLogRecPtr	start_lsn;
	log_index_snapshot logindex_snapshot = test_logindex_snapshot;
	...
	MemoryContextReset(polar_logindex_memory_context());
...
test_logindex_shmem_startup(void)
{
	...
	test_logindex_snapshot =
		polar_logindex_snapshot_shmem_init(LOGINDEX_SNAPSHOT_NAME, 3, 32,
										   polar_wal_logindex_init_lwlock_array,
										   LWTRANCHE_WAL_LOGINDEX_BLOOM_LRU_BUFFER, LWTRANCHE_WAL_LOGINDEX_BLOOM_LRU);
	Assert(test_logindex_snapshot);

	prev_flushable_hook = polar_logindex_table_flushable_hook;
	polar_logindex_table_flushable_hook = test_logindex_table_flushable;
}

PG_FUNCTION_INFO_V1(test_get_polar_parallel_replay_mode);
Datum
test_get_polar_parallel_replay_mode(PG_FUNCTION_ARGS)
{
	char		res[1024];

	snprintf(res, 1024, "primary:0x%X, pitr:0x%X, standby:0x%X",
