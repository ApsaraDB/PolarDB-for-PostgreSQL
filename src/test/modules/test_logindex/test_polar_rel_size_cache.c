 ...
 *
 * IDENTIFICATION
 *	  src/test/modules/test_logindex/test_polar_rel_size_cache.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
test_rel_exists(polar_rel_size_cache_t cache)
{
	...
	polar_record_db_state_with_lock(cache, 0x5000, rlocator.spcOid, rlocator.dbOid, POLAR_DB_DROPED);
	...
	polar_record_db_state_with_lock(cache, 0x5100, rlocator.spcOid, rlocator.dbOid, POLAR_DB_NEW);
	...
	polar_record_rel_size_with_lock(cache, 0x5200, &rlocator, MAIN_FORKNUM, 100);
	tag.blockNum = 200;
	valid = polar_check_rel_block_valid_and_lsn(cache, 0x5110, &tag, &lsn_changed);
	Assert(!valid);
	...
	/*
	 * The block is valid if we didn't add infomation for this relation after
	 * 0x6000
	 */
	...
	rlocator.dbOid = 1000;
	InitBufferTag(&tag, &rlocator, tag.forkNum, tag.blockNum);
	polar_record_db_state_with_lock(cache, 0x6100, rlocator.spcOid, rlocator.dbOid, POLAR_DB_NEW);
	...
	polar_record_rel_size_with_lock(cache, 0x6200, &rlocator, MAIN_FORKNUM, 100);
...
test_rel_size_table_full(polar_rel_size_cache_t cache)
{
	...
	RelFileLocator rlocator;
	int			i;
	XLogRecPtr	lsn = 0x7000,
				lsn_changed;
	BlockNumber size = 100000,
				size101,
				size102;
	...
	for (i = 1; i <= total; i++)
	{
		lsn += i * 10;
		size -= i;
		polar_record_rel_size_with_lock(cache, lsn, &rlocator, MAIN_FORKNUM, size);
	}
	...
	rlocator.dbOid = 100;
	InitBufferTag(&tag, &rlocator, tag.forkNum, 200);
	...
	rlocator.dbOid = 101;
	InitBufferTag(&tag, &rlocator, tag.forkNum, size101 - 1);
	valid = polar_check_rel_block_valid_only(cache, 0x4000, &tag);
	Assert(valid);
	...
	rlocator.dbOid = 101;
	InitBufferTag(&tag, &rlocator, tag.forkNum, size101);
	valid = polar_check_rel_block_valid_and_lsn(cache, 0x4000, &tag, &lsn_changed);
	Assert(!valid);
	Assert(lsn_changed == max_tid2_lsn);
	...
	polar_truncate_rel_size_cache(cache, max_tid2_lsn - 1);
	rlocator.dbOid = 102;
	InitBufferTag(&tag, &rlocator, tag.forkNum, size102);
...
test_polar_rel_size_cache(PG_FUNCTION_ARGS)
{
	...
	cache = polar_rel_size_shmem_init("test_rel_cache", MAX_BLOCKS);
	Assert(cache != NULL);
...
Size
test_rel_size_cache_request_shmem_size(void)
{
	return polar_rel_size_shmem_size(MAX_BLOCKS);
}

void
test_rel_size_cache_shmem_startup(void)
{
	polar_rel_size_cache_t cache;
