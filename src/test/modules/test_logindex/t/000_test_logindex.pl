...
## instant recovery parameters
$node_primary->append_conf(
	'postgresql.conf', q[
	polar_primary_parallel_replay_mode='recovery'
	polar_pitr_parallel_replay_mode='instant_recovery'
	polar_standby_parallel_replay_mode='async_instant_recovery'
]);
$node_primary->start;
$ret = $node_primary->safe_psql($regress_db,
	'select * from test_get_polar_parallel_replay_mode();');
note "ret is $ret";
is($ret, 'primary:0x1, pitr:0x3, standby:0x7');

$node_primary->stop;
$node_primary->append_conf(
	'postgresql.conf', q[
	polar_primary_parallel_replay_mode='disable'
	polar_pitr_parallel_replay_mode='enable'
	polar_standby_parallel_replay_mode=''
]);
$node_primary->start;
$ret = $node_primary->safe_psql($regress_db,
	'select * from test_get_polar_parallel_replay_mode();');
note "ret is $ret";
is($ret, 'primary:0x0, pitr:0x3, standby:0x0');
