...
#define SHARED_TEST_DIR "local_cache_shared_dir"
...
void test_local_cache_worker_main(Datum main_arg);
...
test_prepare_dir(void)
{
	...
	polar_datadir = SHARED_TEST_DIR;

	Assert(polar_make_pg_directory(LOCAL_TEST_DIR) == 0);
	Assert(polar_make_pg_directory(polar_datadir) == 0);

	snprintf(path, MAXPGPATH, "%s/%s", polar_datadir, LOCAL_TEST_DIR);
	Assert(polar_make_pg_directory(path) == 0);
}
...
test_local_cache_write_worker(Datum main_arg)
{
	...
	Assert(found == true);
	polar_local_cache_attach_hashmap(test_meta->cache);
...
test_local_cache_dir_empty(const char *dirname)
{
	...
	dir = AllocateDir(dirname, true);

	if (dir == NULL)
		elog(PANIC, "Could not open dir %s, %d", dirname, errno);
...
{
	int max_segment = PG_GETARG_INT32(0);
	polar_local_cache cache;
	...
	cache = polar_create_local_cache("test_local_cache", LOCAL_TEST_DIR,
									 max_segment, BLOCK_SIZE * BLOCK_PER_SEGMENT, LWTRANCHE_FIRST_USER_DEFINED,
									 io_permission, NULL);
	if (cache)
		polar_local_cache_move_trash(cache->dir_name);

	test_local_cache_io(cache, max_segment, io_permission);
