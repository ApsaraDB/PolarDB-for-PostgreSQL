...
SELECT cron.schedule(repeat('a', 1000), '1');
...
SELECT cron.schedule_in_database(job_name:='can not connect', schedule:='0 11 * * *', command:='VACUUM',database:='pgcron_dbno');
...
SELECT cron.schedule_in_database(job_name:='user does not exist', schedule:='0 11 * * *', command:='VACUUM',database:='pgcron_dbyes');

-- Alter an existing job on a database that does not accept connections
SELECT cron.alter_job(job_id:=2,database:='pgcron_dbno');
...
SELECT cron.schedule_in_database(job_name:='his vacuum', schedule:='0 11 * * *', command:='VACUUM',database:=current_database());
...
select cron.alter_job(job_id:=8,database:='pgcron_dbyes');
SELECT count(database) FROM cron.job;

-- change the database for a job that the user own but can not connect to
select cron.alter_job(job_id:=8,database:='pgcron_dbno');
SELECT count(database) FROM cron.job;
