...
_PG_init(void)
{
	...
	if (!UseBackgroundWorkers)
		DefineCustomIntVariable(
			...
			MaxConnections,
			PGC_POSTMASTER,
			GUC_SUPERUSER_ONLY | POLAR_GUC_IS_VISIBLE | POLAR_GUC_IS_CHANGEABLE,
			NULL, NULL, NULL);
	else
		DefineCustomIntVariable(
			"cron.max_running_jobs",
...
PgCronLauncherMain(Datum arg)
{
	...
	while (!got_sigterm)
	{
		...
		/*
		 * POLAR: To ensure effective synchronization of dynamic shared memory,
		 * a Barriers synchronization mechanism has been implemented.
		 * We put CHECK_FOR_INTERRUPTS() in process's main loop to process Barriers.
		 */
	...
	switch (checkState)
	{
		...
		case CRON_TASK_START:
		{
			...
			if (!UseBackgroundWorkers)
			{
				...
				char DataDirString[MAXPGPATH] = {0};
				...
				/*
				 * Always use the HOST and PORT of the current database as
				 * the HOST and PORT for tasks to prevent task execution
				 * failures due to port changes.
				 */
				if (CronHost && strlen(CronHost) > 0)
				{
					/* if CronHost is configured */
					sprintf(DataDirString, "%s", CronHost);
				}
				else if (!strncmp(polar_unquote_unix_socket_directories, "/", 1))
				{
					/* Unix_socket_directories is Absolute Path */
					sprintf(DataDirString, "%s", polar_unquote_unix_socket_directories);
				}
				else
				{
					/* Unix_socket_directories is Relative Path */
					sprintf(DataDirString, "%s/%s", DataDir, polar_unquote_unix_socket_directories);
				}

				sprintf(nodePortString, "%d", PostPortNumber);
