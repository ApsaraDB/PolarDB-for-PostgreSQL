...
/* POLAR: place it here as ConditionVariableMinimallyPadded to solve conflcts */
...
#define EXTENSION_EXT_NAME "pg_cron_preload"
...
char *CronHost = "";
bool EnableSuperuserJobs = false;
					...
					text *usernameText, bool active, text *jobnameText)
{
	...
	argTypes[2] = TEXTOID;
	argValues[2] = CStringGetTextDatum(DataDirString);
	argCount++;
...
CronExtensionOwner(void)
{
	...
	ScanKeyInit(&entry[0],
				...
				CStringGetDatum(EXTENSION_EXT_NAME));
