...
select polar_get_flushlist_head_bufferid() >= -2 As result;

select polar_get_startup_bufferpin_wait_bufid() >= -1 As result;

select COUNT(*) = 1 AS result from polar_get_normal_buffercache_pages(0);
...
-- fullpage snapshot stat
select COUNT(polar_get_fullpage_stat()) >= 0 As result;
select polar_used_logindex_fullpage_snapshot_mem_tbl_size() >= 0 As result;

select count(*) >= 0 AS result from polar_stat_proxy_info;

select polar_stat_reset_proxy_info();

-- coverage only
SELECT 1 AS dummy FROM polar_wal_buffer_stat_delta;
