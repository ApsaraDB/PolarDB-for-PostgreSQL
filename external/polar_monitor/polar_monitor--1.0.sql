...
-- Register the buffer monitor function.
...
CREATE OR REPLACE FUNCTION polar_get_normal_buffercache_pages(
    IN  buf_id int4,
    OUT buffer_id int4,
    OUT relfilenode oid,
    ...
    OUT buffer_state int8,
    OUT isvalid bool,
    OUT isdirty bool,
    OUT usage_count int2,
    OUT pinning_backends int4,
    OUT oldest_lsn pg_lsn,
    OUT newest_lsn pg_lsn,
    OUT flush_next int4,
    OUT flush_prev int4,
    OUT in_copybuf bool,
    OUT first_touched_after_copy bool,
    OUT recently_modified_count int2,
    OUT oldest_lsn_is_fake bool,
    OUT redo_state int8,
    OUT content_lock_owner int4,
    OUT wait_backend_pid int4)
RETURNS SETOF RECORD
AS 'MODULE_PATHNAME', 'polar_get_normal_buffercache_pages'
LANGUAGE C PARALLEL SAFE;

-- Create a view for normal buffer convenient access.
CREATE OR REPLACE VIEW polar_normal_buffercache AS
        SELECT * FROM polar_get_normal_buffercache_pages(-1);
...
CREATE FUNCTION polar_get_flushlist_head_bufferid(
    OUT buffer_id int4)
RETURNS int4
AS 'MODULE_PATHNAME', 'polar_get_flushlist_head_bufferid'
LANGUAGE C PARALLEL SAFE;

CREATE FUNCTION polar_get_startup_bufferpin_wait_bufid(
    OUT buffer_id int4)
RETURNS int4
AS 'MODULE_PATHNAME', 'polar_get_startup_bufferpin_wait_bufid'
LANGUAGE C PARALLEL SAFE;
...
CREATE FUNCTION polar_mark_buffer_dirty_hint(text, int4)
RETURNS VOID
AS 'MODULE_PATHNAME', 'polar_mark_buffer_dirty_hint'
...
--
-- WAL sender WAL read stats
--
CREATE FUNCTION polar_stat_walsnd_xlog_read(
    OUT pid int4,
    OUT hit int8,
    OUT hit_bytes int8,
    OUT prefetched int8,
    OUT prefetched_bytes int8,
    OUT read int8,
    OUT read_bytes int8
)
RETURNS SETOF RECORD
AS 'MODULE_PATHNAME', 'polar_monitor_stat_wal_sender_xlog_read'
LANGUAGE C;

CREATE FUNCTION polar_stat_walsnd_reset()
RETURNS VOID
AS 'MODULE_PATHNAME', 'polar_monitor_stat_wal_sender_reset'
LANGUAGE C;
...
CREATE FUNCTION polar_get_fullpage_stat(OUT write_fullpage_count int8,
                                    OUT read_oldpage_count int8,
                                    OUT insert_fullpage_lsn pg_lsn,
                                    OUT flush_fullpage_lsn pg_lsn,
                                    OUT read_fullpage_count int8,
                                    OUT restore_oldpage_count int8,
                                    OUT restore_oldpage_rate float4,
                                    OUT fullpage_bgworker_replay_lsn pg_lsn,
                                    OUT load_fullpage_lsn pg_lsn)
RETURNS record
AS 'MODULE_PATHNAME', 'polar_get_fullpage_stat'
LANGUAGE C PARALLEL SAFE;
