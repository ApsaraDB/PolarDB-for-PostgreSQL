...
StaticAssertDecl(lengthof(polar_io_loc_names) == POLAR_IO_LOC_SIZE,
...
StaticAssertDecl(lengthof(polar_dir_type_names) == POLAR_IO_TYPE_SIZE,
...
#define PG_STAT_GET_POLAR_PROCESS_COLS	20
	...
	for (curr_backend = 1; curr_backend <= num_backends; curr_backend++)
	{
		...
		/* IO inof 6~11 */
		set_polar_proc_iostat(beentry->backendid + 1, values, nulls);
...
set_polar_proc_iostat(int backendid, Datum *values, bool *nulls)
{
	...
	if (polar_io_stat_array)
	{
		...
		{
			local_read_ps += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_LOCAL].io_number_read;
			local_write_ps += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_LOCAL].io_number_write;
			local_read_throughput += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_LOCAL].io_throughtput_read;
			local_write_throughput += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_LOCAL].io_throughtput_write;
			INSTR_TIME_ADD(local_read_latency, polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_LOCAL].io_latency_read);
			INSTR_TIME_ADD(local_write_latency, polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_LOCAL].io_latency_write);
...
/*
 * return the IO stat info ever file type
 */
...
#define POLARIOSTATSIZE 20
	...
	polar_proc_io_stat(*cur_polar_proc_io_stat)[POLAR_IO_LOC_SIZE];
	...
	/* 1-based index */
	for (curr_backend = 0; curr_backend < POLAR_MAX_PROC_IO_STAT_SLOTS; curr_backend++)
	{
		int			cur_pid = 0;

		/* Get the next one in the list */
		cur_pid = curr_backend == 0 ? 0 : polar_io_stat_array[curr_backend].pid;
		if (curr_backend > 0 && cur_pid <= 0)
			continue;

		cur_polar_proc_io_stat = polar_io_stat_array[curr_backend].polar_proc_io_stat_dist;
		for (index = 0; index < POLAR_IO_TYPE_SIZE; index++)
		{
			/* for each row */
			...
			/* open_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_open_num);
			/* open_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_open_time));
			/* close_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_close_num);
			/* read_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_number_read);
			/* write_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_number_write);
			/* read_throughput */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_throughtput_read);
			/* write_throughput */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_throughtput_write);
			/* read_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_latency_read));
			/* write_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_latency_write));
			/* seek_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_seek_count);
			/* seek_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_seek_time));
			/* creat_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_creat_count);
			/* creat_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_creat_time));
			/* fsync_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_fsync_count);
			/* fsync_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_fsync_time));
			/* falloc_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_falloc_count);
			/* falloc_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_SHARED].io_falloc_time));
			tuplestore_putvalues(rsinfo->setResult, rsinfo->setDesc, values, nulls);
			/* no cover end */
			...
			/* open_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_open_num);
			/* open_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_open_time));
			/* close_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_close_num);
			/* read_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_number_read);
			/* write_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_number_write);
			/* read_throughput */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_throughtput_read);
			/* write_throughput */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_throughtput_write);
			/* read_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_latency_read));
			/* write_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_latency_write));
			/* seek_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_seek_count);
			/* seek_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_seek_time));
			/* creat_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_creat_count);
			/* creat_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_creat_time));
			/* fsync_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_fsync_count);
			/* fsync_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_fsync_time));
			/* falloc_count */
			values[cols++] = Int64GetDatum(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_falloc_count);
			/* falloc_latency */
			values[cols++] = Float8GetDatum(INSTR_TIME_GET_MICROSEC(cur_polar_proc_io_stat[index][POLAR_IO_LOCAL].io_falloc_time));
			tuplestore_putvalues(rsinfo->setResult, rsinfo->setDesc, values, nulls);
...
polar_io_latency_info(PG_FUNCTION_ARGS)
{
	...
	for (curr_backend = 0; curr_backend < POLAR_MAX_PROC_IO_STAT_SLOTS; curr_backend++)
	{
		...
		cur_num_latency_dist = polar_io_stat_array[curr_backend].num_latency_dist;
		for (loc = 0; loc < POLAR_IO_LOC_SIZE; loc++)
		{
