...
typedef struct polar_login_history_entry
{
	...
	char		application[POLAR_LOGIN_HISTORY_STRING_LENGTH]; /* application of the
																 * last login */
	uint8		failcount;		/* number of failed logins since the last
								 * successful login */
...
polar_load_information(void)
{
	...
	for (int32 i = 0; i < num; i++)
	{
		...
		{
			/*
			 * There is no space in the hash table and some information will
			 * be lost
			 */
...
polar_update_login_history(bool login_success)
{
	...
	/*
	 * Only the postgres process records the login information during the
	 * login process
	 */
	...
	/*
	 * Avoid duplicate recording of information due to errors reported during
	 * the recording process. Therefore, the flag bit is set immediately to
	 * mark the end of the login process.
	 */
