...
/*
 * POLAR: In polarAuditMode mode, we don't need to print any information
 * in pgaudit format, just mark is_need_audit = true.
 * Then print by the original mechanism(print content, format, and time).
 * If it is not polarAuditMode mode, it follows the original pgaudit mechanism
 */
bool		polarAuditMode = true;
...
typedef struct
{
	...
	List	   *permInfos;		/* Permission info rows for each table
...
log_audit_event(AuditEventStackItem *stackItem)
{
	...
	switch (stackItem->auditEvent.logStmtLevel)
	{
		...
		case LOGSTMT_DDL:
			...
			{
					/*
					 * In the case of create and alter role or user mapping
					 * redact all text in the command after the password token
					 * for security.  This doesn't cover all possible cases
					 * where passwords can be leaked but should take care of
	 ...
	 *
	 * 1. The object was selected for audit logging (granted), or 2. The
	 * statement belongs to a class that is being logged
	 *
	...
	/*
	 * POLAR: In polarAuditMode mode, we don't need to print the PGAudit
	 * format info, just mark is_need_audit = true
	 */
	if (polarAuditMode)
	{
		stackItem->auditEvent.logged = true;
		polar_audit_log.is_need_audit = true;
		return;
	}
	...
	if (auditLogStatement && !(stackItem->auditEvent.statementLogged && auditLogStatementOnce))
	{
		...
		if (auditLogParameter)
		{
				 ...
				 paramIdx++)
			{
				...
				/*
				 * Append the param, suppressing long params if the
				 * appropriate GUC is set.
				 */
...
static bool
checkParamListHasNull(ParamListInfo from)
{
	int			i;
	...
	for (i = 0; i < from->numParams; i++)
	{
		...
		if (from->paramFetch != NULL)
			prm = from->paramFetch(from, i + 1, false, &prmdata);
		else
			prm = &from->params[i];
		...
		if (!typByVal && DatumGetPointer(prm->value) == NULL)
			return true;
	}
...
pgaudit_ExecutorStart_hook(QueryDesc *queryDesc, int eflags)
{
	...
	if (auditLogBitmap == 0 && (auditRole == NULL || strlen(auditRole) < 1))
	{
	...
	if (!internalStatement)
	{
		...
		if (!checkParamListHasNull(queryDesc->params))
...
{
	if ((auditRole != NULL && strlen(auditRole) > 0) || auditLogBitmap != 0)
	{
		Oid			auditOid;
			...
			!IsAbortedTransactionBlockState())
		{
			...
			if (auditLogRows && auditEventStack != NULL)
			{
					...
					auditEventStack->auditEvent.rangeTabls != NULL)
				{
					 ...
					 * The SELECT event for CREATE TABLE AS will be logged in
					 * pgaudit_ExecutorEnd_hook() later to get rows.
					 */
...
_PG_init(void)
{
	...
	/* Define pgaudit.polar_log */
	DefineCustomBoolVariable(
							 "pgaudit.polar_mode",

							 "When this function is enabled, the audit output content and format "
							 "is based on Polar only. All previous pgaudit formats are not printed.",
