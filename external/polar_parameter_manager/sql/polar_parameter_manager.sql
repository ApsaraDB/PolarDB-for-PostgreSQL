...
CREATE TABLE t1 (cur_val varchar, cur_is_visible int, cur_is_user_changable int);

-- alter system set should make parameter invisible and unchangeable
copy
(select 'echo ''temp_file_limit = -1'' >> ' || current_setting('data_directory') || '/postgresql.conf')
to '/tmp/set_conf';
\! sh /tmp/set_conf
\! rm /tmp/set_conf
SELECT name, setting_value, is_visible, is_user_changable FROM polar_parameter.parameter_infos where name = 'temp_file_limit';
INSERT INTO t1 SELECT setting_value, is_visible, is_user_changable FROM polar_parameter.parameter_infos where name = 'temp_file_limit';

ALTER SYSTEM set temp_file_limit to -1;
SELECT pg_reload_conf();
SELECT pg_sleep(1);

SELECT is_visible = cur_is_visible, is_user_changable = cur_is_user_changable FROM polar_parameter.parameter_infos, t1 where name = 'temp_file_limit';
