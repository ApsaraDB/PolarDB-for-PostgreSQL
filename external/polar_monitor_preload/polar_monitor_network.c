...
polar_network_stat_shmem_startup(void)
{
	...
	polar_network_stat_array = ShmemInitStruct("polar_network_stat_array",
											   total_size, &found);
	elog(DEBUG1, "polar_network_stat_array share memory total size is %d", (int) total_size);
	if (!found)
	{
		/* First time through ... */
...
#define POLAR_NETWORK_STAT_COLS 7
	...
	/* just one line now */
	{
		...
		{
			values[col++] = UInt64GetDatum(stat.opstat[index].count);
			values[col++] = UInt64GetDatum(stat.opstat[index].bytes);
			values[col++] = UInt64GetDatum(stat.opstat[index].block_time);
		}

		values[col++] = UInt64GetDatum(stat.total_retrans);
...
#define POLAR_PROC_NETWORK_STAT_COLS 14
	...
	for (curr = 1; curr <= num_backends; curr++)
	{
		...
		if (!CHECK_NET_STAT_BACKENDID_VALID(beentry->backendid))
			continue;
		...
		procstat = &polar_network_stat_array[beentry->backendid];

		/* pid */
		for (i = 0; i < POLAR_NETWORK_STAT_MAX; i++)
		{
			values[col++] = UInt64GetDatum(procstat->opstat[i].count);
			values[col++] = UInt64GetDatum(procstat->opstat[i].bytes);
			values[col++] = UInt64GetDatum(polar_network_real_block_time(&procstat->opstat[i]));
		}

		values[col++] = UInt64GetDatum(procstat->sendq);
		values[col++] = UInt64GetDatum(procstat->recvq);
		values[col++] = UInt64GetDatum(procstat->cwnd);
		values[col++] = UInt64GetDatum(procstat->rtt);
		values[col++] = UInt64GetDatum(procstat->rttvar);
		values[col++] = UInt64GetDatum(procstat->total_retrans);
		values[col++] = UInt64GetDatum(procstat->tcpinfo_update_time);
		tuplestore_putvalues(rsinfo->setResult, rsinfo->setDesc, values, nulls);
	}
