...
polar_lock_stat_shmem_startup(void)
{
	...
	polar_lwlocks_stat_array = ShmemInitStruct("polar_lwlocks_stat_array",
											   total_size, &found);
	elog(DEBUG1, "polar_lwlocks_stat_array share memory total size is %d", (int) total_size);
	if (!found)
	{
		/* First time through ... */
	...
	elog(DEBUG1, "polar_locks_stat_array share memory total size is %d", (int) total_size);
	if (!found)
	{
		/* First time through ... */
...
#define POLAR_LOCK_STAT_COLS 7
	...
	for (i = 0; i <= LOCKTAG_LAST_TYPE; i++)
	{
		for (j = AccessShareLock; j <= AccessExclusiveLock; j++)
		{
			...
			values[col++] = UInt64GetDatum(stat->lock_count);
			values[col++] = UInt64GetDatum(stat->block_count);
			values[col++] = UInt64GetDatum(stat->fastpath_count);
			values[col++] = UInt64GetDatum(stat->wait_time);
...
#define POLAR_PROC_LOCK_STAT_COLS 8
	...
	for (curr = 1; curr <= num_backends; curr++)
	{
		...
		LocalPgBackendStatus *local_beentry = pgstat_get_local_beentry_by_index(curr);

		if (!local_beentry)
			continue;

		beentry = &local_beentry->backendStatus;
		if (!CHECK_LOCK_STAT_BACKENDID_VALID(beentry->backendid))
			continue;

		procstat = &polar_locks_stat_array[beentry->backendid];
...
#define POLAR_LWLOCK_STAT_COLS 6
	...
	for (index = 1; index < total; index++)
	{
		...
		/* name */
		if (GetLWLockIdentifier(PG_WAIT_LWLOCK, index))
			values[col++] = CStringGetTextDatum(GetLWLockIdentifier(PG_WAIT_LWLOCK, index));
		else
			values[col++] = CStringGetTextDatum("unassigned");

		values[col++] = UInt64GetDatum(stat.lwlocks[index].sh_acquire_count);
		values[col++] = UInt64GetDatum(stat.lwlocks[index].ex_acquire_count);
		values[col++] = UInt64GetDatum(stat.lwlocks[index].block_count);
		values[col++] = UInt64GetDatum(stat.lwlocks[index].wait_time);
		tuplestore_putvalues(rsinfo->setResult, rsinfo->setDesc, values, nulls);
	}
...
#define POLAR_PROC_LWLOCK_STAT_COLS 7
	...
	for (curr = 1; curr <= num_backends; curr++)
	{
		...
		procstat = &polar_lwlocks_stat_array[beentry->backendid];
		...
		for (index = 0; index < total; index++)
		{
			...
			values[col++] = UInt64GetDatum(procstat->lwlocks[index].sh_acquire_count);
			values[col++] = UInt64GetDatum(procstat->lwlocks[index].ex_acquire_count);
			values[col++] = UInt64GetDatum(procstat->lwlocks[index].block_count);
			values[col++] = UInt64GetDatum(procstat->lwlocks[index].wait_time);
			tuplestore_putvalues(rsinfo->setResult, rsinfo->setDesc, values, nulls);
		}
	}
