...
DECLARE
	...
	after int;
BEGIN
	SELECT sh_acquire_count INTO STRICT before FROM polar_stat_lwlock WHERE name='ProcArrayLock';
...
SELECT COUNT(*)>0 res FROM polar_stat_cpu_backend;
SELECT COUNT(*)>0 res FROM polar_stat_io_backend;
SELECT COUNT(*)>0 res FROM polar_stat_network_backend;
