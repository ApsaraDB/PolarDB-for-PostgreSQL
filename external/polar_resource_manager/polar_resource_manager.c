...
PGDLLEXPORT void polar_resource_manager_main(Datum arg);
...
int			total_mem_limit_remain_size = 262144;
...
int			mem_release_policy = DEFAULT;
...
check_cgroupmem_prefix(char **newval, void **extra, GucSource source)
{
	...
	sprintf(polar_cgroup_mem_usage_path, "%s%s", rawname, CGROUPMEMUSAGEFILE);
...
polar_resource_manager_main(Datum arg)
{
	...
	/* Make polar_resource_manager recognizable in pg_stat_activity */
...
polar_check_mem_exceed(void)
{
	...
	Size		db_engine_mem_usage = 0;
	Size		ins_mem_usage = 0;
	Size		ins_rss = 0;
	Size		ins_mapped_file = 0;

	/* memory.usage_in_bytes */
	Size		cgroup_memory_usage = 0;
	...
	if (polar_get_ins_memoryusage(&cgroup_memory_usage) != 0)
	{
		elog(WARNING, "Failed to get the instance memory usage");
		return;
	}
	...
	db_engine_mem_usage = ins_rss + ins_mapped_file;
	ins_mem_usage = ins_rss + ins_mapped_file + pfsd_rss;

	/* Instance memory usage include pfsd. */
	ins_mem_usage = db_engine_mem_usage + pfsd_rss;

	/*
	 * The existence of an OS kernel bug can cause accounting leaks in
	 * mapped_file, leading to the resource manager mistakenly killing
	 * processes. Therefore, when the combined size of mapped_file and RSS
	 * exceeds memory usage, no restriction handling will be applied.
	 */
	if (db_engine_mem_usage > cgroup_memory_usage)
		return;
