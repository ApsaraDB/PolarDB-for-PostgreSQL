...
char		polar_cgroup_mem_usage_path[MAXPGPATH] = "";
...
/*
 * Get the instance memory limit and memory usage.
 * Parse a /sys/fs/cgroup/memory/memory.usage_in_bytes file.
 */
int
polar_get_ins_memoryusage(Size *usage)
{
	FILE	   *fd = NULL;
	...
	/* Get the memory.stat file handle */
	fd = fopen(polar_cgroup_mem_usage_path, "r");

	if (fd != NULL)
	{
		if (fscanf(fd, "%lu", (Size *) usage) != 1)
			errno = EINVAL;
