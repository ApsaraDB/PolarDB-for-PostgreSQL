...
static void polar_proc_io_stat_add(polar_proc_io_stat x[][POLAR_IO_LOC_SIZE], polar_proc_io_stat y[][POLAR_IO_LOC_SIZE]);
static void polar_num_latency_dist_add(uint64 x[][LATENCY_KIND_LEN][LATENCY_INTERVAL_LEN], uint64 y[][LATENCY_KIND_LEN][LATENCY_INTERVAL_LEN]);
static void print_current_process_io_info(int io_type, const char *pro_name, const char *polar_dir_type_name);
static void polar_io_location_register(int kind, int loc);
...
/* IO statistics collection functions */
...
/* polar_io_stat hooks for polar_vfs. */
...
_PG_init(void)
{
	...
	elog(LOG, "polar_io_stat init done");
}
...
pgis_shmem_request(void)
{
	...
	RequestAddinShmemSpace(mul_size(sizeof(polar_proc_io), POLAR_MAX_PROC_IO_STAT_SLOTS));
}
...
polar_io_stat_shmem_startup(void)
{
	...
	total_size = mul_size(sizeof(polar_proc_io), POLAR_MAX_PROC_IO_STAT_SLOTS);
	...
	polar_io_stat_array = ShmemInitStruct("polar_io_stat_array",
										  total_size, &found);

	elog(DEBUG1, "polar_io_stat_array share memory total size is %d", (int) total_size);
	...
	if (!found)
	{
		...
		/* First time through ... */
		memset(polar_io_stat_array, 0, total_size);
		/* POLAR: polar io stat register all kinds of vfs interfaces. */
		for (i = 0; i < POLAR_VFS_KIND_SIZE; i++)
			polar_kind_to_location[i] = -1;
		polar_io_location_register(POLAR_VFS_LOCAL_BIO, POLAR_IO_LOCAL);
		polar_io_location_register(POLAR_VFS_PFS, POLAR_IO_SHARED);
		polar_io_location_register(POLAR_VFS_LOCAL_DIO, POLAR_IO_SHARED);
	}
...
polar_io_stat_shmem_shutdown(int code, Datum arg)
{
	...
	if (!polar_io_stat_array)
		return;

	my_proc_index = POLAR_PROC_IO_STAT_BACKEND_INDEX();

	if (AmStartupProcess())
	{
		print_current_process_io_info(POLAR_IO_TYPE_SIZE, "startup process", "Total");
		print_current_process_io_info(POLAR_IO_WAL, "startup process", "WAL");
		print_current_process_io_info(POLAR_IO_DATA, "startup process", "DATA");
	}
	...
	if (my_proc_index > 0)
	{
		 ...
		 * consistency.
		 */
		polar_proc_io_stat_add(polar_io_stat_array[0].polar_proc_io_stat_dist, polar_io_stat_array[my_proc_index].polar_proc_io_stat_dist);
		polar_num_latency_dist_add(polar_io_stat_array[0].num_latency_dist, polar_io_stat_array[my_proc_index].num_latency_dist);
		MemSet(&polar_io_stat_array[my_proc_index], 0, sizeof(polar_proc_io));
	}
}

/* Add two POLARPROCIOSTATs*/
static void
polar_proc_io_stat_add(polar_proc_io_stat x[][POLAR_IO_LOC_SIZE], polar_proc_io_stat y[][POLAR_IO_LOC_SIZE])
{
	int			i;
	int			j;

	for (i = 0; i < POLAR_IO_TYPE_SIZE; i++)
		for (j = 0; j < POLAR_IO_LOC_SIZE; j++)
		{
...
/* Add two POLARPROCIOSTATs*/
static void
polar_num_latency_dist_add(uint64 x[][LATENCY_KIND_LEN][LATENCY_INTERVAL_LEN], uint64 y[][LATENCY_KIND_LEN][LATENCY_INTERVAL_LEN])
{
	int			i;
	int			j;
	int			k;

	for (i = 0; i < POLAR_IO_LOC_SIZE; i++)
...
{
	int			backendid = POLAR_PROC_IO_STAT_BACKEND_INDEX();
	bool		once = true;
	int			index = io_type;
	...
	uint64		vfs_open_count,
				...
				vfs_seek_count,
				vfs_fsync_count;
	polar_proc_io_stat *tmp_stat;

	if (backendid == -1)
		return;

	if (io_type > POLAR_IO_TYPE_SIZE || io_type < 0)
		return;
	...
	do
	{
		...
		for (i = 0; i < POLAR_IO_LOC_SIZE; i++)
		{
			tmp_stat = &polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][i];
	...
	} while (!once && (++index) < POLAR_IO_TYPE_SIZE);
...
static void
polar_io_location_register(int kind, int loc)
{
	if ((kind >= 0 && kind < POLAR_VFS_KIND_SIZE) &&
		(loc >= 0 && loc < POLAR_IO_LOC_SIZE))
		polar_kind_to_location[kind] = loc;
	else
		elog(ERROR, "polar_io_location_register error at "
			 "kind = %d, loc = %d, "
			 "POLAR_VFS_KIND_SIZE = %d, "
			 "POLAR_IO_LOC_SIZE = %d",
			 kind, loc, POLAR_VFS_KIND_SIZE, POLAR_IO_LOC_SIZE);
}

int
polar_io_kind_to_location(int kind)
{
	int			loc = -1;
 ...
 *
 * We need to check the result, when call this function.
 * The value of result, -1 means fail, 0 means succeed.
 */
int
polar_collect_io_stat(polar_collect_io_stat_t * io_stat_info, int backendid)
{
	int			index = 0;
	...
	/* check backendid */
	if (backendid < 0 || backendid >= POLAR_MAX_PROC_IO_STAT_SLOTS)
	{
		return -1;
	}
	...
	if (polar_io_stat_array)
	{
		for (index = 0; index < POLAR_IO_TYPE_SIZE; index++)
		{

			shared_read_ps += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_number_read;
			shared_write_ps += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_number_write;
			shared_read_throughput += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_throughtput_read;
			shared_write_throughput += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_throughtput_write;
			INSTR_TIME_ADD(shared_read_latency, polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_latency_read);
			INSTR_TIME_ADD(shared_write_latency, polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_latency_write);

			io_open_num += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_open_num;
			io_seek_count += polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_seek_count;

			INSTR_TIME_ADD(io_open_time, polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_open_time);
			INSTR_TIME_ADD(io_seek_time, polar_io_stat_array[backendid].polar_proc_io_stat_dist[index][POLAR_IO_SHARED].io_seek_time);
...
{
	if (polar_io_stat_array != NULL &&
		UsedShmemSegAddr != NULL)
	{
		int			index,
					loc;

		index = POLAR_PROC_IO_STAT_BACKEND_INDEX();
		if (index < 0)
			return;
		loc = polar_io_kind_to_location(vfdkind);
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
		{
			elog(WARNING, "polar io stat does not recognize that: kind = %d, loc = %d", vfdkind, loc);
			return;
		}
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_open_time, loc, LATENCY_open);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_open_num++;
		polar_io_stat_array[index].pid = MyProcPid;
	}
}
...
polar_stat_io_close_info(int vfdkind, int vfdtype)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_close_num++;
	}
}
...
polar_stat_io_read_info(int vfdkind, int vfdtype, ssize_t size)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_latency_read, loc, LATENCY_read);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_number_read++;
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_throughtput_read += size;
	}
}
...
polar_stat_io_write_info(int vfdkind, int vfdtype, ssize_t size)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_latency_write, loc, LATENCY_write);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_number_write++;
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_throughtput_write += size;
	}
}
...
polar_stat_io_seek_info(int vfdkind, int vfdtype)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_seek_time, loc, LATENCY_seek);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_seek_count++;
	}
}
...
polar_stat_io_creat_info(int vfdkind, int vfdtype)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_creat_time, loc, LATENCY_creat);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_creat_count++;
	}
}
...
polar_stat_io_falloc_info(int vfdkind, int vfdtype)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_falloc_time, loc, LATENCY_falloc);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_falloc_count++;
	}
}
...
polar_stat_io_fsync_info(int vfdkind, int vfdtype)
{
		...
		UsedShmemSegAddr != NULL)
	{
		...
		if (loc < 0 || loc >= POLAR_IO_LOC_SIZE)
			return;
		polar_vfs_timer_end_iostat(&polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_fsync_time, loc, LATENCY_fsync);
		polar_io_stat_array[index].polar_proc_io_stat_dist[vfdtype][loc].io_fsync_count++;
	}
}
...
{
	int			index = POLAR_PROC_IO_STAT_BACKEND_INDEX();
	int			interval;
	uint64		valus = 0;
	...
	if (polar_io_stat_array)
		polar_io_stat_array[index].num_latency_dist[loc][kind][interval]++;
}
...
polar_stat_env_init_hook(polar_vfs_ops ops)
{
	...
	if (polar_io_stat_array && index > 0 && UsedShmemSegAddr != NULL)
		polar_io_stat_array[index].pid = MyProcPid;
}
