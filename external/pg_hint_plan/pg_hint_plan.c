...
/* POLAR: Control whether append enforces parallel execution. */
static bool	polar_enable_force_append_turn_on_parallel = false;
/* POLAR END */
...
_PG_init(void)
{
	...
	DefineCustomBoolVariable("pg_hint_plan.polar_enable_force_append_turn_on_parallel",
							 "Forces the append node turn on parallel.",
							 NULL,
							 &polar_enable_force_append_turn_on_parallel,
							 false,
							 PGC_USERSET,
	...
	/*
	 * POLAR: Do not install hooks in single-user mode. The needs_fmgr_hook
	 * function attempts to look up the Oid of plpgsql using SysCache. However,
	 * during the initdb phase, this lookup fails, causing the initdb process
	 * to fail.
	 */
	if (MyBackendType != B_STANDALONE_BACKEND)
	{
		/* Install hooks. */
				 ...
				 bool using_parent_hint)
{
	...
	foreach (cell, rel->indexlist)
	{
		...
		/*
		 * POLAR: Ignore the hypothetical index and remove it directly from the IndexOptInfo
		 * list without comparing the indexname, because the name of the hypothetical
		 * index cannot be obtained from the catalog.
		 */
		if (info->hypothetical ||
			/* for safety if get NULL rel name */
			!indexname)
		{
							  ...
							  Index rti, RangeTblEntry *rte)
{
	...
	/* POLAR: It is forbidden to force parallel without any control. */
	if (polar_enable_force_append_turn_on_parallel)
	{
		/*
