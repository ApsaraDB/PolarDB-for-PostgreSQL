...
find_word(char *str, int lenstr, char **endword, int *charlen)
{
	...
	while (beginword - str < lenstr && !iswordchr(beginword))
		beginword += pg_mblen_cstr(beginword);
...
make_bigrams(bigm *bptr, char *str, int bytelen, int charlen)
{
	...
	if (charlen < 2)
	{
		compact_bigram(bptr, ptr, pg_mblen_cstr(str));
		bptr->pmatch = true;
		bptr++;
		return bptr;
	}
	...
	if (bytelen > charlen)
	{
		...
		int			lenfirst = pg_mblen_cstr(str),
					lenlast = pg_mblen_cstr(str + lenfirst);
		...
		while ((ptr - str) + lenfirst + lenlast <= bytelen)
		{
			...
			lenfirst = lenlast;
			lenlast = pg_mblen_cstr(ptr + lenfirst);
		}
	}
	else
	{
