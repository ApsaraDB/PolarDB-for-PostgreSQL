...
function compile() {
  ...
  info "Begin configure, flags: $configure_flag"
  ./configure $configure_flag
  info "Begin compile and install PolarDB, flags: $make_flag"
...
function init_primary() {
  ...
  info "Begin initdb, flags: $initdb_flag"
...
function init_follower() {
  local follower_name_prefix=$1
  local follower_type=$2
  local follower_num=$3
  local follower_dir_prefix=$4
  local primary_port=$5

  for i in `seq 1 $follower_num`; do
    follower_name=$follower_name_prefix$i
    follower_dir=$follower_dir_prefix$i
    follower_port=$(random_unused_port)
    if [[ $follower_type == "standby" ]]; then
      local follower_datadir_prefix=$6
      $base_dir/bin/pg_basebackup -h127.0.0.1 -p$primary_port -D$follower_dir --polardata=$follower_datadir_prefix$i -X stream -v
      echo "polar_datadir = 'file-dio://$follower_datadir_prefix$i'" >> $follower_dir/postgresql.conf
    else
    local primary_dir=$6
    local data_dir=$7
    ...
    $base_dir/bin/psql -h127.0.0.1 -p$port postgres -c "SELECT pg_create_physical_replication_slot('$follower_name')"
    echo "port = $follower_port" >> $follower_dir/postgresql.conf
    echo "primary_conninfo = 'host=127.0.0.1 port=$port dbname=postgres application_name=$follower_name'" >> $follower_dir/postgresql.conf
    echo "primary_slot_name = $follower_name" >> $follower_dir/postgresql.conf
    touch $follower_dir/$follower_type.signal
    $base_dir/bin/pg_ctl -D $follower_dir start -c -o --cluster-name="${cluster_name-}$follower_name"
    connstr+="psql -h127.0.0.1 -p$follower_port postgres #$follower_name\n"

    if [[ $follower_type == "standby" && $replica_num > 0 ]]; then
      init_follower ${follower_name}_replica replica $replica_num ${follower_dir}_replica $follower_port $follower_dir $follower_datadir_prefix$i
    fi
  done
}
...
function init_cluster() {
  local primary_dir=$1
  local primary_data_dir=$2
  local primary_port=$3
  local standby_dir_prefix=$4
  local standby_data_dir_prefix=$5
  local replica_dir_prefix=$6

  init_primary $primary_dir $primary_data_dir $primary_port
  init_follower standby standby $standby_num $standby_dir_prefix $primary_port $standby_data_dir_prefix
  init_follower replica replica $replica_num $replica_dir_prefix $primary_port $primary_dir $primary_data_dir
}
...
quiet=on
linux_dist=$(awk -F= '/^ID=/{id=$2} /^VERSION_ID=/{vid=$2} END{print id vid}' /etc/os-release | tr -d '" ')
...
for arg do
  ...
  case "$arg" in
    ...
    --tde)                      initdb_flag+=" -C 'echo \"asdfasdf\"' -e aes-256" ;;
...
base_dir=$prefix/tmp_polardb_pg_17_base
pg_primary_dir=$prefix/tmp_polardb_pg_17_primary
pg_data_dir=$prefix/tmp_polardb_pg_17_data
pg_standby_dir_prefix=$prefix/tmp_polardb_pg_17_standby
pg_standby_data_dir_prefix=$prefix/tmp_polardb_pg_17_data_standby
pg_replica_dir_prefix=$prefix/tmp_polardb_pg_17_replica
...
if [[ $init == "on" ]] || [[ $clean == "on" ]]; then
  ...
  for dir in $pg_primary_dir $pg_standby_dir_prefix* $pg_replica_dir_prefix*; do
    ([[ -d $dir ]] && $base_dir/bin/pg_ctl -D $dir stop -mi) || true
  done
