...
```bash
docker run -it -v <src_to_polardb>:/home/postgres/work \
...
```bash
sudo chmod -R a+wr <src_to_polardb>
sudo chown -R postgres:postgres <src_to_polardb>
```
...
::: warning
后续所有步骤请以 **普通用户 `postgres`** 完成实例的编译和搭建。
:::
