...
在运行前默认已经通过文档 [实例部署：基于单机本地存储](deploy-on-local-storage.md#本地多节点-htap-实例) 中部署好本地多节点 HTAP 实例，总计 1 个主节点（运行于 5432 端口），2 个只读节点（运行于 5433/5434 端口）。可以通过下面的命令来验证：
